"""
train_yolov8n_night.py — Phase 6B.116 §11.47 fine-tune YOLOv8n for night/IR.
...
# Phase 6B.116: limit to classes the day model already handles well.
COCO_KEEP_CLASSES = [
...
def _download_exdark() -> None:
    ...
    NOTE: for Phase 6B.116 v1 we may skip ExDark and rely on native corpus
...
def _collect_native_night_frames(limit: int | None = None) -> list[Path]:
    ...
    Phase 6B.116 v1: only on-disk frames. Once retention captures are added
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Phase 6B.116: fine-tune YOLOv8n for nighttime surveillance"
    )
    parser.add_argument(
...
def main() -> int:
    ...
    print("=== train_yolov8n_night.py (Phase 6B.116 §11.47) ===")
