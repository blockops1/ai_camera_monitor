...
#   bash ai_camera_monitor/scripts/bootstrap-launchctl.sh
set -e
...
if [ -n "$NEW_PID" ]; then
    ...
    tail -30 ai_camera_monitor/logs/launchctl-stdout.log 2>/dev/null || echo "  (no stdout log)"
    echo "---"
    echo "LaunchAgent stderr:"
    tail -30 ai_camera_monitor/logs/launchctl-stderr.log 2>/dev/null || echo "  (no stderr log)"
    exit 1
fi