...
"""
PHASE-V2-018 autonomous driver.

Walks the PRD chain (US-018b through US-018j) serially with hard stops.
...
itself (docs/PHASE-V2-018-*.json) holds the source of truth for story
state.

Usage:
    .venv/bin/python3.11 scripts/run_phase_v2_018.py             # full run
    .venv/bin/python3.11 scripts/run_phase_v2_018.py --start US-018b  # from a specific story
    .venv/bin/python3.11 scripts/run_phase_v2_018.py --dry-run   # show plan only
"""
...
# For the operator's own machine, copy scripts/run_phase_v2_018.env.example to
# scripts/run_phase_v2_018.env (gitignored) and source it before running.
import os as _os
_missing = [k for k in ('DRIVER_REPO_PATH', 'OPERATOR_TELEGRAM_CHAT_ID') if not _os.environ.get(k)]
if _missing:
    raise SystemExit(
        f'Missing required env: {_missing}. '
        f'Source scripts/run_phase_v2_018.env or set them in the environment.'
    )
REPO = Path(_os.environ['DRIVER_REPO_PATH'])
PRD_PATH = REPO / 'docs' / 'PHASE-V2-018-PRD-private-data-externalization.json'
LOG_PATH = REPO / 'logs' / 'phase_v2_018_driver.log'
STATE_PATH = REPO / 'logs' / 'phase_v2_018_state.json'
...
def build_card_body(story: dict) -> str:
    ...
    body = f"""**Story (PHASE-V2-018 / {story['id']}).** {story['title']}
...
The PRD defines an external scanner (US-018i) that owns the canonical leak-pattern list. Until US-018i lands, do not name specific operator handles, IPs, or chat identifier digits in the ACs or PR descriptions — they leak by reference. Verify literal patterns are absent using simple grep.

## Commit message template

`fix(v2): {story['id']} — <one-line summary>`
"""
    # Trim if over 2500
    if len(body) > 2500:
        body = body[:2497] + '...'
...
def main():
    ...
    # Mark US-018i and US-018j done if reached
    log('DRIVER FINISHED')
    notify('PHASE-V2-018 driver finished all reachable stories')
    return 0
