...
STATUS: provisional (Phase.166 §11.87.7, 2026-08-30 — rewrote from
    <legacy-repo>/ to use the canonical dict-wrapped schema that
    known_vehicles/store.py has used since Phase.83. Prior version
...
INPUTS:
    - env FARMSURV_DATA_DIR (default $FARMSURV_PROJECT_ROOT/data or
      ~/ai_camera_monitor/data) — root of the data tree.
    ...
    - CLI args:
        ...
        --label         (required): human label, e.g. "name four's Chevy".
...
DOES NOT DO:
    ...
    - Mint stable V-NNN identities — Phase.57 (2026-08-05) retired
...
WHY HERE:
    Phase.166 §11.87.7 (2026-08-30) rewrite. Old version (copied
    from <legacy-repo>/scripts/) wrote the known_vehicles.json
...
RELATED:
    ...
    - Phase.83 (2026-08-16) — schema migration to dict-wrapped form
    - Phase.57 (2026-08-05) — V-NNN identity layer retired
"""
...
def _label_to_id(label: str) -> str:
    ...
    Examples:
        "name four's Chevy pickup" -> "v_name_fours_chevy_pickup"
        "the operator's blue Tesla"      -> "v_mr_vs_blue_tesla"
    """
...
def enroll(
    ...
    *,
    ...
    Args:
        ...
        label: Human label, e.g. "name four's Chevy pickup".
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        help="Alert id from the Telegram notification (UUID — Phase.57 retired V-NNN suffix)",
    )
    parser.add_argument(
        "--date", default=None,
    ...
    parser.add_argument(
        "--label", required=True,
        help='Human label, e.g. "name four\'s Chevy pickup"',
    )
