#!/usr/bin/env python3
"""
enroll_person.py — Phase.106 / 6B.163 person enrollment CLI.
...
attributes via Qwen3.6 (Phase.163), and writes a JSON identity
...
STATUS: provisional (Phase.163; will stabilize after first live
    enroll with stable attributes)
THREAD SAFETY: single-threaded (interactive CLI)

INPUTS:
    - CLI args:
        --name    (required): display name (e.g. "name one", "name two")
    ...
    - JSON identity file at data/identities/<slug>.json with shape:
      {
        ...
        "stable_attributes": {        <-- Phase.163 (optional)
...
WHY HERE:
    Phase.106 step 10 (initial enrollment); Phase.163 adds
...
CALLED BY:
    - manual invocation: source .venv/bin/activate && python3 scripts/enroll_person.py --name name one

CALLS INTO:
    - infra.face_recognition: recognize_faces (captures embeddings)
    - infra.faces: save_identity, load_identity, add_enrollment_sample
    - infra.persistent_rtsp: get_reader_for_url (Phase.163)
    - infra.vision_analyzer: analyze_frames(mode="person") (Phase.163)
...
RELATED:
    ...
    - infra/persistent_rtsp.py — 24/7 ring buffer (Phase.163)
    - infra/vision_analyzer.py — Qwen3.6 wrapper (Phase.163)
    - infra/person_prompt_template.py — person schema (Phase.163)
...
# Phase.183 — ensure repo root is on sys.path so `import infra.*`
...
EXPECTED_VENV = "<install-path>/ai_camera_monitor/.venv"
...
# Phase.167 §13.4 Commit 17 (T3 C17): the hardcoded IP→name table
...
def _camera_name_by_ip(ip: str) -> str | None:
    ...
    Phase.167 §13.4: replaces the (deleted) CAMERA_NAME_BY_IP
...
def _resolve_rtsp_reader(
    ...
    """Phase.163 — resolve a persistent RTSP reader for the given camera.
...
def _capture_samples_from_reader(
    ...
    """Phase.163 — capture N face samples from the persistent reader.
...
def _capture_samples_one_off(
    ...
    """Fallback path (Phase.163): open a one-off RTSP connection.
...
def _capture_samples(
    ...
    """Phase.163 — capture N face samples.
...
def _capture_samples_from_crops(
    ...
    """Phase.163 — capture face samples from existing JPEG/PNG crops.
...
# Phase.163 — stable attribute enum validators (mirror
...
def _extract_stable_attributes_from_qwen(
    ...
    """Phase.163 — send face crops to Qwen3.6 for stable attribute extraction.
...
def _confirm_or_edit_stable_attributes(
    ...
    """Phase.163 — CLI confirm/edit extracted stable attributes.
    ...
    """
    print(f"\n--- Stable attributes for {name!r} (Phase.163) ---")
...
def _prompt_stable_attributes_from_scratch(
    ...
    """Phase.163 — operator-driven stable-attribute entry (no Qwen3.6).
...
def main() -> int:
    ...
    parser.add_argument(
        "--name",
        required=True,
        help='Display name (e.g. "name one", "name two")',
    )
    parser.add_argument(
        "--role",
        default="unknown",
    ...
    parser.add_argument(
        "--no-stable-attrs",
        action="store_true",
        help="Skip Qwen3.6 stable-attribute extraction (Phase.163). "
    ...
    # Resolve camera_name for Qwen3.6 prompt (Phase.163).
    ...
    # Capture samples (Phase.163: uses persistent reader when available;
    ...
    # Phase.163 — extract stable attributes from the same crops.
