...
"""
PHASE-V2-019 autonomous driver.

Walks the PRD chain (US-019a through US-019last) serially with hard stops.
...
itself (docs/PHASE-V2-019-*.json) holds the source of truth for story
state.

Usage:
    .venv/bin/python3.11 scripts/run_phase_v2_019.py             # full run
    .venv/bin/python3.11 scripts/run_phase_v2_019.py --start US-019a  # from a specific story
    .venv/bin/python3.11 scripts/run_phase_v2_019.py --dry-run   # show plan only
"""
...
# For the operator's own machine, copy scripts/run_phase_v2_019.env.example to
# scripts/run_phase_v2_019.env (gitignored) and source it before running.
import os as _os
_missing = [k for k in ('DRIVER_REPO_PATH', 'OPERATOR_TELEGRAM_CHAT_ID') if not _os.environ.get(k)]
if _missing:
    raise SystemExit(
        f'Missing required env: {_missing}. '
        f'Source scripts/run_phase_v2_019.env or set them in the environment.'
    )
REPO = Path(_os.environ['DRIVER_REPO_PATH'])
PRD_PATH = REPO / 'docs' / 'PHASE-V2-019-PRD-remove-image-resize.json'
LOG_PATH = REPO / 'logs' / 'phase_v2_019_driver.log'
STATE_PATH = REPO / 'logs' / 'phase_v2_019_state.json'
...
def build_card_body(story: dict) -> str:
    ...
    body = f"""**Story (PHASE-V2-019 / {story['id']}).** {story['title']}
...
def main():
    ...
    log('DRIVER FINISHED')
    notify('PHASE-V2-019 driver finished all reachable stories')
    return 0
