...
CALLED BY:
    - operator shell: scripts/configure_webhook.py 192.168.1.39
...
# Default webhook endpoint. Overridden by WEBHOOK_URL env var or --webhook-url.
_DEFAULT_WEBHOOK_URL = "http://192.168.1.111:8090/alert"
...
def main():
    ...
    ap.add_argument("ip", nargs="?", help="Single camera IP (default: --all)")
    ap.add_argument("--all", action="store_true",
                    help="Configure webhook on all cameras in camera-creds.env")
    ap.add_argument("--headed", action="store_true",
                    help="Show browser window")
    ap.add_argument("--webhook-url", default="",
                    help=f"Webhook URL (default: $WEBHOOK_URL or {_DEFAULT_WEBHOOK_URL})")
    ...
    if args.all:
        ...
        for label, info in cameras.items():
            if info["ip"] == args.ip:
                ...
                break
        if match is None:
            print(f"ERROR: IP {args.ip} not found in {creds_env}", file=sys.stderr)
