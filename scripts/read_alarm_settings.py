...
Phase.166 §11.87.5: camera list comes from infra.camera_creds (the
...
CALLED BY:
    ...
    - operator shell: scripts/read_alarm_settings.py 192.168.1.103
...
# labels (e.g. 'Outside Front Solar') — those labels match config/motion_recipe.json
# per-camera override keys.
#
# The hardcoded ALL_CAMERAS list was removed 2026-08-30 in §11.87.5.
#
# Cameras on this property (verified 2026-08-02 firmware inventory):
#    Outside Front Solar  — 192.168.1.103  (gatekeeper)
#    Outside Front Garage — 192.168.1.73
#    Outside Front Power  — 192.168.1.108
#    Outside Back Solar   — 192.168.1.113
#    (RLC-833As not on this script — they don't share the same Alarm Settings
#    page layout.)
...
def open_alarm_settings(page) -> None:
    ...
    Critical context — burned 2026-07-26:
      ...
      click the upper-left quadrant. Note: "the gear is weird. I had to click
    ...
    """)
    if info:
        # Click upper-left part per Note's hint. Sometimes the click
                    ...
                    return inputs.map(inp => {
                        ...
                        while (el && el !== document.body) {
                            ...
                            if (cls.includes('smart-sensitivity-box')) {
                                ...
                                // Verified DOM hierarchy (2026-07-26 on RLC-510A 192.168.1.103):
                                //
                                // Smart Detection:
...
def main():
    ...
    ap.add_argument("ip", nargs="?", help="Single camera IP (default: all from camera-creds.env)")
    ...
    # Phase.166 §11.87.5 — recipe comparison opt-in
    ...
    if args.ip:
        # Single-camera path: find label by IP
        targets = []
        for label, info in cameras.items():
            if info.get("ip") == args.ip:
                targets.append((label, info["ip"]))
                break
        else:
            print(
                f"FATAL: no creds for IP {args.ip} in {CAMERA_CREDS_FILE}",
                file=sys.stderr,
            )
            sys.exit(2)
    else:
        targets = [(label, info["ip"]) for label, info in cameras.items() if info.get("ip")]
