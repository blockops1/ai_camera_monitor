...
def test_perfect_match_returns_matchverdict():
    """Phase.84 absence-evidence fix — perfect match score is 5.0
...
def test_perfect_match_has_wide_gap():
    """name two's pickup matches strongly; gap to jayco is large."""
    sig = _perfect_carson_signature()
    v = match_signature(sig, _known_vehicles())
    assert isinstance(v, MatchVerdict)
    assert v.gap > 1.0
...
def test_below_gap_returns_no_match():
    ...
    # Add a 3rd vehicle that's very close to name two's.
    close = {
        "id": "v_carson_clone",
        "label": "name two's white pickup (clone)",
        "owner": "name two",
        "color": "white",
        "type": "pickup",
        "make": "GMC",
...
def test_custom_confidence_threshold():
    """With a high threshold, even a perfect name two match fails."""
...
def test_d99a38e6_carson_pickup_bug_scenario():
    """The original d99a38e6 bug: name two's white pickup mis-scored
    ...
    a regression test that asserts:
      ...
      1. name two still wins — but for the RIGHT reasons (color+type+
    ...
    assert isinstance(v, MatchVerdict)
    assert v.known_vehicle["id"] == "v_carson_white"
    # Phase.84 absence-evidence fix. With both sig and kv having
