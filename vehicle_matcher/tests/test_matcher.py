...
def test_perfect_match_has_wide_gap():
    """<visitor-name>'s pickup matches strongly; gap to jayco is large."""
    sig = _perfect_carson_signature()
    v = match_signature(sig, _known_vehicles())
    assert isinstance(v, MatchVerdict)
    assert v.gap > 1.0
...
def test_below_gap_returns_no_match():
    ...
    # Add a 3rd vehicle that's very close to <visitor-name>'s.
    close = {
        "id": "v_carson_clone",
        "label": "<visitor-name>'s white pickup (clone)",
        "owner": "<visitor-name>",
        "color": "white",
        "type": "pickup",
        "make": "GMC",
...
def test_custom_confidence_threshold():
    """With a high threshold, even a perfect <visitor-name> match fails."""
...
def test_d99a38e6_carson_pickup_bug_scenario():
    """The original d99a38e6 bug: <visitor-name>'s white pickup mis-scored
    ...
    a regression test that asserts:
      ...
      1. <visitor-name> still wins — but for the RIGHT reasons (color+type+
