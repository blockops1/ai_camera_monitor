...
def _score_candidate(
    ...
    if sig_make and sig_model:
        ...
        if score > 0 and _normalize_color(sig_color) and kv_color:
            if _normalize_color(sig_color) == _normalize_color(kv_color):
                score += 1
    ...
    if sig_make and not sig_model:
        ...
        if kv_model and sig_make.lower() in str(kv_model).lower():
            score += 1
        if _normalize_color(sig_color) and kv_color:
            if _normalize_color(sig_color) == _normalize_color(kv_color):
                score += 1
        if score > 0:
            return score, False
