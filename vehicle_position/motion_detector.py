...
Phase 6B.115 history: previously this module wrapped `detect_motion()`
...
# Phase §11.90 (2026-09-01): MovingObject consolidated into infra.motion_types.
# PositionResult stays here (refactor vocabulary; deliberate rename of MotionResult).
from infra.motion_types import MovingObject
...
class PositionResult:
    ...
    reference_method: str = "gate"  # Phase 6B.115: always "gate"
...
def build_motion_result_from_gate(
    ...
    *,
    ...
    §11.176 (2026-09-06): subject_bbox_a and subject_bbox_b are the
    gate's per-frame subject bboxes (AND of consecutive masks) for
    frames 2 and 3 respectively. Passing them yields a 4-frame trajectory
    with tight subject localization for the middle frames and computed
    positions for frames 1 and 4 (vs the prior "absent → absent → ...").
    ...
    Args:
        ...
        subject_bbox_a: gate's per-frame subject bbox in frame_2 (optional)
        subject_bbox_b: gate's per-frame subject bbox in frame_3 (optional)
