...
* ``build_minimal_motion_telegram_body`` — single-frame, single-line
  ...
  this for the lead motion Telegram per the operator's "just one picture
...
def _format_header(input: MotionTelegramInput) -> str:
    ...
    Phase.167 §13.5 (Commit 13): resolve via the registry so the
...
def build_motion_telegram_body(input: MotionTelegramInput) -> str:
    ...
    interrupt the word-flow. Event time, not send time, per the operator
...
def build_minimal_motion_telegram_body(
    ...
    """Build the minimal lead motion Motion Telegram body (Phase.89).
    ...
    fields that cluttered the alert per the operator's 2026-08-18 instruction.
