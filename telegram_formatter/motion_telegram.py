...
* ``build_minimal_motion_telegram_body`` — single-frame, single-line
  identification + confidence. Phase 6B.89 (PLAN.md §11.20) introduced
  this for the lead motion Telegram per maintainer's "just one picture
...
class MotionTelegramInput:
    ...
    Attributes:
        ...
        alert_id:              Optional alert identifier, kept ONLY for log
                               correlation. Phase 6B.114 removed it from
...
def _format_header(input: MotionTelegramInput) -> str:
    """Header line: camera name only. No alert_id, no timestamp (Phase 6B.114).

    The captured_at_iso goes in a footer line at the end of the body
    (per maintainer OOB — "the date and time the alert is sent at the end
    of the alert text"). Putting it in the header interrupted the
    word-flow of the body.

    Phase 6B.167 §13.5 (Commit 13): resolve via the registry so the
    header reflects the canonical display name. The caller may pass
    either a code (CAM1/FRONT) or a name; display_name_for returns
    spec.name. Falls back to the input string when the identifier
    isn't in the registry (test fixtures, legacy callers).
    """
    return f"🚗 Motion — {display_name_for(input.camera_name)}"
...
def build_motion_telegram_body(input: MotionTelegramInput) -> str:
    ...
    interrupt the word-flow. Event time, not send time, per maintainer
...
# --- minimal body (Phase 6B.89, PLAN.md §11.20) ---------------------------
...
def _build_vehicle_description(vehicle: dict[str, Any]) -> str:
    ...
    Priority order matches ``format_motion_alert_vehicle_line`` (Phase 6B.77):
...
def build_minimal_motion_telegram_body(
    ...
    """Build the minimal lead motion Motion Telegram body (Phase 6B.89).
    ...
    fields that cluttered the alert per maintainer's 2026-08-18 instruction.
    ...
    lines: list[str] = []
    # Phase 6B.167 §13.5 (Commit 13): resolve via the registry so the
    # header reflects the canonical display name. See _format_header
    # for the lookup contract and fallback behavior.
    lines.append(f"🚗 <b>Motion — {display_name_for(input.camera_name)}</b>")
    if input.captured_at_iso:
        lines.append(input.captured_at_iso)
