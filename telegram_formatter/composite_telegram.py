...
Phase 6B.111 (2026-08-21) — RESTORED after the 6B.105c slim dropped it.

The composite Telegram was the SECOND Telegram in the legacy lead motion message
stack (motion + composite + match). Phase 6B.79 (2026-08-14) introduced
render_motion_composite; Phase 6B.105c (2026-08-21) slimmed listener.py
...
CALLED BY:
  - listener/vehicle_event_pipeline.py:emit_result_stage (Phase 6B.111)
...
class CompositeTelegramInput:
    ...
    detected" per maintainer's spec (2026-08-21).
...
def build_composite_telegram_body(input: CompositeTelegramInput) -> str:
    ...
    The header uses <b>Vehicle in motion at</b> per maintainer's spec
    ...
    Phase 6B.114 (2026-08-25): maintainer OOB — "the date and time the
    ...
    maintainer's correction ("it is actually fine to leave it as the
    webhook time").
    """
    lines: list[str] = []
...
def send_composite_alert(
    ...
    Phase 6B.115 (§11.46.6, 2026-08-25):
