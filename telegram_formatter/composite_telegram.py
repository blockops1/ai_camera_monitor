...
The composite Telegram was the SECOND Telegram in the legacy OFS message
...
RELATED:
  - infra.motion_detector.MovingObject — supplies bbox_per_frame + trajectory
...
class CompositeTelegramInput:
    ...
    The composite Telegram (Telegram #2 in the OFS message stack — see
...
def build_composite_telegram_body(input: CompositeTelegramInput) -> str:
    ...
    Layout (Telegram #2 in the OFS vehicle-in-motion message stack):
    ...
    """
    lines: list[str] = []
    lines.append(f"🚗 <b>Vehicle in motion at {input.camera_name}</b>")
