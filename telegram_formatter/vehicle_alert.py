...
PUBLIC API:
    format_qwen_confidence_line(vision_result=None) -> str
        Render the Qwen confidence line for the lead motion Telegram.
...
DOES NOT DO:
    ...
    - Decide which vehicle fields are "interesting" → emit everything Qwen
      returns (except matcher-injected fields) per maintainer OOB 2026-08-11

WHY HERE:
    Phase 6B.106 extraction (2026-08-21). These helpers lived at the top of
...
def format_qwen_confidence_line(vision_result=None) -> str:
    """
    Phase 6B.81 (PLAN.md §11.14.3.B): render the Qwen confidence line for
    the lead motion Telegram. Reads `vision_result["confidence"]`
    (a float 0.0-1.0) and formats it as either `   qwen confidence: 0.62`
    (2-decimal float) or `   qwen confidence: (empty)` (when missing).

    The empty case is explicit so maintainer can distinguish "Qwen returned
...
def format_detector_metadata_lines(motion_result=None) -> list:
    """
    Phase 6B.81 (PLAN.md §11.14): render the detector metadata section of
    the lead motion Telegram. Surfaces the motion detector's structured
    ...
    Args:
        motion_result: a `MotionResult` from `infra.motion_types` (Phase §11.90, 2026-09-01).
            May
    ...
    """
    # Phase 6B.81: the burst is 6 frames (motion_detector docstring
...
def format_motion_alert_vehicle_line(
    ...
    """Phase 6B.77 (2026-08-11) — render Qwen's identification verbatim.
    ...
    for <visitor-name>'s white pickup truck when the matcher wrongly scored
    ...
    Source: maintainer OOB 2026-08-11: "what I want sent in the motion
...
# Phase 6B.78 (2026-08-11) — generic Qwen-dict renderer.
#
# maintainer OOB 2026-08-11: "when I tell you that I want all the output of
...
def annotate_frame_bboxes(
    ...
    """
    Phase 6B.81 (PLAN.md §11.14): draw the detector's per-frame bboxes onto
    the captured frames that accompany the lead motion Telegram. The
    ...
    for i, fp in enumerate(frame_paths):
        ...
        try:
            # Phase 6B.86 (PLAN.md §11.18): bbox_per_frame[i+1] describes
