...
def test_includes_edt_timestamp_string():
    """Phase 6B.99 (PLAN.md §11.26): match Telegram body must carry the EDT
...
def test_includes_matched_label_only_not_id():
    """Phase 6B.121: slim body shows just the label, not (id) + (label) duplicated.
    Pinned: maintainer 2026-08-22 "the old title, what it matched to".
    """
    inp = MatchTelegramInput(
        camera_name="OFS",
        captured_at_iso="t",
...
def test_drops_breakdown_dimensions():
    """Phase 6B.121: per-dimension score breakdowns are gone.
...
def test_drops_thresholds_line():
    """Phase 6B.121: thresholds line removed (was debug noise)."""
    inp = MatchTelegramInput(
        camera_name="OFS",
        captured_at_iso="t",
        verdict=_verdict(),
...
def test_runner_ups_limited_to_two():
    """Phase 6B.121: top-2 runner-ups (maintainer OOB: "the two runner-up").
...
def test_runner_ups_section_omitted_when_only_one_candidate():
    """Phase 6B.121: no Runner-ups section when only the match exists."""
    inp = MatchTelegramInput(
        camera_name="OFS",
        captured_at_iso="t",
...
def test_includes_owner_when_present():
    ...
    body = build_match_telegram_body(inp)
    assert "Owner: <visitor-name>" in body
...
def test_includes_color_make_model_no_body():
    """Phase 6B.121: slim body shows Color + Make/Model but drops Body type.

    "Body: pickup" was redundant with the label "<visitor-name>'s white pickup".
    """
    inp = MatchTelegramInput(
        camera_name="OFS",
        captured_at_iso="t",
...
def test_alert_id_removed_phase_6b114():
    """Phase 6B.114: alert_id removed from header (diagnostic noise).
    ...
    body = build_match_telegram_body(inp)
    assert "[abc123]" not in body  # Phase 6B.114
...
# ---------------------------------------------------------------------------
# Phase 6B.167 §13.5 Commit 13: display-name lookup contract.
# ---------------------------------------------------------------------------


def test_header_uses_display_name_for_lookup(monkeypatch):
    """Phase 6B.167 §13.5 (Commit 13): the header resolves the camera
