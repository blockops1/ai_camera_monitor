"""Unit tests for build_minimal_motion_telegram_body (Phase 6B.89).

Pins the exact body shape so the OFS lead motion alert can't regress
silently. maintainer OOB 2026-08-18: "just one picture, the fourth frame, and
...
def test_body_includes_camera_name():
    ...
    body = build_minimal_motion_telegram_body(inp)
    assert "Outside Front Solar" in body
    assert body.startswith("🚗 <b>Motion — Outside Front Solar</b>")
...
def test_body_handles_missing_vision_result():
    ...
    assert "🚗 <b>Motion — Outside Front Solar</b>" in body
...
def test_body_does_not_leak_matcher_or_pipeline_fields():
    """maintainer OOB 2026-08-11: motion body must NEVER show matcher output.
        ...
        vision_result=_qwen_response(vehicles=[
            {
                ...
                "identified_owner": "<owner-name>",  # matcher's verdict
                "identified": True,
...
def test_minimal_body_header_uses_display_name_for_lookup(monkeypatch):
    """Phase 6B.167 §13.5 (Commit 13): the minimal body header reflects
