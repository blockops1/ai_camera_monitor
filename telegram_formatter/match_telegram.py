"""Match Telegram body + sender (Phase.112).
...
def build_match_telegram_body(input: MatchTelegramInput) -> str:
    ...
    Phase.121 (2026-08-22) — slim body per Note:
    ...
    # Header (kept slim — Phase.114).
    lines.append(f"✅ Match — {input.camera_name}")
...
def _format_kv_known_slim(verdict: MatchVerdict) -> str:
    """Slim known-vehicle details for the match body (Phase.121).
...
# ============================================================================
# Phase.112 — Send + render. Body builders above are pure functions;
# the helpers below do I/O. TG#3 in the OFS message stack — fires
# AFTER TG#1 (arriving) and TG#2 (vehicle in motion + composite
# motion-trail).
#
# Phase.142 (2026-08-27, Note): TG#3 is now send_message(body)
...
def send_match_alert(
    ...
    Phase.142 (2026-08-27, Note): body via send_message, then
    ...
    # Phase.142 (2026-08-27, Note): send the body as a text
    ...
    # and sending it as a single photo with caption. Note specifically
    ...
    log_outbound_telegram(
        ...
        body=body,
        sent=bool(sent),
        extra=f"score={match_telegram_input.verdict.score:.2f} "
...
def send_no_match_alert(
    ...
    Phase.142 (2026-08-27, Note): mirror send_match_alert —
    ...
    log_outbound_telegram(
        ...
        body=body,
        sent=bool(sent),
        extra=f"reason={no_match_telegram_input.no_match.reason} "
              f"crop_count={len(crop_paths)}",
        image_paths=list(crop_paths),
    )
    return bool(sent)
