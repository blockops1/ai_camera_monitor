...
def test_add_existing_replaces():
    ...
    updated["label"] = "name two's NEW white pickup"
    store.add(updated)
    assert len(store) == 1
    fetched = store.get_by_id("v_carson_white")
    assert fetched is not None
    assert fetched["label"] == "name two's NEW white pickup"
...
def test_load_known_vehicles_default_path(monkeypatch, tmp_path):
    """Phase.83 (2026-08-16): when no path is provided, load from the
...
def test_load_known_vehicles_default_path_missing_raises(monkeypatch, tmp_path):
    """Phase.83: if the default path doesn't exist, fail loudly.
...
def test_load_known_vehicles_real_default_path():
    ...
    Catches schema-vs-data drift (Phase.83 found the real file was
