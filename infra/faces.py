"""
faces.py — Identity storage for ai_camera_monitor.

Lifted 2026-08-22 from <legacy-repo>/src/faces.py
(archived at ~/archive/2026-08-22-phase6b106-prelift-archive/) for
Phase.106 (person-gatekeeper tier). Logic is byte-identical; only
...
OUTPUTS:
    ...
    - Phase.163: `stable_attributes` block is preserved through save/
...
PUBLIC API:
    ...
    _slug_from_name(name) -> str — "the operator" → "mr_v", non-alnum → _
...
def save_identity(identity: dict) -> str:
    ...
    # Phase.163 — stable visual attributes (silhouette, skin_tone,
