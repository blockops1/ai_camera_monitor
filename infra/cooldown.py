...
PUBLIC API:
    ...
    is_in_vision_block_cooldown(cooldown_seconds: int = 1800) -> bool
        Global rate-limit for the per-alert "🔍 VISION_OBSERVATIONS" Telegram.
        ...
        photo. Default 1800s (30 min). Phase 6B.101.
    ...
    MotionCooldown(window_seconds: int = 60)
        ...
        listener.py Phase 6B.108 (2026-08-21). The standard alert path has
...
DOES NOT DO:
    ...
    - Track matcher failures → that lives in infra.matcher_failures (Phase 6B.108)

WHY HERE:
    Phase 6B.108 extraction (2026-08-21). Originally `MotionCooldown` lived
...
# Phase 6B.101 — global rate-limit on the per-alert "🔍 VISION_OBSERVATIONS"
...
# Used to kill the Outside Back Solar overnight flood: 80+ webhooks with
# different UUIDs but the same underlying event type.
_bucket_cooldown: dict[str, float] = {}

# Phase 6B.101 — single global key (the alert body always sends; this map
# only throttles the optional "🔍 VISION_OBSERVATIONS" Telegram block).
...
def is_in_vision_block_cooldown(cooldown_seconds: int = DEFAULT_VISION_BLOCK_COOLDOWN) -> bool:
    ...
    Phase 6B.101. Single key ("_last_sent") — true rate-limit, not per-camera.
...
# (Phase 6B.108, extracted from listener.py 2026-08-21)
# ---------------------------------------------------------------------------
