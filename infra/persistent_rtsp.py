...
OUTPUTS:
    ...
    - writes file: <output_dir>/<camera>_<ts>.jpg when get_recent_frames
      is called (saves the ring buffer's frames as JPEGs)
...
PUBLIC API:
    set_reader(camera_name: str, reader: PersistentRTSPReader) -> None
        Register a reader under its canonical camera name (e.g.
        "Outside Front Solar"). Replaces any prior reader under the
        same name. Called at listener bootstrap.
    get_reader(camera_name: str) -> PersistentRTSPReader | None
        Look up a reader by canonical name. Returns None if absent.
    ...
    PersistentRTSPReader(rtsp_url, ring_size=RING_SIZE_DEFAULT, ...,
        ...
        .get_recent_frames(n, max_size) -> list[str]
            Snapshot the last N decoded frames as JPEGs.
        .get_frames_by_offset(indices, max_size) -> list[str]
            Snapshot specific ring buffer indices as JPEGs.
        .is_healthy() -> bool
...
DOES NOT DO:
    ...
    - Crop generation — infra.motion_detector owns that
    - Match vehicles — infra.vehicle_matcher owns that

WHY HERE:
    The Reolink 510A at OFS has a kernel-side pre-buffer that dumps
    ...
    2026-08-16 — Proactive hourly reconnect (Phase.80, PLAN §11.13).
    PyAV's container.demux() blocks indefinitely on a zombie socket
    without raising — observed after 41h of uptime on OFS (frames
    ...
    2026-08-17 — Multi-camera registry (Phase.87, PLAN §11.17).
...
CALLED BY:
    ...
    - listener.listener.bootstrap: set_reader() per gatekeeper camera
...
RELATED:
    ...
    - data/frames/<camera>_<ts>/ — the JPEGs this module writes

Slot management:
    The camera caps concurrent RTSP sessions at 3-4. We hold:
    - 1 slot for Outside Front Solar 24/7
    - 1 slot for Outside Front Garage 24/7 (since Phase.87)
    <nas> Surveillance Station holds another. The mobile app and
    any browser sessions use the rest. Two slots is within budget.
...
RING_SIZE_DEFAULT = 180  # ~12s lookback at 15fps — supports 6-frame bursts
                      ...
                      # pre-event motion trail Note asked for on 2026-08-06.
                      # NOTE: 2026-08-07 Phase.60 attempted to bump to 240
                      # to support offsets [60,90,120,150,180,210], but Note
...
# 2026-08-28 — Phase.155 (PLAN §11.78). Cap on consecutive failure-driven
...
# 2026-08-16 — Phase.80 (PLAN §11.13). Proactive hourly reconnect.
...
# --------------------------------------------------------------------------
# Module-level registry (Phase.87, PLAN §11.17)
# --------------------------------------------------------------------------
...
def set_reader(camera_name: str, reader: PersistentRTSPReader) -> None:
    """Register a reader under its canonical camera name.

    Replaces any prior reader registered under the same name. Called at
    listener bootstrap, once per gatekeeper camera (OFS, OFG, ...).
    """
    with _registry_lock:
        _readers[camera_name] = reader
        log.info(
            "PersistentRTSPReader registered for %s (url=%s)",
            camera_name, reader._rtsp_url.split("@")[-1] if "@" in reader._rtsp_url else reader._rtsp_url,
        )
...
def get_reader(camera_name: str) -> PersistentRTSPReader | None:
    """Look up a reader by canonical name. Returns None if absent."""
    with _registry_lock:
        return _readers.get(camera_name)
...
def _strip_rtsp_creds(rtsp_url: str) -> str:
    ...
    Example: rtsp://admin:secret@192.168.1.103:554/path
          -> rtsp://192.168.1.103:554/path
...
class PersistentRTSPReader:
    ...
    JPEG-encoded bytes (q85) into a deque. The main thread can call
    .get_recent_frames(n) at any time to get the most recent N frames as
    JPEG file paths.

    2026-08-28 — Ring buffer now stores JPEG bytes (~720 KB/frame for typical
    outdoor 2304x1296 footage) instead of decoded PIL.Image (~8.54 MB/frame).
    Memory savings: ~1.5 GB RSS at 90-frame ring × 2 cameras. JPEG encoding
    happens once on ingest (~17% of one CPU core at 30 fps total); decode
    happens on demand when an alert is captured (~67 ms for 12 frames).
    Public API unchanged: get_recent_frames() / get_frames_by_offset() still
    return the same JPEG file paths.
    """
    ...
    def __init__(
        ...
        Args:
            ...
            scheduled_reconnect_seconds: 2026-08-16 (Phase.80, PLAN
            ...
            max_reconnect_attempts: 2026-08-28 (Phase.155, PLAN
        ...
        # 2026-08-28: ring holds JPEG-encoded bytes (q85), not PIL.Image.
        # Memory cost: ~720 KB/frame instead of ~8.54 MB/frame at 2304x1296.
    ...
    def stream_fps(self) -> float:
        ...
        Phase.160 (2026-08-28): added so motion_gate can compute fps-aware
    ...
    def ring_size(self) -> int:
        ...
        Phase.160 (2026-08-28): exposed so motion_gate can compute
    ...
    def start(self) -> None:
        ...
        self._thread.start()
        # 2026-08-16 — Phase.80 (PLAN §11.13). Watchdog closes the
    ...
    def get_recent_frames(
        ...
        """Return the most recent N frames as JPEG file paths.

        Snapshots the current ring buffer, writes each frame to disk in
        output_dir/frame_001.jpg, frame_002.jpg, etc. Returns the list of
        ...
        Args:
            ...
            output_dir: Directory to write frame_001.jpg ... frame_NNN.jpg.
        ...
        for i, frame_bytes in enumerate(frames, start=1):
            ...
            # Decode JPEG bytes back to PIL.Image. Memory diet 2026-08-28:
            # ring stores JPEG bytes, decoded on demand here.
    ...
    def get_frames_by_offset(
        ...
        """Return frames at specific deque indices as JPEG file paths.
        ...
        Args:
            indices: List of deque positions to sample. Order is preserved
                in the output — frame_001.jpg = indices[0], frame_002.jpg =
                indices[1], etc.
            output_dir: Directory to write frame_001.jpg ... frame_NNN.jpg.
            max_size: (width, height) to downscale to. Aspect preserved.
                If None, no resizing (full resolution).
    ...
    def _scheduled_reconnect_watchdog(self) -> None:
        """2026-08-16 — Phase.80 (PLAN §11.13). Proactive cycle of
    ...
    def _run_loop(self) -> None:
        ...
        Phase.155 (PLAN §11.78): failure-driven reconnects are capped at
        ...
        Per the 2026-08-28 OFG incident (6 errors in 80s with ERRNO 60 /
        ...
        while not self._stop_event.is_set():
            try:
                ...
                # 2026-08-28 — Phase.155 (PLAN §11.78). Cap on
    ...
    def _decode_iteration(self) -> None:
        ...
        for packet in container.demux(stream):
            ...
            for frame in packet.decode():
                ...
                if frame is None:
                    continue
                # Convert to PIL.Image once, encode to JPEG bytes, store in ring.
                # Memory diet: storing JPEG bytes (~720 KB/frame) instead of
                # the decoded PIL.Image (~8.54 MB/frame) cuts ring-buffer RSS
                # by ~12x. Decoded frames are reconstructed on demand when
                # get_recent_frames / get_frames_by_offset is called.
                try:
                    img = frame.to_image()
                    buf = io.BytesIO()
                    ...
                    log.debug(f"frame JPEG encode failed: {e}")
                    continue
                with self._ring_lock:
                    self._ring.append(jpeg_bytes)
