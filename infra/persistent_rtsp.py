...
PUBLIC API:
    ...
    set_reader_by_code(code: str, reader: PersistentRTSPReader) -> None
        ...
        "FRONT"). Primary API going forward (Phase.167 §13.5
...
WHY HERE:
    ...
    camera; with CAM2 joining the gatekeeper tier
    ...
    2026-08-30 — Code-keyed registry (Phase.167 §13.5 Commit 10).
    The listener grew to register persistent readers for every camera
    (Phase.160), and the registry key is shifting from operator
...
Slot management:
    The camera caps concurrent RTSP sessions at 3-4. As of Phase.160
    ...
    keyed by CameraSpec.code (Phase.167 §13.5 Commit 10); the
    listener iterates over `infra.cameras.load_cameras()` and calls
    `set_reader_by_code(spec.code, reader)` per camera. <nas>
...
RING_SIZE_DEFAULT = 180  # ~12s lookback at 15fps — supports 6-frame bursts
                      ...
                      # pre-event motion trail the operator asked for on 2026-08-06.
                      # NOTE: 2026-08-07 Phase.60 attempted to bump to 240
                      # to support offsets [60,90,120,150,180,210], but the operator
...
def set_reader(camera_name: str, reader: PersistentRTSPReader) -> None:
    ...
    Per Phase.167 §13.5 Commit 10: this function is the back-compat
...
# ---------------------------------------------------------------------------
# CameraSpec-aware registry state (Phase.167 §13.5 Commit 10, 2026-08-30)
# ---------------------------------------------------------------------------
#
...
def init_reader_registry(cameras: list[CameraSpec]) -> None:
    ...
    code-keyed storage too. Per Phase.167 §13.5 Commit 10.
...
def set_reader_by_code(code: str, reader: PersistentRTSPReader) -> None:
    ...
    Replaces any prior reader under the same code. Per Phase.167
    §13.5 Commit 10.
    """
    with _registry_lock:
        _readers[code] = reader
