"""Tests for infra/gate_cooldown.py — per-camera × per-event-type cooldown (Phase 6B.154, PLAN §11.77).
...
class TestResolveCooldownSeconds:
    ...
    def test_returns_exact_key_value(self, tmp_path):
        ...
        _write_config(tmp_path, {
            "CAM2": {"gate_cooldown": {"vehicle": 60, "person": 30}}
        })
        assert get_gate_cooldown_seconds("CAM2", "vehicle") == 60
        assert get_gate_cooldown_seconds("CAM2", "person") == 30
    ...
    def test_uses_per_camera_default_for_unknown_event(self, tmp_path):
        ...
        _write_config(tmp_path, {
            "CAM2": {"gate_cooldown": {"default": 45, "vehicle": 60}}
        })
        assert get_gate_cooldown_seconds("CAM2", "vehicle") == 60
        assert get_gate_cooldown_seconds("CAM2", "person") == 45  # default applies
        assert get_gate_cooldown_seconds("CAM2", "motion") == 45   # default applies

    def test_normalizes_people_to_person_key(self, tmp_path):
        """'people' (Reolink payload) reads the 'person' config key."""
        _write_config(tmp_path, {
            "CAM2": {"gate_cooldown": {"person": 30}}
        })
        assert get_gate_cooldown_seconds("CAM2", "people") == 30
        assert get_gate_cooldown_seconds("CAM2", "person") == 30

    def test_zero_value_means_no_cooldown(self, tmp_path):
        """Explicit 0 → no cooldown (don't suppress)."""
        _write_config(tmp_path, {
            "CAM2": {"gate_cooldown": {"vehicle": 0, "person": 30}}
        })
        assert get_gate_cooldown_seconds("CAM2", "vehicle") == 0
        assert get_gate_cooldown_seconds("CAM2", "person") == 30
...
class TestIsInGateCooldown:
    ...
    def test_first_call_returns_false_records_timestamp(self, tmp_path):
        ...
        _write_config(tmp_path, {"CAM2": {"gate_cooldown": {"vehicle": 60}}})
        in_cooldown, last_seen = is_in_gate_cooldown("CAM2", "vehicle")
    ...
    def test_second_call_within_window_returns_true(self, tmp_path):
        ...
        _write_config(tmp_path, {"CAM2": {"gate_cooldown": {"vehicle": 60}}})
        is_in_gate_cooldown("CAM2", "vehicle")  # first
        in_cooldown, last_seen = is_in_gate_cooldown("CAM2", "vehicle")  # second
    ...
    def test_third_call_after_window_returns_false(self, tmp_path, monkeypatch):
        ...
        _write_config(tmp_path, {"CAM2": {"gate_cooldown": {"vehicle": 1}}})
        is_in_gate_cooldown("CAM2", "vehicle")  # first
        time.sleep(1.1)  # window is 1s
        in_cooldown, _ = is_in_gate_cooldown("CAM2", "vehicle")  # after window
        assert in_cooldown is False

    def test_caller_window_overrides_config(self, tmp_path, monkeypatch):
        """Explicit window_seconds arg beats config."""
        _write_config(tmp_path, {"CAM2": {"gate_cooldown": {"vehicle": 9999}}})
        # Config says 9999s but caller says 1s
        is_in_gate_cooldown("CAM2", "vehicle", window_seconds=1)
        time.sleep(1.1)
        in_cooldown, _ = is_in_gate_cooldown("CAM2", "vehicle", window_seconds=1)
    ...
    def test_per_camera_independence(self, tmp_path):
        ...
        _write_config(tmp_path, {
            "CAM2": {"gate_cooldown": {"vehicle": 60}},
            "CAM5": {"gate_cooldown": {"vehicle": 60}},
        })
        is_in_gate_cooldown("CAM2", "vehicle")  # first alert CAM2
        in_cooldown_CAM5, _ = is_in_gate_cooldown("CAM5", "vehicle")  # CAM5 is fresh
        assert in_cooldown_CAM5 is False

    def test_per_event_type_independence(self, tmp_path):
        """Cooldown on (cam, vehicle) does not affect (cam, person)."""
        _write_config(tmp_path, {
            "CAM2": {"gate_cooldown": {"vehicle": 60, "person": 60}}
        })
        is_in_gate_cooldown("CAM2", "vehicle")  # first alert vehicle
        in_cooldown_person, _ = is_in_gate_cooldown("CAM2", "person")  # person is fresh
    ...
    def test_no_cooldown_when_window_zero(self, tmp_path):
        ...
        for _ in range(5):
            in_cooldown, _ = is_in_gate_cooldown("CAM2", "vehicle")
            assert in_cooldown is False
...
class TestClearAll:
    ...
    def test_clear_resets_in_memory_map(self, tmp_path):
        ...
        _write_config(tmp_path, {"CAM2": {"gate_cooldown": {"vehicle": 60}}})
        is_in_gate_cooldown("CAM2", "vehicle")
        clear_all_gate_cooldowns()
        in_cooldown, _ = is_in_gate_cooldown("CAM2", "vehicle")
    ...
    def test_clear_resets_config_cache(self, tmp_path):
        ...
        _write_config(tmp_path, {"CAM2": {"gate_cooldown": {"vehicle": 30}}})
        # Cached value still applies
        assert get_gate_cooldown_seconds("CAM2", "vehicle") == 60
        clear_all_gate_cooldowns()
        # Cache cleared, fresh read
        assert get_gate_cooldown_seconds("CAM2", "vehicle") == 30
...
class TestThreadSafety:
    ...
    def test_concurrent_calls_no_corruption(self, tmp_path):
        ...
        def worker():
            try:
                for _ in range(50):
                    res = is_in_gate_cooldown("CAM2", "vehicle")
                    results.append(res)
