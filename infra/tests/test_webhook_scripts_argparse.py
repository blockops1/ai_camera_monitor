...
scripts/apply_all_tuning.py — Phase.166 §11.87.6.
...
def _sample_cameras_dict() -> dict:
    """Synthetic operator-data-free fixture (Phase.167 §13.7).
...
def _sample_cameras_list():
    """Synthetic operator-data-free fixture (Phase.167 §13.5 Commit 9).
...
class TestConfigureWebhookArgparse:
    ...
    def test_webhook_url_default_when_nothing_set(self, monkeypatch):
        # Phase.167 §13.4: the hardcoded default URL was an operator PII leak.
    ...
    def test_no_hardcoded_homedir_path(self):
        """Isolation rule: no Path.home() / '<legacy-repo>' refs."""
        src = _load_script_source(CONFIGURE_WEBHOOK)
        assert "Path.home()" not in src
        assert "<legacy-repo>/" not in src
...
class TestSharedModulePurity:
    ...
    def test_no_prod_repo_references(self, script):
        ...
        assert "<legacy-repo>/" not in src or "absolute path" in src.lower()
...
# ============================================================================
# Phase.167 §13.4 — `--camera` + `--list-cameras` flags (verify + configure)
#
...
class TestPhase6B167VerifyWebhookCameraFlag:
    """Phase.167 §13.4 — verify_webhook.py --camera / --list-cameras."""
...
class TestPhase6B167ConfigureWebhookCameraFlag:
    """Phase.167 §13.4 — configure_webhook.py --camera / --list-cameras."""
