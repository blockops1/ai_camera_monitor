...
Covers:
    ...
    - _post_to_vision: routes DEFAULT_URL/None → configured URL via
      httpx (Phase.146; pool removed in 6B.147)
...
class TestConstants:
    ...
    def test_default_url_is_localhost_8093(self):
        # Phase.158 / §11.81 (2026-08-28): default URL now points at
    ...
    def test_timeout_is_generous(self):
        # Phase.61 bumped 60s → 150s for 6-frame combined prompts.
...
class TestPostToVisionRouting:
    """Phase.146: single httpx path. Pool removed in 6B.147.
    ...
    def test_default_url_uses_httpx_directly(self):
        ...
        # Phase.146: Authorization header is always sent (empty dict
...
class TestPostToVisionAuth:
    """Phase.146: Authorization: Bearer *** sent when token is set."""
