"""Unit tests for infra/<nas>_preview.py — <nas> NAS preview locator.

Phase.113b (Note 2026-08-25): regression test for the
...
from infra.<nas>_preview import (  # noqa: E402
...
def test_iterdir_with_retry_backoff_timing(tmp_path):
    ...
    with (
        ...
        mock.patch("infra.<nas>_preview.time.sleep") as mock_sleep,
    ):
        _iterdir_with_retry(tmp_path)
...
# ---------------------------------------------------------------------------
# find_preview() end-to-end against a fake <nas> directory tree
# ---------------------------------------------------------------------------


def test_find_preview_with_fake_<nas>_tree(tmp_path, monkeypatch):
    """End-to-end: build a fake @SSRECMETA tree, verify find_preview
    returns the closest file. Uses monkeypatch on NAS_ROOT to
    ...
    file.
    """
    import infra.<nas>_preview as sp

    # Build a minimal <nas> tree under tmp_path
    # Structure: NAS_ROOT/<Camera>/@SSRECMETA/Preview/<YYYYMMDD{AM,PM}>/<epoch_dir>/<ts>
    ...
    monkeypatch.setattr(sp, "NAS_ROOT", str(tmp_path))
