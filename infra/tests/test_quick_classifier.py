...
# ---------------------------------------------------------------------------
# Night suppression tests — Phase 6B.116 §11.47
# ---------------------------------------------------------------------------
...
def test_night_suppress_fires_when_timestamp_and_pilimage(monkeypatch):
    """Phase 6B.116 timestamp-fix: the night heuristic must work end-to-end when the
    caller passes a PIL.Image (the default Phase 6B.115 path) and a real timestamp.

    This is the regression test for the bug discovered on 2026-08-26: the heuristic
    was dormant because Phase 6B.115 changed crops from disk paths to PIL.Image,
...
def test_night_suppress_skipped_when_only_pilimage_no_timestamp(monkeypatch):
    """Phase 6B.116 timestamp-fix (negative case): without a timestamp AND without
...
def test_quickverdict_has_reason_field():
    """Phase 6B.116 reason plumbing — every QuickVerdict must have a reason
