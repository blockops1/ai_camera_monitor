"""Tests for infra.cameras — Phase 6B.167 §13.4.
...
# Phase 6B.167 §13.4 synthetic test fixture (NEW schema).
CAM1_IP=10.0.0.1
...
# Legacy schema test fixture (Phase 6B.167 §13.3 fallback path).
...
def mixed_env(tmp_path: Path) -> Path:
    """Both schemas present — NEW takes precedence per Phase 6B.167 §13.3."""
...
class TestParseLegacyFallback:
    ...
    def test_legacy_code_is_cam_n(self, legacy_schema_env: Path):
        """Legacy code is the CAM{N} code (Phase 6B.167 §13.4)."""
        specs = cameras.load_cameras(str(legacy_schema_env))
...
class TestModuleContract:
    """Phase 6B.167 §13.4 — cameras.py is the new way to enumerate cameras.
...
class TestRtspHostExtraction:
    """Phase 6B.167 §13.4 — _rtsp_host helper (Phase 6B.166 §11.87.2 carry-over).
...
class TestIpRotationWarning:
    """Phase 6B.167 §13.4 — IP-rotation warning surfaced from camera_creds.
    ...
    cameras. Phase 6B.166 §11.87.2 added this warning in infra.camera_creds;
...
# ---------------------------------------------------------------------------
# Phase 6B.167 §13.5 Commit 13: display_name_for / code_for
# ---------------------------------------------------------------------------
