...
def test_dimension_functions_count_23() -> None:
    """23 dimensions per the PRD (Phase.29b)."""
...
def test_normalize_color_silver_to_gray() -> None:
    """Phase.26a decision — silver is gray, not silver is metal."""
...
# Phase.84 (2026-08-16) — absence-evidence fallacy fix.
...
def test_dim_feature_match_both_false_does_not_match() -> None:
    """Phase.84: cab_marker_lights=False == False is absence-evidence,
...
def test_dim_feature_match_positive_match_still_works() -> None:
    """Phase.84: when values are genuinely present and equal, the
...
def test_default_dimension_weights_color_mismatch_dominant() -> None:
    """Phase.84 (2026-08-16) — Note: 'if a blue truck is
...
def test_normalize_bed_cover_aliases() -> None:
    ...
    # Phase.84 absence-evidence fix: 'none' (and 'null', 'false')
...
def test_score_vehicle_no_match_returns_zero() -> None:
    ...
    Phase.84 (2026-08-16) — color_mismatch bumped -2.0 → -4.0
    (Note: 'big penalty'). total is now -4.0 instead of -2.0.
    """
...
def test_score_vehicle_applies_negative_weight() -> None:
    ...
    Phase.84 (2026-08-16) — bumped from -2.0 to -4.0 per Note
