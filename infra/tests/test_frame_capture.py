...
Covers:
    ...
    - Re-exports: downscale_for_qwen, crop_face_region_from_4k,
      QWEN_INPUT_SIZE, INSIGHTFACE_CROP_SIZE, load_camera_creds,
...
class TestCaptureFromSnapshot:
    ...
    def test_copies_most_recent_jpegs(self):
        with tempfile.TemporaryDirectory() as snap_dir, tempfile.TemporaryDirectory() as output_dir:
            # Create 3 JPEGs with staggered mtimes.
            for i in range(3):
            ...
            for path in result:
                ...
                # Filename pattern: snapshot_001.jpg, snapshot_002.jpg, snapshot_003.jpg
    ...
    def test_count_limits_results(self):
        with tempfile.TemporaryDirectory() as snap_dir, tempfile.TemporaryDirectory() as output_dir:
            # Create 5 JPEGs, but only request 2.
            for i in range(5):
                p = os.path.join(snap_dir, f"frame_{i}.jpg")
                with open(p, "wb") as f:
    ...
    def test_non_jpeg_files_ignored(self):
        with tempfile.TemporaryDirectory() as snap_dir, tempfile.TemporaryDirectory() as output_dir:
            # Create a JPEG and a non-JPEG file.
            ...
            assert len(result) == 1
            # Output is renamed to snapshot_001.jpg (not the source basename).
            assert os.path.basename(result[0]) == "snapshot_001.jpg"
...
class TestCaptureFrames:
    ...
    def test_rtsp_failure_falls_back_to_snapshot(self):
        with tempfile.TemporaryDirectory() as output_dir, tempfile.TemporaryDirectory() as snap_dir:
            # Create a snapshot file so the snapshot path has something.
