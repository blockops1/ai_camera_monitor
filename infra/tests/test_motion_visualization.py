...
Phase.115 (§11.46.6): render_motion_composite now takes in-memory
...
def _make_frame_set_pil(tmpdir: str, with_motion: bool = True) -> list:
    ...
    Phase.115: returns PIL.Image directly (no on-disk intermediate).
...
def test_render_composite_returns_path(tmp_alert_dir):
    ...
    expected = os.path.join(tmp_alert_dir, "composite.jpg")
    assert result == expected
    assert os.path.isfile(expected)


def test_render_composite_default_output_path(tmp_alert_dir):
    """No output_filename → defaults to composite.jpg in output_dir."""
    frames = _make_frame_set_pil(tmp_alert_dir)
    bbox_a, bbox_b = _gate_bboxes_for_frame_set()
    result = render_motion_composite(
        frames=frames,
...
def test_render_composite_jpeg_quality_matches_motion_detector(tmp_alert_dir):
    """Composite JPEG quality must match motion_detector's output (90)."""
    frames = _make_frame_set_pil(tmp_alert_dir)
    bbox_a, bbox_b = _gate_bboxes_for_frame_set()
    ...
    # Read it back and confirm it's a valid JPEG.
...
def test_render_composite_bbox_format_is_xywh_not_xyxy(tmp_alert_dir):
    """Phase.115: bbox_a/bbox_b are (x, y, w, h), not (x1, y1, x2, y2).
