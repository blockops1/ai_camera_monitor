...
accompany the CAM1 lead motion Telegram, so maintainer can see at a glance which
...
Phase 6B.81 (PLAN.md §11.14): per-frame bbox annotation for the lead motion
...
def test_annotate_draws_green_bbox_on_frame(tmp_path):
    ...
    Phase 6B.86 (PLAN.md §11.18): annotate_frame_bboxes now draws
...
def test_annotate_handles_six_frames_with_bboxes(tmp_path):
    ...
    This is the realistic CAM1 case (Phase 6B.62+ sends a 6-frame trail).
    Each frame is in a different part of the 1280x960 grid so the bboxes
    are distinguishable across frames.

    Phase 6B.86 (PLAN.md §11.18): bbox_per_frame[i+1] is drawn on
