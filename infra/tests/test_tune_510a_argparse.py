...
class TestCliFlags:
    ...
    def test_ip_alone_parses(self):
        args = _build_parser().parse_args(["<camera-3-ip>"])
        assert args.ip == "<camera-3-ip>"

    def test_read_flag_parses(self):
        args = _build_parser().parse_args(["<camera-3-ip>", "--read"])
        assert args.read is True
        assert args.apply is False

    def test_apply_flag_parses(self):
        args = _build_parser().parse_args(["<camera-3-ip>", "--apply"])
    ...
    def test_motion_sensitivity_flag_parses(self):
        args = _build_parser().parse_args(
            ["<camera-3-ip>", "--apply", "--motion-sensitivity", "40"]
        )
    ...
    def test_negative_int_is_parsed_not_rejected(self):
        ...
        args = _build_parser().parse_args(
            ["<camera-3-ip>", "--apply", "--motion-sensitivity", "-1"]
        )
    ...
    def test_non_int_is_rejected(self):
        ...
        with pytest.raises(SystemExit):
            _build_parser().parse_args(
                ["<camera-3-ip>", "--apply", "--motion-sensitivity", "abc"]
            )
