...
def _build_parser():
    ...
    parser = argparse.ArgumentParser(
        description="Read or apply motion sensitivity recipe on Reolink REOLINK_MODEL cameras."
    )
    ...
    # Phase 6B.166 §11.87.4 — new CLI flags
...
class TestCliFlags:
    ...
    def test_ip_alone_parses(self):
        args = _build_parser().parse_args(["CAMERA_IP"])
        assert args.ip == "CAMERA_IP"

    def test_read_flag_parses(self):
        args = _build_parser().parse_args(["CAMERA_IP", "--read"])
        assert args.read is True
        assert args.apply is False

    def test_apply_flag_parses(self):
        args = _build_parser().parse_args(["CAMERA_IP", "--apply"])
    ...
    def test_motion_sensitivity_flag_parses(self):
        args = _build_parser().parse_args(
            ["CAMERA_IP", "--apply", "--motion-sensitivity", "40"]
        )
    ...
    def test_negative_int_is_parsed_not_rejected(self):
        ...
        args = _build_parser().parse_args(
            ["CAMERA_IP", "--apply", "--motion-sensitivity", "-1"]
        )
    ...
    def test_non_int_is_rejected(self):
        ...
        with pytest.raises(SystemExit):
            _build_parser().parse_args(
                ["CAMERA_IP", "--apply", "--motion-sensitivity", "abc"]
            )
...
# ===========================================================================
# Phase 6B.167 §13.5 (Commit 8) — --camera / --list-cameras tests
# ===========================================================================

SYNTHETIC_CAMERAS_ENV = """\
# Synthetic test env file for Phase 6B.167 Commit 8 (NEW schema)
CAM1_IP=10.10.1.21
