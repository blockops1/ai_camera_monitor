...
Phase.101 — pins for is_in_vision_block_cooldown (new), plus coverage
for the existing is_in_cooldown / is_in_bucket_cooldown / make_bucket_key
/ clear_all_cooldowns (no existing test file).

Per the test-serves-design rule (the operator 2026-08-19): pin behavior, not
...
def test_vision_block_default_is_30_minutes():
    """Default must match the operator's request: 30 min (1800 s)."""
...
def test_vision_block_global_across_alerts():
    """Phase.101 requirement: the rate-limit is GLOBAL, not per-alert.
...
# ---------------------------------------------------------------------------
# MotionCooldown (Phase.108) — extracted from listener.py
# ---------------------------------------------------------------------------
