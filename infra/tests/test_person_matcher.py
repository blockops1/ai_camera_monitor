...
Phase.106. Pure-function tests; no I/O.
...
class TestMatchByFaceRecognition:
    ...
    def test_unknown_face_falls_through_to_clothing(self):
        ...
        # known_persons=[name two (blue)] → match by blue
        vision_result = {
            "persons": [{"person_id": "p1", "clothing_upper": {"color": "blue"}}],
            "primary_person_index": 0,
        }
        ...
        known = [{"name": "name two", "clothing_upper_color": "blue", "role": "resident"}]
        result = match_person(vision_result, known, face_recognition)
        assert isinstance(result, MatchVerdict)
        assert result.matched_name == "name two"
    ...
    def test_none_face_recognition_skips_path(self):
        ...
        known = [{"name": "name one", "clothing_upper_color": "red", "role": "owner"}]
...
class TestMatchByClothing:
    ...
    def test_below_threshold_returns_no_match(self):
        # Phase.163 (Tier 3 stable-attribute match added) — this
    ...
    def test_unknown_color_returns_no_match(self):
        # Phase.163 (Tier 3 stable-attribute match added) — see
...
class TestFaceBeatsClothing:
    def test_face_match_takes_precedence(self):
        # Two different colors — face says "name one" (red shirt),
        # clothing would match name two (red shirt enrolled for name two),
        ...
        known = [
            {"name": "name two", "clothing_upper_color": "blue", "role": "resident"},
        ]
        result = match_person(vision_result, known, face_recognition)
        assert isinstance(result, MatchVerdict)
        assert result.matched_name == "name one"  # face wins
        assert result.matched_via == "face_recognition"
