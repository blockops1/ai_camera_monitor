...
class TestMatchByFaceRecognition:
    ...
    def test_unknown_face_falls_through_to_clothing(self):
        ...
        # known_persons=[employee_b (blue)] → match by blue
        vision_result = {
            "persons": [{"person_id": "p1", "clothing_upper": {"color": "blue"}}],
            "primary_person_index": 0,
        }
        ...
        known = [{"name": "employee_b", "clothing_upper_color": "blue", "role": "resident"}]
        result = match_person(vision_result, known, face_recognition)
        assert isinstance(result, MatchVerdict)
        assert result.matched_name == "employee_b"
    ...
    def test_none_face_recognition_skips_path(self):
        ...
        known = [{"name": "maintainer", "clothing_upper_color": "red", "role": "owner"}]
...
class TestFaceBeatsClothing:
    def test_face_match_takes_precedence(self):
        # Two different colors — face says "maintainer" (red shirt),
        # clothing would match employee_b (red shirt enrolled for employee_b),
        ...
        known = [
            {"name": "employee_b", "clothing_upper_color": "blue", "role": "resident"},
        ]
        result = match_person(vision_result, known, face_recognition)
        assert isinstance(result, MatchVerdict)
        assert result.matched_name == "maintainer"  # face wins
        assert result.matched_via == "face_recognition"
