"""Unit tests for infra.faces (Phase 6B.106 lift).

The lifted module mirrors ~/farm-surveillance/src/faces.py (archived
...
def test_slug_from_name__collapses_runs_of_punctuation():
    """'employee_b's Truck!!' → 'carson_s_truck' (one underscore per run)."""
    from infra.faces import _slug_from_name
    assert _slug_from_name("employee_b's Truck!!") == "carson_s_truck"
...
def test_slug_from_name__preserves_digits():
    from infra.faces import _slug_from_name
    assert _slug_from_name("employee_b 2") == "carson_2"
