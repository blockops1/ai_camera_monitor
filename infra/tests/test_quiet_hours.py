...
Phase 6B.112 (maintainer 2026-08-24): refactored from fixed 21:00-07:00 wall-clock
...
def test_quiet_hours_cameras_excludes_doors():
    ...
    Phase 6B.167 §13.4 Commit 17 (T3 C17): §13.4-migration refactor;
...
def test_front_door_outside_never_silenced_during_night():
    """CAM1 is not in the set — even at 02:00, we don't suppress."""
...
def test_ofs_silenced_at_midnight():
    """CAM1 at EDT 00:00 (deep night) → silenced."""
    dt = _edt(0)
...
def test_ofs_not_silenced_at_evening_before_sunset():
    """CAM1 at EDT 19:00 (1 hour before sunset) → NOT silenced."""
    dt = _edt(19)
    assert in_quiet_hours(dt, "Outside Front Solar") is False
...
def test_no_hardcoded_quiet_hours_window_constants():
    """Phase 6B.112: time logic moved to infra.time_of_day. Quiet hours module
