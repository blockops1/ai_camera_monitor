...
Phase 6B.167 §13.5 (Commit 12) — eligibility sets are now code-keyed
...
Test inventory:
  ...
  TestSubmitOverflowPolicy:
    - inside-tier job dropped when heap full (Phase 6B.61 unchanged)
...
def _patch_eligibility_sets(monkeypatch):
    ...
    Without this, the production sets (containing "OUTSIDE_FRONT_SOLAR" etc.)
...
# ---------------------------------------------------------------------------
# Phase 6B.167 §13.5 Commit 12: code-keyed eligibility sets
# ---------------------------------------------------------------------------
...
class TestPhase6B167ByCodeKeyedSets:
    ...
    Phase 6B.168 (2026-08-31): the deployment switched from the legacy
    env format (codes like OUTSIDE_FRONT_SOLAR) to the NEW cameras.env
    schema (CAM{N} codes per Phase 6B.167 §13.4). Production now uses
    CAM{N} codes; legacy codes like OUTSIDE_FRONT_SOLAR do not appear
    in the NEW schema and must NOT be in the eligibility sets.
    """
    ...
    def test_gatekeeper_set_is_code_keyed(self):
        ...
        # Phase 6B.168: NEW schema uses CAM{N} codes; the legacy
        # OUTSIDE_FRONT_SOLAR identifier no longer appears in cameras.env
        # so it must not appear in this set either.
        assert "OUTSIDE_FRONT_SOLAR" not in GATEKEEPER_CAMERAS
        # The CAM5 camera (Outside Front Solar in NEW schema) is the
        # canonical gatekeeper and must be present.
    ...
    def test_phase6a_set_is_code_keyed(self):
        ...
        # Phase 6B.168: NEW-schema CAM{N} codes must be present.
        for code in (
            "CAM1",
            "CAM3",
            "CAM4",
    ...
    def test_gatekeeper_and_phase6a_overlap(self):
        """Sanity: gatekeeper set and Phase 6A set overlap, but neither
        is a strict subset of the other — they serve different policy
        concerns (priority bump vs outside-eligibility).

        Phase 6B.168: GATEKEEPER_CAMERAS includes all CAM{N} (the
        listener treats every camera as a gatekeeper per §11.78);
        PHASE6A_ELIGIBLE_CAMERAS only includes outside-zone cameras
        (CAM3..6). Overlap is the outside gatekeepers.
        """
        overlap = GATEKEEPER_CAMERAS & PHASE6A_ELIGIBLE_CAMERAS
        # Every outside-zone camera is both a gatekeeper (priority 0)
        # and Phase 6A eligible (priority 1) — outside gatekeepers get
        # priority 0, inside cameras get priority 5.
        assert "CAM5" in overlap
        assert "CAM6" in overlap
        # Back Door Inside (CAM2) is a gatekeeper but NOT Phase 6A
        # eligible — it's the only interior camera. Front Door Outside
        # (CAM1) is technically outside-facing per cameras.env so it
        # appears in both sets.
        assert "CAM2" not in PHASE6A_ELIGIBLE_CAMERAS
...
# ---------------------------------------------------------------------------
# submit() — overflow policy unchanged (Phase 6B.61)
# ---------------------------------------------------------------------------
