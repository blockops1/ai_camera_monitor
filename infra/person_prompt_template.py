"""
person_prompt_template.py — Phase 6B.106 person-event prompt + schema.

STATUS: provisional (Phase 6B.106; will stabilize after live telemetry)
...
OUTPUTS:
  ...
  - PERSON_SCHEMA_JSON: a plain-string JSON literal embedded in the prompt
    ...
    facial_hair, glasses) added in Phase 6B.163 for tier-3 matching.
...
DOES NOT DO:
  ...
  - Does NOT detect motion or faces. (See infra.motion_types +
    infra.face_recognition. The motion gate owns frame-level detection
    since Phase 6B.115; infra.motion_detector is now a dataclass shim.)
  - Does NOT match persons to enrolled identities. (See
    infra.person_matcher — built in §11.36 step 6.)

CALLED BY:
  - infra.prompt_templates.select_prompt_template (Phase 6B.106) when
...
Design notes (Phase 6B.106, maintainer green-lit 2026-08-22):
  - Qwen3-VL is asked for the FULL attribute set per Qwen2.5-VL +
    ...
    face_bbox). maintainer: "fully use this vision model, do everything
  ...
  - Both frames sent simultaneously (single multi-image call, no
    intermediate crops). Per Phase 6B.100 multi-crop pattern.
"""
...
# ============================================================================
# Phase 6B.106 schema design (mirrors VEHICLE_CROP_PROMPT_TEMPLATE's "report
...
PERSON_PROMPT_TEMPLATE_FORMAT = (
    ...
    "Person-event analysis (Phase 6B.106).\n\n"
    ...
    "Stable identity attributes (Phase 6B.163):\n"
