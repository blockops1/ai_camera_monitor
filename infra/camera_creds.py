...
PUBLIC API:
    ...
    get_http_user(ip, env_path=None) -> str | None
        ...
        in the env file. Phase 6B.166 §11.87.2.
...
DOES NOT DO:
    ...
    - Re-implement env parsing — delegates to infra.cameras (Phase 6B.167 §13.4)

WHY HERE:
    Per AGENTS.md §4 (one module = one job). Originally extracted from
    frame_capture.py in Part 9 step 6. Phase 6B.167 §13.4 turned the
    ...
    Module-level camera_map was REMOVED in Phase 6B.167 §13.4 — the
...
CALLED BY:
    ...
    - scripts/cam_browser.py: get_http_user() / get_http_password() at
      CLI login (Phase 6B.166 §11.87.2)
...
RELATED:
    ...
    - infra.cameras: the actual parser (Phase 6B.167 §13.4 — Commit 5)
...
# Phase 6B.167 §13.4 — Commit 6: this module is now a thin wrapper.
...
def load_camera_creds(env_path: str) -> dict:
    ...
    for s in specs:
        entry: dict[str, str] = {}
        if s.rtsp_url:
            # Match pre-Phase 6B.167 behavior: only include the key when
