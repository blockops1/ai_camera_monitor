...
DOES NOT DO:
    ...
    - Decide motion — infra.motion_detector owns that (Phase.78).
...
PROMPT_TEMPLATE = (
    ...
    # the legacy prompt. Motion is owned by infra.motion_detector; the
...
# ===========================================================================
# Phase.78 (2026-08-14) — removed VEHICLE_MOTION_PROMPT_TEMPLATE.
# Motion is owned by infra.motion_detector (pairwise differential).
...
# infra.motion_detector. This template also asked Qwen for motion /
...
VISION_SCHEMA_JSON: dict[str, Any] = {
    ...
    "properties": {
        ...
        "vehicles": {
            ...
            "items": {
                ...
                "properties": {
                    ...
                    # vehicles[]. Motion is owned by infra.motion_detector
                    ...
                    "vehicle_features": {
                        ...
                        "properties": {
                            ...
                            # distinguishing features (the operator 2026-08-01 OOB
        ...
        # Motion is owned by infra.motion_detector; the differential
        # is the sole authority. Qwen describes vehicles; the
        # differential decides which ones are moving.
        # Phase.78 (2026-08-14) — removed vehicle_motion.
        # Same rationale: motion is owned by infra.motion_detector.
        "face_visibility": {
            "type": "object",
            "properties": {
...
def select_prompt_template(
    ...
    # it ("vision model is smarter than Yolo" — the operator 2026-08-29).
    ...
    removed. Motion is owned by infra.motion_detector. Qwen only
    describes what's in the frame.
      ...
      - mode="animal"   → ANIMAL_PROMPT_TEMPLATE_FORMAT (Phase.165,
        ...
        Per the operator 2026-08-29: "vision model is smarter than Yolo" —
    ...
    raise ValueError(
        ...
        f"removed because motion is owned by infra.motion_detector. "
...
def _build_messages(
    ...
    Frames are sent to Qwen at NATIVE resolution — no downscaling. the operator
