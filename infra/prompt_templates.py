...
DOES NOT DO:
    ...
    - Decide motion — listener.motion_gate_pipeline owns that
      (Phase 6B.115, refactored out of infra.motion_detector; the
...
WHY HERE:
    ...
    Phase 6B.78 (2026-08-14) — REMOVED VEHICLE_MOTION_PROMPT_TEMPLATE
...
CALLS INTO:
    - (Phase 6B.131) no downscale: frames are sent at native resolution
    - (no env vars — Phase 6B.78 removed FARMSURV_COMBINED_PROMPT)
...
PROMPT_TEMPLATE = (
    ...
    # Phase 6B.78 (2026-08-14) — REMOVED vehicle_motion instruction from
    # the legacy prompt. Motion is owned by the motion gate
    # (infra/frame_diff + listener/motion_gate_pipeline; Phase 6B.115); the
...
# ===========================================================================
# Vehicle-motion-aware multi-frame prompt (Phase 6B.13 + 6B.19 + 6B.48)
# ===========================================================================
# Phase 6B.78 (2026-08-14) — REMOVED VEHICLE_MOTION_PROMPT_TEMPLATE.
# Motion is owned by the motion gate (pairwise differential via
# infra/frame_diff + listener/motion_gate_pipeline; Phase 6B.115).
...
# ===========================================================================
# Crop-only ID prompt (Phase 6B.66)
# ===========================================================================
...
VEHICLE_CROP_PROMPT_TEMPLATE = (
    ...
    # Phase 6B.77 (2026-08-11) — free-text description field. Qwen
...
# ===========================================================================
# Phase 6B.78 (2026-08-14) — REMOVED VEHICLE_COMBINED_PROMPT_TEMPLATE.
# Same rationale as VEHICLE_MOTION_PROMPT_TEMPLATE: motion is owned by
# Motion is owned by the motion gate (infra/frame_diff +
# listener/motion_gate_pipeline; Phase 6B.115). infra.motion_detector
...
# ===========================================================================
# Focused crop classifier prompt (Phase 6B.6)
# ===========================================================================
...
# Phase 6B.74 (2026-08-10) — VEHICLE_CROP_SCHEMA_JSON.
    ...
    "properties": {
        "vehicles": {
            ...
            "description": (
                ...
                "adjacent vehicle too \u2014 see Phase 6B.129b \u00a711.52."
            ),
            "items": {
                "type": "object",
                "properties": {
        ...
        # Phase 6B.129b \u00a711.52 \u2014 these are no longer emitted by Qwen;
...
VISION_SCHEMA_JSON: dict[str, Any] = {
    ...
    "properties": {
        ...
        # Phase 6B.6 — per-vehicle detection list with bbox. The first
        ...
        # parked behind employee_a's, etc.). We capture ALL of them so the
        ...
        "vehicles": {
            ...
            "items": {
                ...
                "properties": {
                    ...
                    # Phase 6B.19 (2026-07-27) — per-vehicle motion.
                    ...
                    # Phase 6B.78 (2026-08-14) — REMOVED motion /
                    ...
                    # Phase 6B.115). infra.motion_detector is a dataclass
                    ...
                    # Phase 6B.18 — discriminating per-vehicle features.
                    ...
                    "vehicle_features": {
                        ...
                        "properties": {
                            ...
                            "badge_text_readable": {"type": ["string", "null"]},
                            # Phase 6B.48 — bigger, more permanent
                            # distinguishing features (maintainer 2026-08-01 OOB
                            ...
                            "cab_marker_lights": {
                                # Phase 6B.66 — accept string OR boolean.
                                ...
                                # verified 2026-08-08 against the 09:26 CAM1
        ...
        "primary_vehicle_index": {
            "type": "integer",
            "minimum": 0,
            "description": "DEPRECATED 2026-07-27 (Phase 6B.19). Kept for backward compat with old consumers. Defaults to 0. Use vehicles[0] directly — motion is no longer a per-vehicle concept (Phase 6B.78).",
        },
        # Phase 6B.27 (2026-07-30) — best frame for the moving vehicle.
...
def select_prompt_template(
    ...
    # Phase 6B.66 — accept Optional[str] so analyze_frames can pass
    ...
    mode: str | None = "auto",
    # Phase 6B.165 §11.86.3 (wider-scope revision 2026-08-29) —
    ...
    # it ("vision model is smarter than Yolo" — maintainer 2026-08-29).
    ...
    Phase 6B.78 (2026-08-14): simplified vehicle dispatch. The motion
    ...
    describes what's in the frame.
      ...
      - mode="crop"     → VEHICLE_CROP_PROMPT_TEMPLATE (single tight
        ...
        no scene narrative — Phase 6B.66, 2026-08-08). Used by
      ...
      - mode="person"   → PERSON_PROMPT_TEMPLATE_FORMAT (Phase 6B.106,
        ...
        gatekeeper events.
      - mode="animal"   → ANIMAL_PROMPT_TEMPLATE_FORMAT (Phase 6B.165,
        ...
        Per maintainer 2026-08-29: "vision model is smarter than Yolo" —
    ...
    captured_at (Phase 6B.20): the wall-clock ISO timestamp of the
    ...
    """
    # Phase 6B.74 (2026-08-10) — crop mode short-circuit BEFORE the
    ...
    # root cause of employee_b's Silverado being misidentified as maintainer's F-350
    ...
    # Phase 6B.106 (2026-08-22) — person mode short-circuit. Same shape
    ...
    # Phase 6B.165 (2026-08-29) — animal mode short-circuit. Same shape
    ...
    # Phase 6B.78 (2026-08-14): VEHICLE_MOTION_PROMPT_TEMPLATE and
    ...
    if mode == "crop":
        # Phase 6B.66 — single tight crop, identification-only.
    ...
    raise ValueError(
        ...
        f"(Phase 6B.78). The VEHICLE_{mode.upper()}_PROMPT_TEMPLATE was "
        f"removed because motion is owned by the motion gate "
        f"(infra/frame_diff + listener/motion_gate_pipeline; Phase 6B.115). "
...
def _build_messages(
    ...
    Frames are sent to Qwen at NATIVE resolution — no downscaling. maintainer
    ...
    crop-only (Phase 6B.100), so the only path hitting this function is
    ...
    captured_at (Phase 6B.20): forwarded to the prompt template as a
    ...
    for path in frame_paths:
        try:
            # Read native-resolution JPEG bytes directly. No resize.
