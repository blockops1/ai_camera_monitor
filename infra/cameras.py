"""
cameras.py — Camera identity + IP/credential lookup (Phase 6B.167 §13.4).
...
INPUTS:
    - file $FARMSURV_CAMERAS_ENV or infra.paths.CAMERAS_ENV_FILE (NEW
      schema, Phase 6B.167 §13.2) — gitignored, identity-only
...
PUBLIC API:
    ...
    display_name_for(identifier: str, env_path: str | None = None) -> str
        ...
        input unchanged when no spec matches. Phase 6B.167 §13.5
    ...
    load_camera_aliases(env_path: str | None = None) -> tuple[dict[str, str], dict[str, str]]
        ...
        shorthand → CameraSpec.code. Phase 6B.167 §13.4 Commit 18
    ...
    has_rtsp(camera_name: str, env_path: str | None = None) -> bool
        §11.115.1 RTSP-presence filter. True iff the camera is in the
        registry AND has a non-empty `rtsp://...` rtsp_url. False for
        unknown cameras, missing rtsp_url, or malformed scheme. Used by
        the listener to drop webhooks from cameras without RTSP.
...
WHY HERE:
    Phase 6B.167 §13.4. The fleet has grown from 2 to 6 cameras and is
...
CALLED BY:
    - listener.listener: load_cameras() at bootstrap (Phase 6B.167 T6)
    - scripts/cam_browser.py: by_ip(ip) at login (Phase 6B.167 T3)
    - scripts/configure_webhook.py: by_code() / all_codes() (Phase 6B.167 T1)
    - scripts/verify_webhook.py: same (Phase 6B.167 T1)
    - scripts/read_alarm_settings.py: same (Phase 6B.167 T1)
...
class CameraSpec:
    ...
    Phase 6B.167 §13.4. Replaces the operator-specific FRONT/BACK/OUTSIDE_*
...
def _default_env_path() -> str:
    """Resolution order for env_path when caller passed None:
      1. $FARMSURV_CAMERAS_ENV (Phase 6B.167 §13.4 — operator can pin a
...
def _warn_if_ip_rotated(spec: CameraSpec) -> None:
    ...
    LEGACY-schema parses. Phase 6B.166 §11.87.2 hardening.
    """
...
# ---------------------------------------------------------------------------
# NEW-schema parser (CAM1_IP, CAM1_NAME, ...) — Phase 6B.167 §13.2
# ---------------------------------------------------------------------------
...
#
# Phase 6B.167 §13.3: this is the FALLBACK. The operator's existing
...
#
# Phase 6B.167 §13.6 (public-repo impact): when this module ships in
...
# Phase 6B.167 §13.4 (T3 Commit 17): legacy prefixes now resolve to
...
_LEGACY_PREFIX_TO_CODE: dict[str, str] = {
    ...
    "FRONT":                  "CAM1",
    "BACK":                   "CAM2",
    "OUTSIDE_FRONT_GARAGE":   "CAM3",
    "OUTSIDE_FRONT_POWER":    "CAM4",
    "OUTSIDE_FRONT_SOLAR":    "CAM5",
    "OUTSIDE_BACK_SOLAR":     "CAM6",
    # --- Synthetic test prefixes (public-repo tests use these) ---
    "TEST_FRONT":          "CAM1",
    "TEST_BACK":           "CAM2",
    "TEST_SIDE":           "CAM3",
...
def _parse_legacy_fallback(kv: dict[str, str]) -> list[CameraSpec]:
    ...
    IP-resolution priority per camera:
      ...
      2. IP extracted from {PREFIX}_RTSP_URL host (Phase 6B.166 §11.87.0
    ...
    for prefix, code in _LEGACY_PREFIX_TO_CODE.items():
        ...
        if not ip:
            # Fallback: extract from RTSP URL host (Phase 6B.166 §11.87.0)
            ...
            ip = extracted
        specs.append(CameraSpec(
            code=code,                                # §13.4: CAM{N} code (Phase 6B.167 §13.4)
...
def load_cameras(env_path: str | None = None) -> list[CameraSpec]:
    ...
    Side effects:
        ...
        disagree (Reolink DHCP-rotation drift signal — Phase 6B.166
...
def code_for(identifier: str, env_path: str | None = None) -> str:
    ...
    Phase 6B.167 §13.5 (Commit 13): inverse of display_name_for.
...
def display_name_for(identifier: str, env_path: str | None = None) -> str:
    ...
    Phase 6B.167 §13.5 (Commit 13): decouple Telegram bodies (and other
...
# ---------------------------------------------------------------------------
# §11.115.1 — RTSP-presence filter
# ---------------------------------------------------------------------------
#
# maintainer directive 2026-09-02 PM: "we should ignore any web hook from a camera
# that does not have an RTSP stream running. So by definition any camera
# that has an RTSP stream set up is a gatekeeper and everything else gets
# ignored."
#
# The listener calls has_rtsp() for every incoming alert and drops the
# alert if it returns False. There is no per-camera opt-in: the *only*
# criterion is "does this camera have an rtsp:// URL configured in the
# registry." If you don't have an RTSP stream, you don't get processed.


def _find_spec(camera_name: str, env_path: str | None = None) -> CameraSpec | None:
    """Look up a camera by friendly name OR by CAM{N} code.

    Returns the matching CameraSpec, or None if no match. Exhaustive:
    tries both name and code match for every loaded spec. This is
    different from `by_code` (raises KeyError) and `display_name_for`
    (returns input unchanged on miss). `has_rtsp` needs a clean
    found/not-found signal.
    """
    for spec in load_cameras(env_path):
        if spec.code == camera_name or spec.name == camera_name:
            return spec
    return None


def has_rtsp(camera_name: str, env_path: str | None = None) -> bool:
    """Return True iff the named camera has a non-empty rtsp:// URL.

    Used by the listener as the RTSP-presence filter (§11.115.1). Any
    camera that returns False here is silently dropped at webhook intake;
    no vision call, no Telegram send.

    Resolution: by friendly display name (the name as it appears in webhook
    payloads and Telegram) OR by CAM{N} code. Either form is accepted;
    the function tries both.

    Args:
        camera_name: friendly name from the alert payload, or CAM{N}
            code as a fallback.
        env_path: absolute path to the env file. If None, uses the
            default resolution chain (`_default_env_path`).

    Returns:
        True  — camera is in the registry AND has rtsp_url set AND
                rtsp_url starts with "rtsp://".
        False — camera unknown, rtsp_url missing, or rtsp_url malformed.

    Note: this function is intentionally permissive on the URL prefix
    (any "rtsp://..." string passes). It does NOT verify the stream is
    actually live — that's `infra.persistent_rtsp`'s job. has_rtsp is a
    registry-level check: "is this camera supposed to have an RTSP feed?"
    """
    spec = _find_spec(camera_name, env_path=env_path)
    if spec is None:
        return False
    return bool(spec.rtsp_url) and spec.rtsp_url.startswith("rtsp://")


# Phase 6B.167 §13.4 (C18, 2026-08-31): operator-private chat
# shorthand. The operator types "OFS" in chat; we resolve it to
...
def _parse_aliases_from_kv(
    ...
    on the shorthand itself (operator types `OFS`, not `ofs`).
    """
    import re
...
def load_camera_aliases(
    ...
    Phase 6B.167 §13.4 (C18, 2026-08-31). The operator populates
    CAM{N}_SNAPSHOT_ALIAS and CAM{N}_PREVIEW_ALIAS in cameras.env
    to make `/snapshot?camera=OFS` and `/preview?camera=OFS` work
