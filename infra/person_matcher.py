...
STATUS: provisional (Phase.163; Tier 3 stable-attribute matching added;
...
WHY HERE:
    ...
    (per Phase.106 §11.36 step 6). Designed for forward-compat: §11.36b
...
# Phase.106 — clothing color match threshold (cosine similarity).
...
# Phase.163 — Tier 3 stable-attribute weighted ensemble. Priority
# order per the operator 2026-08-29: silhouette > hair > skin_tone > facial_hair >
...
# Phase.163 — valid enum values per stable attribute are documented
...
class MatchVerdict:
    ...
    # Phase.163 — optional per-attribute breakdown when matched_via
...
class NoMatch:
    ...
    suppress: bool = False  # True when verified false positive — suppresses Telegram (Phase.162)
...
# ---------------------------------------------------------------------------
# Phase.163 — Tier 3 stable-attribute matching (weighted ensemble).
#
...
def _match_by_stable_attributes(
    ...
    """Phase.163 — Tier 3 weighted-ensemble stable-attribute match.
...
def match_person(
    ...
    # Path 2: clothing color match. Per the operator 2026-08-22, when face is
    ...
    # Path 3 (Phase.163): weighted-ensemble stable-attribute match.
