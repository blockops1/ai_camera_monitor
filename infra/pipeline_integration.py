...
OUTPUTS:
    ...
    - side effect: Phase 6A modules are loaded from ~/person-tracker/
...
WHY HERE:
    Phase 6A lives in the separate ~/person-tracker/ repo per the
...
CALLS INTO:
    - ~/person-tracker/infra/face_recognition (lazy)
    - ~/person-tracker/infra/property_state (lazy)
    - ~/person-tracker/infra/response_engine (lazy)
    - sys.path.insert: scope-limited to the function call

RELATED:
    - ~/person-tracker/ — the separate repo that owns Phase 6A
...
# live in the person-tracker project, not here. They're imported lazily inside
...
def _load_phase6a_modules():
    ...
    try:
        ...
        log.warning(f"Phase 6A modules not available (person-tracker project not installed?): {err}")
        return None
...
def run_phase6a_recognition(
    ...
    try:
        ...
        # Phase 6A modules live in the person-tracker project. Load them
        # lazily so a refactor-only deployment (no person-tracker) doesn't
        ...
        #
        # Phase 6B.167 §13.5 Commit 12: PHASE6A_ELIGIBLE_CAMERAS now
        ...
        for attempt, (idx, bbox_small) in enumerate(candidates, start=1):
            ...
            # Phase 6B.131 (maintainer 2026-08-26): Qwen now receives the frame at
...
def _run_insightface_on_frame(
    ...
    try:
        # face_recognition is lazy-loaded from ~/person-tracker/ at runtime;
    ...
    # property_state is lazy-loaded from ~/person-tracker/ at runtime;
    ...
    # response_engine is lazy-loaded from ~/person-tracker/ at runtime.
