...
WHY HERE:
    ...
    algorithm with pairwise-diff. Note's observation: "you would just
...
CROP_PAD_PCT = 0.0             # Phase.82: no padding — Qwen sees the
                              ...
                              # the visualization). Note 2026-08-16: padding
...
# tuned for the Garage camera where vehicles move ~130 px per 2s interval,
# but the Solar camera at 192.168.1.103 sees faster cross-frame motion
# (Tesla drive-by 15:08 EDT: 322 px jump between frame 2→3 because pairwise
# diff produces a trailing-edge bbox whose center is the gap left behind,
# not the vehicle's current position). With 300 px, the tracker lost the
# candidate at frame 3 and reported frames_seen=2 (failed MIN_FRAMES_SEEN=3).
# 600 covers the realistic upper bound for fast-moving vehicles on either
# camera without admitting noise (paired bboxes have to actually be near
# the predicted position; far-away blobs still get rejected).
...
# 320x240 each) per Note's request 2026-08-10. Vertical separation
# matters for his OFS camera (gate zone vs parking zone vs far-back).
...
def detect_motion(
    ...
    #    objects we'd need Hungarian assignment, but for OFS (single dominant
