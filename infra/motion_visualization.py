...
INPUTS:
    ...
    - output_dir: directory to write composite.jpg into (defaults to
      the alert's output_dir passed via ctx).
    - output_filename: filename for the composite (default "composite.jpg").
...
OUTPUTS:
    - Writes one JPEG to disk at <output_dir>/<output_filename> at quality=90.
    - Returns the absolute path to the rendered JPEG, or '' on failure.
...
PUBLIC API:
    def render_motion_composite(
        ...
        output_filename: str = "composite.jpg",
        diff_threshold: int = 40,
        min_blob_area: int = 500,
    ) -> str:
        # Returns the path to the rendered JPEG, or '' on any failure.
...
WHY THIS MODULE EXISTS (Phase.79, Note 2026-08-14)
  ...
  reference image he showed was OFS alert — rendered as the
  cumulative pairwise differential union painted in red, on the
...
def _median_background_from_frames(bgr_frames: list[np.ndarray]) -> np.ndarray | None:
    ...
    native resolution (e.g., 2304x1296 for OFS), and the per-pixel
    median is computed directly. A moving object occupies <half the
...
def render_motion_composite(
    ...
    Returns the absolute path to the rendered JPEG on success.
    """
    ...
    # Write JPEG.
    if not cv2.imwrite(output_path, out, [cv2.IMWRITE_JPEG_QUALITY, JPEG_QUALITY]):
        return ""
