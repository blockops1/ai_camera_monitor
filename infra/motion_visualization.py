...
Phase 6B.115 (§11.46.6, 2026-08-25): the function now takes in-memory
...
INPUTS:
    - frames: list of 4 PIL.Image.Image at NATIVE resolution.
      Phase 6B.115: previously frame_paths (4 disk paths from the gate).
...
CALLED BY:
    - listener.vehicle_event_pipeline._send_vehicle_messages (Phase 6B.115)
    - telegram_formatter.composite_telegram.send_composite_alert (Phase 6B.115)
...
WHY THIS MODULE EXISTS (Phase 6B.79, maintainer OOB 2026-08-14)
  maintainer asked for "one image that shows the differences between the
  ...
  Phase 6B.115 (2026-08-25, maintainer OOB):
...
# Phase 6B.115: number of frames the motion gate produces per vehicle alert.
N_FRAMES_EXPECTED = 4
...
# (Phase 6B.115 — was RESIZE_W/RESIZE_H coords in the legacy flow).
...
def _project_bbox(
    ...
    """Phase 6B.115: legacy projection helper kept as identity (native-res bboxes).
...
def _native_bbox_to_corners(
    ...
    Phase 6B.115: the gate's diff bboxes are already in native coords,
...
def _median_background_from_frames(bgr_frames: list[np.ndarray]) -> np.ndarray | None:
    ...
    Phase 6B.115: no resize. The 4 gate frames are loaded at their
...
def _cumulative_diff_mask_from_frames(
    ...
    Phase 6B.115: for i in 1..(N-1): absdiff(frame[i], frame[i-1])
