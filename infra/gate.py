...
STATUS: provisional (Phase 6B.107 §11.37, 2026-08-23; Phase 6B.109 §11.39 v2 fixes 2026-08-24;
        Phase 6B.137 §11.59 catchall-when-no-vehicle fix 2026-08-27;
...
DOES NOT DO:
  ...
  - Does NOT tighten streak crops via YOLO (Phase 6B.144 §11.66). The
...
Architecture (LOCKED §11.37, modified §11.66 + §11.93 Phase 6B.169):
...
the bbox during the 1-2s inter-frame interval (Operator: "about half
...
# ---------------------------------------------------------------------------
# Phase 6B.109 §11.39 — V2 opt-in via env var
# ---------------------------------------------------------------------------
#
...
class GateVerdict:
    ...
    Phase 6B.115 (§11.46.6): the gate's output now includes in-memory
...
# Phase 6B.152 — per-camera × per-event-type gate configuration. See
...
def is_gate_enabled(camera_name: str, event_type: str) -> bool:
    """Phase 6B.152 — is the gate enabled for (camera, event_type)?
    ...
    JSON config keys are camera env-var prefixes (whatever you named
    them in camera-creds.env — e.g. CAMERA_A, CAMERA_B, ...). The
    pipeline passes camera_id directly; no translation layer required.
    """
...
def load_thresholds(camera_name: str) -> dict[str, float]:
    ...
    Configuration keys are camera IDs (whatever you named them in
    camera-creds.env — e.g. CAMERA_A, CAMERA_B, ...), not abstract
    CAM{N} codes. The pipeline passes camera_id directly as
    camera_name; no translation layer required.
    """
...
def _classify_crop(
    ...
    Phase 6B.115: crop may be a disk path (legacy, GATE_KEEP_DISK_ARTIFACTS=true)
    OR an in-memory PIL.Image (new, default). Both routes go through
    classifier.classify_frame() which handles the union.

    Phase 6B.116 (timestamp-fix): `timestamp` is forwarded to classify_frame()
    ...
    # IMPORTANT (Phase 6B.116 timestamp-fix): only re-stamp if the classifier
    ...
    # person threshold (e.g. 0.30 for a noisy perimeter camera)
    # is permissive.
...
def _route_decision(
    ...
    if not high_conf:
        ...
        # Phase 6B.116 — prefer the most informative reason from the per-crop
...
def run(
    ...
    Args:
      ...
      timestamp: optional webhook event timestamp (Phase 6B.116 timestamp-fix).
