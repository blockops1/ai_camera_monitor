...
THREAD SAFETY: thread-safe for read-only spec interpretation (spec
    ...
    in production after Phase 6B.29d cutover)
...
KNOWN VIOLATIONS (see PLAN.md Part 9 §5):
    This module still owns the Phase 6B.26a serial-gate interpreter
...
def score_top_n(
    ...
    """Phase 6B.77 (2026-08-11) — score every kv and return the top N.
...
def _match_with_spec(spec: dict[str, Any], sig: Sig, known: list[KnownVehicle]
    ...
    """DEAD CODE — Phase 6B.26a serial-gate spec interpreter.

    The production matcher is match_vehicle_scored (Phase 6B.29b/6B.29d).
...
# ---------------------------------------------------------------------------
# Shadow-mode dead code — Phase 6B.26a side-by-side validation harness
# ---------------------------------------------------------------------------

# Phase 6B.29b — scored-combination matcher shadow counters.
...
def compare_with_legacy(spec: dict[str, Any], sig: Sig, known: list[KnownVehicle],
    ...
    # vehicle_state.py deletion in Phase 6B.57.)
