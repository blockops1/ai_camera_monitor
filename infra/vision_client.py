...
PUBLIC API:
    DEFAULT_URL
        ...
        (Phase.158 / §11.81 — unified Qwen3.6-35B-A3B server;
         was 8080 with Qwen3-VL-8B before the swap).
    TIMEOUT = 150
        Seconds per frame batch. Bumped 60s → 150s in Phase.61
...
# 127.0.0.1:8093 with no auth — Phase.158 / §11.81 unified
# Qwen3.6-35B-A3B server (was 8080 with Qwen3-VL-8B pre-§11.81).
DEFAULT_URL = load_vision_config().url
# Phase.61 (2026-08-07): bumped 60s → 150s. Multi-frame (6 frames)
