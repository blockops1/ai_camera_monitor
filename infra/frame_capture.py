...
OUTPUTS:
    - return value: list[str] — paths to captured PNG files
    - writes files: <output_dir>/frame_001.png, frame_002.png, ... (§11.88 2026-09-01)
    - writes files: <output_dir>/snapshot_001.png, ... (fallback path; §11.88)
...
PUBLIC API:
    ...
    capture_frames(rtsp_url: str, output_dir: str, count: int = 3,
        ...
        Capture `count` PNG frames (§11.88 2026-09-01). Order of preference:
          ...
          2. On-demand RTSP via PyAV — opens a fresh connection, demuxes
             ...
             Used for non-CAM1 cameras (no persistent reader registered).
          3. Camera push snapshot directory (Synology @PushServ).
...
KNOWN VIOLATIONS (see PLAN.md Part 11.8):
    - When a PersistentRTSPReader is in play for the URL (today: only
      CAM1 / gatekeeper camera), capture_frames MUST read from it. The
      ...
      dropped, not degraded. This is the explicit contract maintainer
    ...
    - Trade-off: during the ~5s warmup window after listener boot, the
      ...
      The first CAM1 event in that window will be dropped rather than
      producing a degraded capture.

DOES NOT DO:
    - Downscale frames for Qwen → REMOVED §11.88 (was infra.image_prep.downscale_for_qwen)
...
WHY HERE:
    ...
    §11.88 (2026-09-01) — frame_capture now writes PNG lossless (was
    JPEG). The frame format aligns with persistent_rtsp's ring buffer
    format (also PNG), so the disk output of get_recent_frames and
    the output of _capture_from_rtsp are byte-compatible.
...
CALLS INTO:
    - infra.persistent_rtsp: get_reader_for_url() resolves the right
      registered reader for an inbound alert (Phase 6B.87, PLAN §11.17)
    - av (PyAV): RTSP demux + decode
    - os, glob, shutil (stdlib): snapshot fallback
    - logging (stdlib): log per-frame captures + fallbacks

RELATED:
    - infra.image_prep: post-capture image processing (crop; downscale REMOVED §11.88)
...
# full 4K frame).
#
# §11.88 (2026-09-01): Qwen3-VL is now given the NATIVE-RESOLUTION frames
# (see infra.image_prep.NATIVE_RES = (2304, 1296)). The downscale step in
# image_prep.downscale_for_qwen is removed. Qwen tokens per image went
# from ~950 (at 720p) to ~4100 (at native) — fits within --ctx-size 16384
# under --parallel 4.
DEFAULT_MAX_SIZE: tuple[int, int] = (3840, 2160)
...
def capture_frames(
    ...
    """
    Capture `count` PNG frames (§11.88 2026-09-01) from an RTSP stream.

    Args:
        rtsp_url: Full RTSP URL with credentials.
        output_dir: Directory to write frame_001.png, frame_002.png, etc.
        ...
        offset: File numbering starts at `offset + 1`. Used by Phase 6B.9
    ...
    # registered for this URL (today: only CAM1), the capture MUST
    ...
    if reader is None:
        ...
        # (Phase 6B.87, PLAN §11.17). Strips creds before matching so
        ...
        # (CAM1, CAM2, ...), this is the path that picks the correct one.
    ...
    if isinstance(reader, PersistentRTSPReader):
        ...
        if frame_offsets is not None:
            # Pre-event motion trail path (gatekeeper / CAM1).
    ...
    # on-demand RTSP capture (canonical path for non-CAM1 cameras).
...
def _capture_from_rtsp(
    ...
    Why the FFmpeg `select` pattern works (and `time.sleep` does not):
      ...
      than the previous one. maintainer observed this 2026-08-03: 6 frames
    ...
    try:
        ...
        container = av.open(
            ...
            options={
                ...
                # Reference: CAM4 forum fix for "Reolink RTSP / no live-streaming,
        ...
        for packet in container.demux(stream):
            ...
            try:
                ...
                frame_path = os.path.join(
                    output_dir, f"frame_{frame_number:03d}.png"
                )
                # §11.88 (2026-09-01) — PNG lossless, NOT JPEG. PyAV infers
                # format from the .png extension. Lossless preserves face
                # detail for InsightFace and removes the round-trip
                # encode (raw -> jpeg q85 -> decode).
...
def _capture_from_snapshot(
    ...
    """
    Copy the most recent snapshot from the Synology push snapshot directory.
    ...
    for i, src in enumerate(files[:count]):
        # §11.88 (2026-09-01) — re-encode to PNG lossless via PIL so the
        # bytes are honest about the file extension. Without this, the
        # snapshot fallback would silently keep JPEG bytes under a .png
        # name (which the §11.88 lock-in was meant to fix).
        dst = os.path.join(output_dir, f"snapshot_{i + 1:03d}.png")
        try:
            ...
            log.debug(f"snapshot re-encode failed for {src}: {_err}")
            continue
...
#
# §11.88: QWEN_INPUT_SIZE was removed (replaced by NATIVE_RES in image_prep).
# Existing imports like `from infra.frame_capture import QWEN_INPUT_SIZE`
# will fail loudly — that's intentional; callers should switch to NATIVE_RES
# or stop needing the constant entirely (downscale removed).
# ---------------------------------------------------------------------------
