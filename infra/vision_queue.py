...
WHY HERE:
    ...
    Overflow policy (Phase 6B.61): if heap size >= MAX_QUEUE_DEPTH
    when an INSIDE job arrives, the job is dropped with
    QueueOverflowError. Gatekeeper + outside jobs NEVER drop.

    Code-keyed eligibility sets (Phase 6B.167 §13.5 Commit 12, 2026-08-30;
    updated Phase 6B.168 2026-08-31 to CAM{N} identifiers per the NEW
    cameras.env schema):
...
CALLS INTO:
    ...
    - infra.cameras (Phase 6B.167 Commit 12): load_cameras() inside
...
HISTORY:
    2026-08-30 — Phase 6B.167 §13.5 Commit 12: replaced name-keyed
    ...
    2026-08-31 — Phase 6B.168: switched the literal code values from
...
class QueueOverflowError(Exception):
    ...
    Phase 6B.61 (2026-08-07): inside-tier jobs are dropped on overflow rather
...
PRIORITY_INSIDE = 5      # everything else (descriptive only)
# Phase 6B.61 — MAX_QUEUE_DEPTH for overflow drop policy. When the heap
...
# Phase 6B.168 (2026-08-31): under the NEW cameras.env schema (Phase
...
# listener/vehicle_event_pipeline.py:652 (Phase 6B.168).
#
...
# dynamically — tracked as Phase 6B.169 follow-up.
...
# Phase 6B.24 Fix C — heartbeat was originally planned as a separate
...
# Phase 6B.168 (2026-08-31): same NEW-schema migration as
...
def code_for(camera: str) -> str:
    ...
    Phase 6B.167 §13.5 Commit 12: callers historically passed friendly
...
class VisionQueue:
    ...
    def submit(
        ...
        **kwargs: Any,
        ...
        Phase 6B.167 §13.5 Commit 12: `camera` may be either a
        ...
        with self._cv:
            # Phase 6B.61: drop low-priority (inside) jobs on overflow rather
