...
WHY HERE:
    ...
    GATEKEEPER_CAMERAS and PHASE6A_ELIGIBLE_CAMERAS now contain
    CameraSpec.code values (e.g. operator-assigned identifiers like
    "CAM1"), not friendly names. The translation from
    a caller-passed name to the canonical code happens via code_for()
    inside submit(). This removes operator-flavored camera names
    from the queue source so the file is operator-agnostic.
...
HISTORY:
    2026-08-30 — Phase.167 §13.5 Commit 12: replaced name-keyed
        ...
        to codes via infra.cameras.load_cameras(). Operator camera
        names no longer appear in this source file.
"""
...
# any code in this set routes it ahead of all outside-tier work.
#
