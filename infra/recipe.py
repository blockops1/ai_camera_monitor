...
CALLED BY:
    - scripts/tune_510a_motion_sensitivity.py: resolve_for_camera()
      at --apply / --dry-run (Phase 6B.166 §11.87.4 - planned)
    - scripts/apply_all_tuning.py: iterates resolve_for_camera() per
      camera (Phase 6B.166 §11.87.7 - planned)
...
# Slider key whitelist — exactly the 7 fields the Reolink REOLINK_MODEL
...
# Inclusive (min, max) for each slider, matching the Reolink REOLINK_MODEL
...
def resolve_for_camera(
    ...
    cameras = recipe.get("cameras", {})
    # Phase 6B.167 §13.4 Commit 17 (T3 C17): recipe.cameras keys are
    ...
    # ("<CAMERA_LABEL>") or legacy codes ("OUTSIDE_FRONT_SOLAR")
