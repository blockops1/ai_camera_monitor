...
# already runs Time Machine to a the NAS share, which gives disaster
...
#                         Updated by the perimeter vision sightings
#                         (the gatekeeper camera).
...
#
# Configuration keys (motion_gate_thresholds.json, etc.) use camera
# IDs directly (whatever you named them in camera-creds.env — e.g.
# CAMERA_A, CAMERA_B, ...). The pipeline passes camera_id through
...
# Retention
# Frames: 24 hours locally (semi-realtime — historical goes to the NAS).
...
# Bumped to 24h on 2026-07-28 (Operator) — wanted a full-day buffer for
