...
INPUTS:
    ...
    - env BROWSER_CHROME_PATH (default /Applications/Google Chrome.app/...)
      — Phase.166 §11.87.2. Override for cam_browser.py Playwright launch.
    - file camera-creds.env — camera RTSP credentials
    - file telegram-creds.env — Telegram bot credentials
    - file llm-creds.env — LLM and vision model endpoints (Phase.146)
...
OUTPUTS:
    ...
    - CAMERA_CREDS_FILE, TELEGRAM_CREDS_FILE, LLM_CREDS_FILE — env
      file paths (Phase.146 added LLM_CREDS_FILE)
    - BROWSER_CHROME_PATH — Phase.166 §11.87.2, Chrome binary path
    - MOTION_RECIPE_FILE — Phase.166 §11.87.3, motion recipe JSON path
...
CALLED BY:
    ...
    - infra.recipe: MOTION_RECIPE_FILE (Phase.166 §11.87.3)
...
RELATED:
    - .gitignore (data/, logs/, <sibling-project>/) — files this module
...
# NOTE: DATA_DIR is set above (lines 21-25) with FARMSURV_DATA_DIR override
...
# already runs Time Machine to a <nas> share, which gives disaster
...
# name_four.json were silently mirrored on every face enrollment. Privacy
...
# Vehicle state
#   known_vehicles.json  — vehicles the operator recognizes (color, type, label)
#   on_property.json     — runtime: is each vehicle on the property right now?
#                         Updated by CAM1 vision sightings
#                         (the gatekeeper camera; previously Building Front
#                         Corner pre-2026-07-29).
#   identity.json        — V-NNN stable IDs + per-vehicle attributes (Phase.6+).
...
# Animal state (Phase.165 §11.86.5, 2026-08-30).
#   known_animals.json — animals the operator recognizes (species, features, label).
...
# Vehicle tracker artifacts (Phase.6 audit trail, 2026-07-24).
...
# vehicle events (project plan §"Phase.9 vehicle-first two-message
...
LLM_CREDS_FILE = os.path.join(PROJECT_ROOT, "llm-creds.env")  # Phase.146

# Phase.167 §13.2 NEW-schema camera registry. Precedence:
...
# Motion recipe (Phase.166 §11.87.3, 2026-08-30).
...
# (e.g. a portable Chrome, a different Chromium build). Phase.166 §11.87.2.
...
# Retention
# Frames: 24 hours locally (semi-realtime — historical goes to <nas>).
...
# Bumped to 24h on 2026-07-28 (name one) — wanted a full-day buffer for
