...
PUBLIC API:
    ...
    _populate_legacy_fields_from_vehicles(result: dict) -> None
        Phase.129b (§11.52) — copy vehicles[primary_vehicle_index]
...
DOES NOT DO:
    ...
    - Decide what fields mean — the parser fills defaults for fields
      ...
      extra fields (Phase.78). Motion is not this module's job.

WHY HERE:
    Qwen3-VL responses are unreliable. Phase.66 (2026-08-08) shipped
    the cab_marker_lights "false"-as-string fix; Phase.77 relaxed the
...
def _populate_legacy_fields_from_vehicles(result: dict) -> None:
    """Phase.129b (§11.52) — populate top-level legacy fields from
    ...
    # Phase.130 (§11.53) — also populate the legacy `colors.vehicle`
...
def _validate_vision_result(result: dict) -> dict | None:
    ...
    Phase.66 (2026-08-08): relaxed the dict/object requirement. Vision
    ...
    silently for the 09:26 CAM1 Silverado arrival (alert 653ba844), which
    ...
    # Phase.6 — new structured vehicle list. Defaults: empty list
    ...
    # Phase.129b (§11.52) — BACKWARD-COMPAT population. The crop
    ...
    # Phase.78 (2026-08-14) — removed vehicle_motion and
    ...
    # doesn't request. Motion comes from infra.motion_detector.
    ...
    # Phase.27 (2026-07-30): top-level best_frame_index for vehicle
...
def _error_result(reason: str) -> dict:
    # Phase.78 (2026-08-14) — removed moving_vehicle_indices and
