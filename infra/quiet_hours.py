...
WHY HERE:
    Note 2026-08-04 OOB: "I am thinking that maybe we need to turn off
    ...
    Phase.112 (Note 2026-08-24): time logic refactored from a fixed
...
# OFS = Outside Front Solar, OFP = Outside Front Power, OBS = Outside
# Back Solar, OFG = Outside Front Garage. Front Door Outside and Back
# Door Inside are NOT in this set — the doorbell signals are security-
# relevant at any hour.
...
def in_quiet_hours(now: datetime, camera_name: str) -> bool:
    ...
    QUIET_HOURS_CAMERAS (FDO, BDI) are never silenced regardless of hour.
    ...
    """
    if camera_name not in QUIET_HOURS_CAMERAS:
        return False
