...
PUBLIC API:
  ...
  - subject_bbox_from_mask(mask, padding_px=8, erode_iterations=2)
      -> tuple[int, int, int, int] | None
        # Phase 6B.171 STRICT: tighter bbox on the moving SUBJECT, not the trail
  ...
  - subject_bbox_from_two_masks(mask_2to3, mask_3to4, padding_px=8,
                                 min_cc_area_px=500)
      -> tuple[int, int, int, int] | None
        # Phase 6B.173 STRICT: bbox at the logical AND of two consecutive
...
Phase 6B.171 NOTES (subject bbox — why this exists, why STRICT):
  - Problem: diff_pair_with_bbox returns the bbox of the largest connected
    ...
    of two (the "only 1 of 2 has a vehicle" symptom maintainer reported).
  ...
  - STRICT commit (maintainer 2026-09-01): subject_bbox_from_mask returns
...
def subject_bbox_from_mask(
    ...
    """Phase 6B.171 (2026-09-01): tighter bbox on the MOVING SUBJECT, not
    the trail of motion.
...
def subject_bbox_from_two_masks(
    ...
    maintainer: "I want a bbox at the logical AND of the motion from 1 to 2
    and 2 to 3."
    ...
    is applied to frame_3 (the anchor frame shared by both diffs). maintainer:
