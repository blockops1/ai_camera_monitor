...
def _apply_vision_none_baseline_override(
    ...
    if not timestamp or not _is_off_hours(timestamp):
        return alert
    if not camera_name or camera_name not in _get_vision_none_cameras():
        return alert
...
def _apply_parked_vehicle_baseline_override(
    ...
    if not timestamp or not _is_off_hours(timestamp):
        return alert
    if not camera_name or camera_name not in _get_parked_vehicle_cameras():
        return alert
    current_level = alert.get("threat_level", -1)
    if current_level != 1:
        return alert
...
def _apply_distant_vehicle_baseline_override(
    ...
    same pattern now handled for Outside Front Solar (north-pasture-road
    ...
    if not timestamp or not _is_off_hours(timestamp):
        return alert
    if not camera_name or camera_name not in _get_distant_vehicle_cameras():
        return alert
    current_level = alert.get("threat_level", -1)
    if current_level != 1:
        return alert
...
def _apply_static_object_baseline_override(
    ...
    if not timestamp or not _is_off_hours(timestamp):
        return alert
    if not camera_name or camera_name not in _get_static_object_cameras():
        return alert
    current_level = alert.get("threat_level", -1)
    if current_level != 1:
        return alert
...
def _apply_baseline_overrides(
    ...
    1. parked_vehicle_baseline  — IR-reflection hallucination on parking
       cameras (Outside Back Solar — dump trailer + F-350 reflective strips).
       Most specific. Older guardrail.
    2. distant_vehicle_baseline — vision sees a vehicle but flags it as
       ambiguous / distant / off-property (Outside Front Solar north-pasture-
