"""
camera_audio.py — Phase 6B.106 — Reolink CGI audio dispatch for camera speakers.

Phase 6B.106 (2026-08-22). Sends a `cmd=AudioFilePlay` request to a Reolink
...
STATUS: provisional (Phase 6B.106; will stabilize after first live audio
...
CALLS INTO:
    ...
    - infra.cameras (Phase 6B.167 §13.5 Commit 11) — for the
      camera_name → IP lookup. Replaces the previous hardcoded
      `_CAMERA_IP` dict so this module has no operator-specific
      IPs.

RELATED:
    - PLAN §11.36 — Phase 6B.106 audio dispatch design
    - PLAN §13.5 — Phase 6B.167 Commit 11 (infra.cameras-backed
      lookup)
    - camera configs — Reolink REOLINK_MODEL spec for the AudioFilePlay
      CGI endpoint (verified in production REOLINK_MODEL manual).

HISTORY:
    2026-08-30 — Phase 6B.167 §13.5 (Commit 11). Camera-name → IP
...
# Camera-name → IP resolution (Phase 6B.167 §13.5 Commit 11) is now
...
# Reolink client. Phase 6B.106 §11.36 step 9 — first iteration
...
def _resolve_camera_ip(camera_name: str) -> str | None:
    ...
    Phase 6B.167 §13.5 (Commit 11) — replaces the previous module-
