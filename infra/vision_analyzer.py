...
RELATED:
    - infra/vision_client.py — single-endpoint HTTP transport
      (Phase 6B.146; vision pool removed in 6B.147)
...
from infra.prompt_templates import (  # noqa: F401
    PROMPT_TEMPLATE,
...
def analyze_frames(
    ...
    # Phase 6B.66 (2026-08-08): prompt-mode override for the vehicle path.
    ...
    # 3-crop vehicle identifier (Phase 6B.65). Default None preserves
    ...
    payload = {
        ...
        "temperature": 0,
        "max_tokens": 2048,
        # Phase 6B.74 (2026-08-10) — schema dispatch by mode.
        ...
        # color+type-only scoring, and <visitor-name>'s white Silverado gets
        # misidentified as <owner-name>'s F-350 (alert ddeaefc5 false positive,
        ...
        "response_format": {
            "type": "json_schema",
            "name": "vehicle_crop" if mode == "crop" else "vision_analysis",
            "strict": True,
            "schema": (
                VEHICLE_CROP_SCHEMA_JSON
                if mode == "crop"
    ...
    return _error_result(f"could not parse response: {raw[:100]}")
...
def analyze_frames_queued(
    ...
    event_hint: str | None = None,
    captured_at: str | None = None,
    # Phase 6B.66 — forwarded to analyze_frames → select_prompt_template.
