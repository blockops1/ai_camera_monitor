...
PUBLIC API:
    DEFAULT_DIMENSION_WEIGHTS — default per-dimension weights (Phase 6B.29b)
...
DEFAULT_DIMENSION_WEIGHTS: dict[str, float] = {
    ...
    "roofline_match":       1.0,
    "tailgate_match":       1.0,
    # Phase 6B.48 (2026-08-01) — replaced 6B.46's hitch_match (0.5)
    # with three bigger, more permanent distinguishing features. maintainer
    ...
    # Phase 6B.75 (2026-08-10) — negative scores for color mismatch.
    # maintainer OOB 2026-08-10: *"the matcher tries to pick a blue truck,
...
def _dim_color_mismatch(sig: Sig, kv: KnownVehicle, spec: dict[str, Any]) -> bool:
    """Phase 6B.75: returns True when sig.color is known, kv.color is
...
def _dim_feature_match(sig: Sig, kv: KnownVehicle, feature_key: str) -> bool:
    ...
    Phase 6B.47 (2026-08-01) — read from FLAT sig first (post-6B.46
    refactor of `_signature_for_vehicle`), fall back to nested
    `sig.vehicle_features` for backward compat with older sigs or
    direct callers.

    Phase 6B.84 (2026-08-16) — absence-evidence fallacy fix (d99a38e6).
...
def _feature_present(val: object) -> bool:
    """Phase 6B.84 (2026-08-16) — does this value count as positive
...
# Phase 6B.48 — bed_cover alias handling. Qwen says "topper" or "camper_shell"
...
def _normalize_bed_cover(val: object) -> str | None:
    ...
    Phase 6B.84 (2026-08-16) — also returns None for absence signals
...
def _dim_bed_cover_match(sig: Sig, kv: KnownVehicle, spec: dict[str, Any]) -> bool:
    ...
    Phase 6B.84 (2026-08-16) — gates on _feature_present so both
...
DIMENSION_FUNCTIONS: dict[str, Any] = {
    ...
    # Phase 6B.75 (2026-08-10) — negative-color dimensions
    ...
    # Phase 6B.48 — replacing 6B.46's hitch_match.
...
def score_vehicle(sig: Sig, kv: KnownVehicle, spec: dict[str, Any]
    ...
    for dim_name, dim_fn in DIMENSION_FUNCTIONS.items():
        ...
        # Phase 6B.75 (2026-08-10) — changed from `if weight <= 0` to
