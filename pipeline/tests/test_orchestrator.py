...
def _synthetic_frames(tmp_path, n=4, moving=True):
    ...
    Phase 6B.115: 4 frames (gate's count) instead of 6.
    Phase 6B.115 (§11.46.6): returns BOTH disk paths AND in-memory
    ...
    pil_frames = []
    for i in range(n):
        # 3-channel JPEG so cv2.imwrite matches the live CAM1 format.
...
def _fake_gate_crops(out_dir):
    ...
    Phase 6B.115: tests that exercise the vision path need a non-empty
...
def test_run_pipeline_with_motion_and_match(tmp_path, monkeypatch):
    ...
    assert "CAM1" in result.motion_telegram_body
    ...
    # Phase 6B.121: the slim match body shows the human label, not
    ...
    assert "employee_b's white pickup" in result.second_telegram_body
...
def test_run_pipeline_empty_known_vehicles(tmp_path, monkeypatch):
    ...
    Phase 6B.115: the gate is the sole producer of crops. To exercise
...
def test_run_pipeline_alert_id_removed_phase_6b114(tmp_path, monkeypatch):
    """Phase 6B.114: alert_id no longer in user-facing Telegram bodies.
