...
## Section 2: Web Search (DuckDuckGo) → `duckduckgo-search`

> Full skill: `duckduckgo-search` (absorbed — complements passive recon with active search)

Free web search via DuckDuckGo — text, news, images, video. No API key required.

### When to Use
- General research about a domain, company, or topic
- News and current events lookup
- Image search for reference materials
- Background research that complements passive domain intelligence

### Key Features
- Text, news, images, and video search
- No API key required
- Privacy-respecting search
- `ddgs` CLI preferred when installed; falls back to Python `ddgs` library
