...
that decides whether the vehicle in the alert is "employee_b's pickup"
...
This repo is the **refactored tree** (`~/farm-surveillance-refactor/`).
There is also a legacy tree at `~/farm-surveillance/` which is dormant
...
3. The **listener's routing decision** sends each webhook to either
   ...
   - Vehicle pipeline handles CAM1 (vehicle tracker). It gets
     `event=md`, `event=vehicle`, or any event where the gate's YOLO
     says vehicle.
   - Person pipeline handles CAM2 (person tracker) and gets
...
5. If the gate passes, the **vision step** (`infra/vision_client.py`)
   ...
   **pairwise differential** (`abs(frame_3 − frame_4)` brightened,
   ...
   inference share the same unified endpoint since Phase 6B.158.
...
8. The **notifier** (`infra/notifier.py`) sends the alert to Telegram
   ...
   CAM2). Cooldowns prevent alert floods.
...
- One or more **Reolink IP cameras** with motion/vehicle/person detection
  ...
  to be reachable via a routable IP). Tested cameras: REOLINK_MODEL,
  REOLINK_MODEL, RLC-81MA, Reolink Doorbell. Any Reolink camera that can
...
| Ollama **or** llama.cpp (`llama-server`) | Local inference for Qwen3.6-35B-A3B (vision + text in one model since Phase 6B.158). | See [Local LLMs](#local-llms-vision--alert-generation) below. |
...
Since Phase 6B.158 (§11.81), the listener calls a **single** unified
...
```
FRONT_RTSP_URL=rtsp://user:pass@CAMERA_IP:554/h264Preview_01_main
BACK_RTSP_URL=rtsp://user:pass@CAMERA_IP:554/h264Preview_01_main
OUTSIDE_FRONT_SOLAR_RTSP_URL=rtsp://user:pass@CAMERA_IP:554/...
```
...
```bash
# 1. Clone
git clone <this-repo> ~/farm-surveillance-refactor
...
│   ├── _gate_aware_capture.py     # Frame producer for the gate (Phase 6B.115)
...
│   ├── camera_creds.py          # RTSP credential parser (Phase 6B.166 §11.87.6)
│   ├── recipe.py                # Motion-recipe loader (CLI flags + config/motion_recipe.json + env vars) (Phase 6B.166 §11.87.3)
│   ├── camera_aliases.py        # Friendly-name resolver (CAM1 → "Outside Front Solar")
...
│   ├── tune_510a_motion_sensitivity.py    # REOLINK_MODEL CGI motion-tuning (CLI flags + JSON recipe + env vars)
...
│   ├── enroll_animal.py                   # Enroll an animal from a captured alert (Phase 6B.165)
...
│   ├── _test_ben_*.py                     # Legacy person_a-face tuning probes (one-time use)
...
| env `PIPELINE_USES_GATE_CROPS=1` | Wire the gate's `GateVerdict.crop_paths` directly into the vision pipeline (no legacy re-capture). Phase 6B.108a/6B.115 default. |
...
| env `PERSON_AUDIO_ENABLED` | Optional microphone capture from cameras (Phase 6B.10x, parked — defaults off). |
| env `VISION_LLM_URL` | Vision LLM chat completions URL. Default `http://127.0.0.1:8093/v1/chat/completions` (unified qwen3.6 endpoint since Phase 6B.158). Override via env var or `llm-creds.env`. |
...
| env `TEXT_LLM_URL` | Text LLM chat completions URL. Default `http://127.0.0.1:8093/v1/chat/completions` (same endpoint as vision — unified server since Phase 6B.158). |
...
1. **Built-in defaults** — Qwen3.6-35B-A3B on
   ...
   text (unified server since Phase 6B.158 / §11.81). Production runs
...
```
# Vision LLM — local qwen3.6 unified endpoint (Phase 6B.158)
VISION_LLM_URL=http://127.0.0.1:8093/v1/chat/completions
VISION_LLM_TOKEN=
VISION_LLM_MODEL=qwen3.6

# Text LLM — same unified endpoint as vision (Phase 6B.158)
...
| `/preview?camera=<name>&time=<ISO>` | GET | Synology NAS preview thumbnail for a given timestamp (Phase 6B.124). Use case: post-hoc lookups when Reolink AI missed a slow-moving vehicle. |
...
# Operator snapshot
curl -sS 'http://localhost:8090/snapshot?camera=CAM1' -o /tmp/ofs.jpg
...
- **Latency:** end-to-end alert is **~15–73 s** (today, 16 measured
  events on CAM1+CAM2+OFBS; 28–48 s typical, with one 112 s outlier on
  CAM2 during a sustained motion burst). The motion gate deliberately
  ...
  vision LLM time.
- **Operator scripts (Phase 6B.166 §11.87):** all 8 operator scripts in
...
- **Listener:** PID 41135 on commit `e8f477d` (Phase 6B.166 §11.87.8,
...
- **Routing:** CAM1 → vehicle pipeline (vehicle tracker). CAM2 → person
...
This is the refactored tree. The legacy `~/farm-surveillance/` tree
...
| 2026-08-17 | Phase 6B.88: `GET /snapshot` endpoint | `ee1cc10` |
| 2026-08-21 | Phase 6B.105c: `_process_alert` slim — 4248 → 1855 lines | (multiple) |
| 2026-08-23 | Phase 6B.108a: gate-aware pipeline cutover — gate runs ABOVE person-branch | `0c04506` |
| 2026-08-23 | Phase 6B.107: motion gate cutover with YOLOv8n pre-filter (§11.37) | (multiple) |
| 2026-08-24 | Phase 6B.109: V2 gate fixes (tighter routing, threshold alignment) | `2f9499e` |
| 2026-08-25 | Phase 6B.115: gate as sole producer of frames + crops (in-memory verdict) | (multiple) |
| 2026-08-26 | Phase 6B.116b: night-heuristic timestamp plumbing + reason field | `e710bbe` |
| 2026-08-26 | Phase 6B.119-123: code-quality toolchain (bandit + mypy + vulture + pre-commit) | `ab720e3`...`9bc4cfe` |
| 2026-08-27 | Phase 6B.140 (§11.61): person-gatekeeper camera CAM1 → CAM2 | `da148c1` |
| 2026-08-27 | Phase 6B.141 (§11.64): person-emit 6-image Telegram media group | `6e6e8ac` |
| 2026-08-27 | Phase 6B.142 + 6B.143 (§11.65): match-alert 2-crop album + CAM2 pre-event trail | `da4f812` |
| 2026-08-27 | Phase 6B.144 (§11.66): revert YOLO-tighten + send 3-image payload to Qwen | `c429ee7` |
| 2026-08-27 | Phase 6B.145 (§11.67): promote `event=md` → 'people' on gate=person | `76e79f9` |
| 2026-08-27 | Phase 6B.146 (§11.68): configurable LLM endpoints via `llm-creds.env` + env vars (Bearer auth) | `0f36f74` |
| 2026-08-27 | Phase 6B.147 (§11.69): drop `infra/vision_pool.py`, single vision URL only | `a56e008` |
| 2026-08-28 | Phase 6B.163 person-stable-attributes: Tier 3 stable-attribute matching for person identity | (multiple) |
| 2026-08-28 | Phase 6B.165: animal event pipeline (audit-only, no Telegram) + known-animals registry | (multiple) |
| 2026-08-30 | Phase 6B.166 §11.86: animal matcher + wider-scope schema (Qwen authoritative); per-camera animal cooldowns | (multiple) |
| 2026-08-30 | Phase 6B.166 §11.87: parameterize 8 operator scripts (CLI flags + `config/motion_recipe.json` + env vars); added `infra/recipe.py` and `infra/camera_creds.py`; 53 new tests in `test_enroll_vehicle_argparse.py`. Closed: `f47554d`...`0bb3260`...`e8f477d` (8 commits). | `f47554d`→`e8f477d` |

After cutover:

- `~/Library/LaunchAgents/ai.farm.surveillance-listener-refactor.plist`
  is the active plist (managed via `launchctl`).
- `~/farm-surveillance/` stays on disk indefinitely — read-only fallback.
...
The repo's quality toolchain (Phase 6B.119+) is **ruff + bandit + mypy
...
mypy **0 errors** (down from 88 in 6B.123 — all fixed per maintainer
...
(unrelated to §11.87 — known employee_a/employee_b color-weight case under
...
- **Phase 6B.166 §11.86** — animal event pipeline (audit-only), known-animals
  registry + enrollment script, wider-scope schema, per-camera animal
  cooldowns.
- **Phase 6B.166 §11.87** — operator-script parameterization refactor:
...
- **§11.47 — Train a night-specific YOLOv8 model.** Scaffolding is in
  ...
  data.
- **§11.42 — Color penalty too weak (employee_a blue / employee_b white case).**
  `vehicle_matcher/scoring.py` treats color mismatch as a single zero
  dimension among 7. employee_a's blue Chevrolet (not yet enrolled) is
  being matched to employee_b's white Chevrolet Silverado at score 4.50
  (above the 0.6 threshold). Three proposed fixes: (a) enroll
  employee_a's truck, (b) strengthen the color penalty to a negative
  score, (c) both. Awaiting maintainer decision.
- **6B.110 — QuickClassifier top_class priority.** Highest-confidence
  ...
  animal > other). Awaiting maintainer prioritization.
...
5. **`paths.py` is the only place paths live.** Never hardcode
   `HOME_DIR/...`. Use `infra.paths.PROJECT_ROOT`, `DATA_DIR`,
   `LISTENER_LOG`, etc.
6. **No cross-tree coupling.** Nothing in this repo may reference
   `~/farm-surveillance/`. The legacy tree is the rollback target.
