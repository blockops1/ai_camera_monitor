...
Phase.100: sends ALL crops to Qwen in a single multi-image API
...
two live-burn cases this fixed (OFS foliage, OFG Silverado).
"""
...
# Phase.100: per-crop selection logic (`_informative_score`,
...
# alerts that motivated the change (OFS foliage, OFG Silverado).
...
def _persist_raw_vision(
    ...
    """
    Phase.85/6B.100: persist the raw Qwen response to
    ``<output_dir>/raw_vision_multi.json``. Phase.100 sends all crops
...
def identify_from_crops(
    ...
    """Send ALL crops to vision in a single API call (Phase.100).
    ...
    Args:
        ...
        output_dir: If provided, persist the single multi-crop vision
            ...
            Phase.85/6B.100: enables forensic debugging when Qwen
        ...
        pairwise_diff_path: Phase.144 (§11.66). Path to the
    ...
    Behavior change (6B.100):
        ...
        - On OFS foliage events, the third crop's vision call hallucinated
          "green suv" while the first two correctly said "no vehicle",
          and the picker promoted the hallucination.
        - On OFG Silverado+Tesla, three valid signatures tied at score 16
        ...
        - OFS foliage → "no vehicle visible" (correct suppression)
        - OFG Silverado+Tesla → "Silverado 1500 Z71" (correct moving
          vehicle)
    ...
    # --- Run vision ONCE on all crops (Phase.100) ---
    ...
    # Phase.144 (§11.66): append pairwise diff image so Qwen sees
    ...
    # Phase.85/6B.100: persist raw vision response for forensic debugging.
    if output_dir:
        _persist_raw_vision(
