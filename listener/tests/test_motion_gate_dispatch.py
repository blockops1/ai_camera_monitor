...
def test_maybe_run_motion_gate_uses_frame_offsets_for_gatekeeper_camera(monkeypatch):
    ...
    # the offsets anchor T_w-2s, T_w+0s, T_w+2s, T_w+4s — the user-specified
    # trail from 2026-08-28 OOB. See _compute_gatekeeper_offsets docstring.
    class _FakeReader:
        stream_fps = 2.0
        ring_size = 32
    ...
    # at 2fps ring=32 capture_delay=8s, that's (11, 15, 19, 23) =
    # (T_w-2s, T_w+0s, T_w+2s, T_w+4s).
    assert mock_capture.call_count == 1
    call_kwargs = mock_capture.call_args.kwargs
    assert call_kwargs.get("frame_offsets") == [11, 15, 19, 23], (
        f"CAM5 gatekeeper expected frame_offsets=[11, 15, 19, 23] "
        f"(T_w-2s, T_w+0s, T_w+2s, T_w+4s); got {call_kwargs.get('frame_offsets')!r}"
    )
...
def test_maybe_run_motion_gate_uses_frame_offsets_for_ofg_person_gatekeeper(monkeypatch):
    ...
    the operator's "person pipeline working just like the vehicle pipeline"
    directive requires.
    ...
    call_kwargs = mock_capture.call_args.kwargs
    assert call_kwargs.get("frame_offsets") == [11, 15, 19, 23], (
        f"CAM3 person-gatekeeper expected frame_offsets=[11, 15, 19, 23] "
        f"(T_w-2s, T_w+0s, T_w+2s, T_w+4s); got {call_kwargs.get('frame_offsets')!r}"
    )
    assert call_kwargs.get("count") == 4
...
def test_gatekeeper_cameras_constant_is_all_six_cameras():
    ...
    expected = frozenset({
        ...
        # gatekeeper CAM1; remaining 5 added §11.79).
