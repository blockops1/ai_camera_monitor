...
def test_maybe_run_motion_gate_uses_frame_offsets_for_gatekeeper_camera(monkeypatch):
    """Phase 6B.128 — CAM5 gatekeeper gets a 4-frame pre-event motion trail
    ...
    Phase 6B.160 (2026-08-28) bugfix: at 2fps the ring spans 45s, so the
    ...
    # the offsets anchor T_w+2s, T_w+4s, T_w+6s, T_w+8s — Phase 6B.174
    ...
    # (T_w+2s, T_w+4s, T_w+6s, T_w+8s) — Phase 6B.174 default.
...
def test_maybe_run_motion_gate_no_frame_offsets_for_non_gatekeeper(monkeypatch):
    ...
    Phase 6B.143 (2026-08-27): CAM3 is now a gatekeeper.
...
def test_maybe_run_motion_gate_uses_frame_offsets_for_ofg_person_gatekeeper(monkeypatch):
    """Phase 6B.143 (2026-08-27, maintainer OOB): CAM3 is a PERSON_GATEKEEPER_CAMERAS
    ...
    maintainer's "person pipeline working just like the vehicle pipeline"
    directive requires.

    Mirrors test_maybe_run_motion_gate_uses_frame_offsets_for_gatekeeper_camera
    but for the CAM3 camera.

    Phase 6B.160 (2026-08-28) bugfix: same as CAM5 test — at 2fps the ring
...
def test_person_gatekeeper_cameras_constant_is_all_six_cameras():
    ...
    active cameras (was just CAM3 through 6B.140). maintainer OOB 2026-08-28:
...
def test_all_gatekeeper_cameras_union():
    """Phase 6B.143 (2026-08-27): ALL_GATEKEEPER_CAMERAS is the union
...
def test_gatekeeper_frame_offsets_count_matches_gate_input():
    ...
    Phase 6B.160 (2026-08-28): with ring=90, the new constant is
