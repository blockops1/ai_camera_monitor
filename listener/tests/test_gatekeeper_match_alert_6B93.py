...
gatekeeper-camera match-alert path. History:
  ...
  Phase.104 (2026-08-20) — CAM3 demoted from gatekeeper tier.
    Note: "I just want CAM3 to be like the other cameras. Not a vehicle
...
def test_ofg_vehicle_event_reaches_match_alert_path():
    ...
    re-added it (along with the 5 other non-CAM5 cameras) after Note
...
def test_pre_6B93_gate_would_have_dropped_ofg():
    ...
    entirely (per Note's design), so CAM3 once again does not reach
