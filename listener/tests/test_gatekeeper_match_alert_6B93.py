...
gatekeeper-camera match-alert path. History:
  ...
  Phase.87 (2026-08-17) — added CAM3 as a second gatekeeper camera.
  Phase.93 (2026-08-18) — fixed the CAM3 silent-drop bug:
    ...
    v_owner1_darkblue_tesla_y even though the crops would have scored above
    the spec threshold. The fix: the gate changed to
    `if camera_name in GATEKEEPER_CAMERAS:` and CAM3 was added to the set.

  Phase.104 (2026-08-20) — CAM3 demoted from gatekeeper tier.
    the operator: "I just want CAM3 to be like the other cameras. Not a vehicle
...
Pins (post-6B.104, then post-§11.79 on 2026-08-28):
  - §11.79: GATEKEEPER_CAMERAS now contains ALL 6 cameras (CAM5, CAM3,
    ...
    rows). Note 2026-08-28: "Let's do this for now and see how it
...
def test_ofg_vehicle_event_reaches_match_alert_path():
    ...
    re-added it (along with the 5 other non-CAM5 cameras) after the operator
...
def test_obs_vehicle_event_reaches_match_alert_path():
    ...
    Persistent RTSP (Phase.151), 3-crop vision (gate's standard
...
def test_pre_6B93_gate_would_have_dropped_ofg():
    ...
    entirely (per the operator's design), so CAM3 once again does not reach
