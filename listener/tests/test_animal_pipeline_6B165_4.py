...
# Approved by the operator on 2026-08-29. See PLAN §11.86.4.
# Phase.167 §13.4 Commit 17 (T3 C17): keys are CAM{N} codes per
...
class TestRealConfigAnimalCooldowns:
    ...
    need the operator's explicit approval (PLAN §11.86.4)."""
    ...
    def test_camera_has_animal_cooldown(self, config, camera, expected):
        ...
        assert gc["animal"] == expected, (
            ...
            "Approved values: CAM5/CAM3/CAM4/CAM6=300s, CAM1=180s, CAM2=60s (the operator 2026-08-29)."
        )
...
class TestIsGateEnabledAnimal:
    ...
    def test_animal_defaults_true_when_gate_enabled_absent(self, monkeypatch, tmp_path):
        ...
        Phase.167 §13.4 Commit 17: JSON keys are CAM{N} codes per
...
class TestConfigSchemaContract:
    ...
    def test_animal_cooldown_not_zero_for_residential_cameras(self):
        ...
        Phase.167 §13.4 Commit 17: real-config lookup uses CAM{N}
        ...
        assert cam2_animal == 60, (
            ...
            f"(the operator 2026-08-29). CAM2 is the workshop; resident "
