...
Phase.106 (2026-08-22). Tests the 4 stages (capture → identify → match
...
- gate_aware_person_capture reads from a GateVerdict fixture
  (Phase.139, §11.60); the legacy `capture_frames` RTSP pull
  was removed.
...
- send_photo_with_caption / send_message / send_photo_group record their
  ...
  (Phase.141: person_emit now uses send_message + send_photo_group
...
Phase.139 (§11.60): tests now provide a GateVerdict via the
...
def gate_verdict_with_person_frames(tmp_path):
    """GateVerdict with 4 PIL frames for person path (Phase.139, §11.60).
    ...
    indices 1 and 2). Phase.140 (2026-08-27): camera_id = CAM3.
    """
...
# ---------------------------------------------------------------------------
# Stage 1: capture (Phase.139 stub — gate is the sole frame producer)
# ---------------------------------------------------------------------------


class TestPersonCaptureStage:
    """Phase.139: person_capture_stage is a deprecation stub.
...
class TestPersonIdentifyStage:
    def test_identify_with_face_visible(self, captured_frame_paths, vision_result_person_visible):
        ...
        with patch(
            ...
            return_value={
                "faces": [{
                    ...
                    "embedding": [0.1] * 512,
                    "identified_name": "name two",
                    "confidence": 0.7,
                    "is_known": True,
                }],
                "identified_person": "name two",
                "best_confidence": 0.7,
            },
        ):
        ...
        assert ctx.face_recognition["identified_person"] == "name two"
...
class TestPersonMatchStage:
    def test_match_face_recognition_wins(
        ...
        ctx = _make_ctx(
            ...
            face_recognition={
                "faces": [{
                    "bbox": [100, 100, 200, 200],
                    "embedding": [0.1] * 512,
                    "identified_name": "name one",
                    "confidence": 0.85,
                    "is_known": True,
                }],
                "identified_person": "name one",
        ...
        assert ctx.matched_name == "name one"
    ...
    def test_match_clothing_when_face_no_match(
        ...
        with patch(
            "listener.person_event_pipeline._load_known_persons_for_matching",
            return_value=[{"name": "name two", "clothing_upper_color": "red", "role": "resident"}],
        ):
            person_match_stage(ctx)
        assert ctx.matched_name == "name two"
    ...
    def test_match_empty_vision_returns_no_match(self, captured_frame_paths):
        ...
        with patch(
            "listener.person_event_pipeline._load_known_persons_for_matching",
            return_value=[{"name": "name one", "clothing_upper_color": "red"}],
        ):
            person_match_stage(ctx)
        assert ctx.matched_name is None
...
class TestPersonEmitStage:
    def test_emit_sends_telegram_with_structured_body(
        ...
        # Phase.141 (2026-08-27): body now goes via send_message
        ...
        assert "name one" in sent_text
        assert "🚶" in sent_text
        # Phase.140 (2026-08-27): camera is CAM3, not CAM1
        assert "CAM3" in sent_text
        # Phase.141 (2026-08-27): album-skip path runs here
    ...
    def test_emit_falls_back_to_text_only_when_no_frames(self):
        ...
        with patch(
            ...
            ctx.frame_paths = []
            person_emit_stage(ctx)
        # Phase.141 (2026-08-27): body always goes via send_message.
    ...
    def test_emit_sends_2_image_album_crops_only(
        ...
        Image.new("RGB", (200, 300), color=(255, 0, 0)).save(
            str(out_dir / "frame_003_crop180_200_320x480.jpg"), "JPEG"
        )
        Image.new("RGB", (200, 300), color=(0, 255, 0)).save(
            str(out_dir / "frame_004_crop200_220_300x460.jpg"), "JPEG"
        )
    ...
    def test_emit_album_sends_partial_when_one_crop_missing(
        ...
        """Phase.153 (2026-08-28): if only one crop exists (gate
...
class TestProcessPersonEvent:
    """Phase.139: orchestrator tests use a GateVerdict with PIL frames
    ...
    def test_full_pipeline_match(
        ...
        assert result["matched_name"] == "name one"
    ...
    def test_pipeline_no_person_in_vision_returns_unknown(
        ...
        with patch(
            ...
            result = process_person_event(ctx)
        assert result["matched_name"] is None
        # Phase.162: no_person_in_frame is suppressed — no Telegram, no body
