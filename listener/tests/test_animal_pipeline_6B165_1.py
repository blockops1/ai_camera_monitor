...
WHAT THIS TESTS:
    Phase.165 §11.86.1 animal pipeline scaffold:
...
RUN: cd <install-path>/ai_camera_monitor && .venv/bin/python -m pytest \\
...
def test_dispatch_branch_routes_animal_event_to_animal_pipeline(monkeypatch):
    ...
    # The animal branch must come BEFORE the vehicle catch-all
    # (lazy imports) so animals don't fall through to vehicle pipeline
    animal_branch_pos = src.find('event_lower == "animal"')
    lazy_imports_pos = src.find("# Lazy imports")
    assert 0 < animal_branch_pos < lazy_imports_pos, (
        "animal dispatch branch must come before the vehicle "
        "catch-all (lazy imports block)"
    )
