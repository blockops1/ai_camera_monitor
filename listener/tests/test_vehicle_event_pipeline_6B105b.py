"""
test_vehicle_event_pipeline_6B105b.py — Phase 6B.105b unit tests for the
...
from listener.vehicle_pipeline.alert import generate_alert_stage
...
from listener.vehicle_pipeline.select_frame import select_best_frame_stage
from vehicle_matcher import MatchVerdict, NoMatch

# Phase 6B.115: capture_stage was REMOVED. gate_aware_vehicle_capture
...
def base_ctx(tmp_path) -> AlertContext:
    ...
    # Phase 6B.115 (§11.46.6): identify_stage's gate path requires
...
def non_gatekeeper_ctx(base_ctx: AlertContext) -> AlertContext:
    ...
    # Phase 6B.168: keep camera_code in sync with camera_name mutation.
    # "Back Door Outside" isn't in cameras.env so _code_for_camera returns
    # it unchanged — which is fine, because it isn't in the gatekeeper
    # set anyway.
...
#
# Phase 6B.115 (2026-08-25): capture_stage was REMOVED. The motion gate
...
class TestIdentifyStage:
    ...
    def test_identify_stage_vehicle_with_motion_uses_3crop(self, vehicle_ctx, monkeypatch):
        ...
        Phase 6B.115: the legacy detect_motion() is gone. The motion
...
# --- Phase 6B.121: TG#1 frame selection --------------------------------


class TestArrivingFrameSelection:
    """Phase 6B.121 (2026-08-22): TG#1 (arriving Telegram) must send the
    ...
    def test_tg1_uses_first_non_absent_trajectory_frame(
        ...
        # Phase 6B.115: trajectory is 4 cells (frame_001..frame_004).
    ...
    def test_tg1_falls_back_to_frame_paths_zero_when_all_absent(
        ...
        class FakeMotionObject:
            # Phase 6B.115: 4-cell trajectory (gate's frame count).
    ...
    def test_tg1_picks_fourth_frame_when_first_three_absent(
        ...
        """maintainer OOB: 'probably the fourth' — if frame 1-3 are absent,
        ...
        class FakeMotionObject:
            # Phase 6B.115: 4-cell trajectory.
...
# --- Phase 6B.120 regression: vehicle-without-motion fallback mode ---


class TestIdentifyStageFallbackMode:
    """Phase 6B.132 (§11.54): no vehicle fallback.

    maintainer 2026-08-26: "I don't want the non-crop fallback to exist. We
    ...
    def test_vehicle_without_motion_is_suppressed(self, vehicle_ctx, monkeypatch):
        ...
        description. maintainer's directive: kill the fallback, suppress the
        ...
        def fake_analyze(*args, **kwargs):
            raise AssertionError(
                ...
                "Phase 6B.132: no fallback, alert must be suppressed"
            )
    ...
    def test_non_vehicle_first_pass_keeps_motion_mode_hint(self, base_ctx, monkeypatch):
        ...
        in Phase 6B.132. The non-vehicle path is unchanged in this commit;
        a separate redesign lands later (per maintainer's plan).
        """
        from listener.vehicle_pipeline.identify import _non_vehicle_first_pass
...
# --- Phase 6B.121: alert.jsonl gets match body when matched -------------


class TestEmitAlertJsonlMatchBody:
    """Phase 6B.121 (2026-08-22): when the match loop populates
...
# --- Phase 6B.122 regression: b079e97a "Normal Daytime Scene" bug -------


class TestEmitAlertJsonlDeferUntilAfterMatchLoop:
    """Phase 6B.122 (2026-08-22): 6B.121 was broken — it tried to swap
    ...
    def test_append_alert_receives_swapped_body_when_match_loop_populates_first(
        ...
        monkeypatch.setattr(
            "listener.vehicle_pipeline.emit._emit_match_loop",
        ...
        # Phase 6B.123 (2026-08-22): no-match → L2 (unknown vehicle at CAM5)
        # per maintainer's threat-level spec. Was L1 in 6B.122 (when the swap
...
# --- Phase 6B.123 regression: vehicle threat level routing ---


class TestVehicleThreatLevelRouting:
    """Phase 6B.123 (2026-08-22, maintainer OOB): vehicle threat-level routing.

    Per maintainer 2026-08-22:
    ...
    def test_match_routes_to_L1(self, vehicle_ctx, monkeypatch):
        ...
        maintainer 2026-08-22: 'L one are things that I want to get an
    ...
    def test_no_match_routes_to_L2(self, vehicle_ctx, monkeypatch):
        ...
        maintainer 2026-08-22: 'L2 would be an unknown vehicle moving
