...
class TestEmptyFramesGate:
    """single_pipeline drops events when the motion gate produced no frames.

    Phase §11.115 regression (2026-09-02): the motion gate can suppress
    with an empty frame_paths list when no subject is detected across the
    4-frame window. single_pipeline.run must treat this as a log-only
    "no_frames" outcome rather than crashing in frame_diff_fn on
    IndexError.
    """
    ...
    def test_drops_alert_when_no_frames(self) -> None:
        ...
        result = single_pipeline.run(
            ...
            camera_code="CAM5",
            captured_at="2026-09-02 12:34:56",
            frame_paths=[],  # motion gate suppressed → empty list
            event_type="motion",
            **deps,
        )
        # No Qwen, no Telegram, no matcher — but log_only=True so the
        # outcome is observable (skipped_reason + log_only flag).
    ...
    def test_no_frames_does_not_call_frame_diff(self) -> None:
        """frame_diff_fn must NOT be invoked when frame_paths is empty
        (otherwise it crashes on IndexError)."""
        deps = _make_deps(
            qwen_responses=[],
            has_rtsp=True,
        )
...
class TestPairwiseDiffInjected:
    """§11.115.22: pairwise_diff image is prepended to the Qwen payload.

    Note's diagnosis (2026-09-03): one moving vehicle past two parked
    vehicles → Qwen returned 3 vehicles because it had no spatial context
    for which was moving. The pairwise_diff PNG (already on disk from
    §6B.144) carries that context. Without it in the prompt, Qwen
    hallucinates the parked vehicles as the subject.

    single_pipeline.run accepts an optional `pairwise_diff_path` and
    prepends it as the first image, so Qwen sees:
        [diff, crop_a, crop_b]
    Order is fixed: diff first so prompt templates can name images 1/2/3.
    """

    def test_qwen_receives_pairwise_diff_first_when_provided(self) -> None:
        deps = _make_deps(
            qwen_responses=[
                '{"class": "vehicle", "confidence": 0.9, "reasoning": "x"}',
                "{}",
        ...
        single_pipeline.run(
            ...
            frame_paths=["/tmp/orig/0.jpg", "/tmp/orig/1.jpg"],
            event_type="motion",
            pairwise_diff_path="/tmp/diff/pairwise.png",
            **deps,
        )
        # Both Qwen calls received [diff, crop_a, crop_b] — diff first
        for call in deps["qwen_fn"].calls:
            assert call["frame_paths"] == [
                "/tmp/diff/pairwise.png",
                "/tmp/diff/A.jpg",
                "/tmp/diff/B.jpg",
            ]

    def test_qwen_backward_compatible_without_pairwise_diff(self) -> None:
        """No pairwise_diff_path → legacy 2-crop payload (don't break §11.115.5)."""
        deps = _make_deps(
            qwen_responses=[
                '{"class": "vehicle", "confidence": 0.9, "reasoning": "x"}',
                "{}",
        ...
        # Legacy behavior preserved when pairwise_diff_path is omitted
        for call in deps["qwen_fn"].calls:
            assert call["frame_paths"] == ["/tmp/diff/A.jpg", "/tmp/diff/B.jpg"]
