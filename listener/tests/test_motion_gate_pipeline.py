"""Tests for listener/motion_gate_pipeline.py — pre-Qwen motion gate (Phase 6B.107).
...
class FakeClassifier:
    ...
    Phase 6B.116 (timestamp-fix): classify_frame accepts a `timestamp` kwarg
...
def test_route_rule2_only_one_person_falls_through_to_rule5():
    ...
    Phase 6B.137 (§11.59): the historical catchall returned decision="vehicle"
...
def test_route_rule5_mixed_vehicle_wins():
    ...
    Phase 6B.137 (§11.59): this test now uses person + car (a vehicle IS in
...
def test_run_one_person_only_routes_to_vehicle(tmp_path):
    """Phase 6B.137 (§11.59): person + non-vehicle-other → SUPPRESS, not vehicle.
...
def test_route_decision_prefers_per_crop_reason():
    """Phase 6B.116 reason plumbing — when both crops suppress, the routing
...
def test_route_decision_falls_back_to_no_object_detected():
    """Phase 6B.116 reason plumbing — when both crops suppress with no reason
...
# Phase 6B.137 (§11.59) — see PLAN §11.59 for context.
#
...
def test_route_catchall_no_vehicle_mix_suppresses_with_class_reason():
    """Phase 6B.137 (§11.59): top is "train" with the OTHER crop also a
...
def test_route_catchall_zebra_suppresses_with_class_reason():
    """Phase 6B.137: 'zebra' detected in one crop (which YOLO occasionally
...
def test_route_catchall_fire_hydrant_suppresses_with_class_reason():
    """Phase 6B.137: fire_hydrant (or any other small COCO class) detected
...
def test_route_catchall_vehicle_in_other_position_still_wins():
    """Phase 6B.137: 'fire hydrant' top + 'car' lower — vehicle IS in the
...
def test_route_decision_matches_production_log_9a78a254():
    """Phase 6B.137 regression test — the specific alert that exposed the bug.
...
# Phase 6B.152 — per-camera × per-event-type gate_enabled tests.
...
class TestIsGateEnabled:
    """Phase 6B.152 — per-camera gate_enabled matrix."""
...
# ---------------------------------------------------------------------------
# Phase 6B.169 §11.93 — crop source frame regression tests
# ---------------------------------------------------------------------------
