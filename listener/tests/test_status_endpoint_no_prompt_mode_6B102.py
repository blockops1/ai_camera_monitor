"""
test_status_endpoint_no_prompt_mode_6B102.py — Phase 6B.102 / PLAN §11.30.

Phase 6B.78 (2026-08-14) deleted VEHICLE_COMBINED_PROMPT_TEMPLATE (and
VEHICLE_MOTION_PROMPT_TEMPLATE) from infra/prompt_templates.py. Phase 6B.50's
...
Phase 6B.102 deletes that block entirely. These tests pin:
...
def test_status_200_no_prompt_mode_field():
    """Phase 6B.102: /status must not include a prompt_mode key.
    ...
    assert "prompt_mode" not in body, (
        ...
        f"Phase 6B.102 deleted this block; if you're seeing this, "
...
def test_status_no_import_error_in_any_value():
    """Phase 6B.78's silent ImportError shouldn't be hiding anywhere in /status.
    ...
    assert not leaks, (
        ...
        f"Phase 6B.78's silent-import-failure pattern should not return."
    )
...
def test_prompt_module_no_longer_exports_combined_template_constant():
    ...
    assert not hasattr(pt, "VEHICLE_COMBINED_PROMPT_TEMPLATE"), (
        ...
        "Phase 6B.78 deliberately deleted it; re-adding resurrects the "
        "silent-ImportError pattern Phase 6B.102 cleaned up."
    )
    assert not hasattr(pt, "VEHICLE_MOTION_PROMPT_TEMPLATE"), (
        ...
        "Phase 6B.78 deleted both motion-judging prompts together."
    )
