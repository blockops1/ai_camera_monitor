"""
test_vehicle_event_pipeline_6B111.py — Phase 6B.111 integration tests.
...
def vehicle_ctx(tmp_path) -> AlertContext:
    ...
    Phase 6B.115: ctx.frames pre-populated with 4 PIL stubs so
...
class TestTrajectoryInjection:
    ...
    def test_no_motion_skips_injection(self, vehicle_ctx, monkeypatch):
        ...
        Phase 6B.132 (§11.54): the previous test asserted that the
        ...
        # Phase 6B.132: this MUST NOT be called for vehicle-without-motion.
    ...
    def test_non_vehicle_event_skips_injection(self, base_ctx, monkeypatch):
        ...
        ctx = AlertContext(
            ...
            # Phase 6B.168: CAM3 is intentionally outside the gatekeeper
...
class TestCompositeTelegram:
    ...
    def test_composite_fires_for_vehicle_with_motion(
        ...
        # Phase 6B.115: send_composite_alert takes bbox_a, bbox_b, trajectory
    ...
    def test_composite_fires_in_emit_result(self, vehicle_ctx, monkeypatch):
        ...
        Phase 6B.112: notify() was removed from emit_result_stage. TG#1
        (arriving) now fires from identify_stage end. TG#2 (composite)
        fires here in emit_result, BEFORE the match loop (TG#3) per maintainer
