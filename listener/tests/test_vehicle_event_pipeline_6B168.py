"""
test_vehicle_event_pipeline_6B168.py — Phase 6B.168 regression test.

Bug fixed: match_stage() (and 4 sibling gatekeeper checks) compared
ctx.camera_name (the friendly webhook label, e.g. "Outside Front Solar")
...
matcher never ran. Live symptom: drove F150 in/out/in over 60 minutes
and got zero vehicle alerts — six detected events, six L0 fallbacks
(alert ids 22aea5bb, c12ad03e, b85fb18a, 6f098a21, + 2 more) on
2026-08-31.

Fix: AlertContext.camera_code is populated once at the listener driver
boundary (listener.py:driver) via _code_for_camera(camera_name), and
all 5 gatekeeper membership checks in this module compare camera_code
(not camera_name) against gatekeeper_cameras.
...
This test pins the new contract:
  ...
  2. match_stage uses camera_code, not camera_name, for the gatekeeper
     membership test — and the new "is not a gatekeeper" log surfaces
...
class TestAlertContextCameraCode:
    ...
    def test_camera_code_default_is_empty_string(self):
        """Phase 6B.168: when camera_code is not explicitly set, the
    ...
    def test_camera_code_not_derived_without_post_init(self):
        """Phase 6B.168 (final): AlertContext deliberately does NOT
        ...
        (test_vehicle_event_pipeline_6B105b.non_gatekeeper_ctx).
        """
        ctx = AlertContext(
            alert_id="x",
...
class TestVisionQueueEligibilitySets:
    ...
    def test_gatekeeper_cameras_contains_cam5(self):
        ...
        assert "CAM5" in GATEKEEPER_CAMERAS, (
            "Phase 6B.168: GATEKEEPER_CAMERAS must contain CAM5 under "
            "the NEW schema. Pre-fix it contained only the legacy "
            "code 'OUTSIDE_FRONT_SOLAR', so submit()'s membership test "
            f"never matched. Actual: {sorted(GATEKEEPER_CAMERAS)}"
        )
    ...
    def test_gatekeeper_cameras_no_legacy_friendly_codes(self):
        ...
        for legacy in ("OUTSIDE_FRONT_SOLAR", "FRONT", "BACK"):
            assert legacy not in GATEKEEPER_CAMERAS, (
                f"Phase 6B.168: legacy friendly code {legacy!r} leaked "
    ...
    def test_gatekeeper_and_listener_sets_match(self):
        ...
        assert L == Q, (
            ...
            "These must stay in sync — see Phase 6B.167 follow-up."
        )
