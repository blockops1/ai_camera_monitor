"""Tests for the Phase.144 (§11.66) changes:
  ...
  - the gate writes a pairwise_diff.jpg at the output dir
...
def test_pairwise_diff_writes_jpeg(tmp_path):
    """_write_pairwise_diff_image produces a JPEG at output_dir/pairwise_diff.jpg."""
    ...
    p = Path(out)
    assert p.exists()
    assert p.name == "pairwise_diff.jpg"
    assert p.parent == tmp_path
    assert p.stat().st_size > 500  # not a blank/empty JPEG
...
def test_vision_prompt_describes_three_images():
    ...
    prompt = render_crop_prompt("CAM1", "2026-08-27 12:00:00 EDT")
