...
Phase.108a-rev1 (§11.38.13): the motion gate now runs ABOVE the person-branch
...
def _reset_gate_cooldown_state():
    """Reset infra.gate_cooldown state between tests (Phase.154 / §11.77).
...
def _call_process_alert(
    ...
    with patch(
        ...
        try:
            with patch("vehicle_event_pipeline.process_alert") as mock_vehicle:
                L._process_alert(
            ...
            with patch("listener.vehicle_event_pipeline.process_alert") as mock_vehicle:
                L._process_alert(
                    alert_id="test-alert-id",
                    camera_name=camera_name,
                    timestamp="2026-08-23T11:30:00-04:00",
...
def test_gate_suppress_short_circuits_person_event_on_CAM5(monkeypatch, gate_suppress_verdict):
...
def test_gate_suppress_short_circuits_vehicle_event_on_CAM5(monkeypatch, gate_suppress_verdict):
...
def test_gate_person_verdict_on_CAM5_routes_to_person_pipeline(monkeypatch, gate_person_verdict):
    ...
    Pre-§11.80 (Phase.140 → §11.80 gap): CAM1 was class_disabled /
    off-person-gatekeeper, so this test asserted vehicle routing. §11.80
    re-promotes CAM1 to the person pipeline per Note 2026-08-28.
    """
    monkeypatch.setenv("MOTION_GATE_ENABLED", "1")
    result = _call_process_alert(
        monkeypatch,
...
def test_gate_vehicle_verdict_on_CAM5_routes_to_vehicle_pipeline(monkeypatch, gate_vehicle_verdict):
...
def test_gate_vehicle_verdict_on_non_CAM5_routes_to_vehicle_pipeline(monkeypatch, gate_vehicle_verdict):
...
def test_gate_disabled_non_CAM5_routes_to_vehicle(monkeypatch, gate_vehicle_verdict):
...
# only. Note 2026-08-28: "make every camera a person gatekeeper
...
def test_CAM3_person_event_routes_to_queue_person(monkeypatch):
...
# ---------------------------------------------------------------------------
# Phase.129a (§11.51) — event promotion logic (gate verdict overrides
...
def test_md_event_promoted_to_vehicle_when_gate_says_vehicle(
    ...
    """Phase.129a (§11.51) — Reolink's `type=md` should become 'vehicle'
    ...
    an identification. (Phase.132 §11.54 later deleted the vehicle
...
# ---------------------------------------------------------------------------
# Phase.145 (§11.67) — promote `event=md` to 'people' when the gate's
...
def test_md_event_promoted_to_person_when_gate_says_person_on_CAM2(
    ...
    """Phase.145 (§11.67) — event=md on CAM3 + gate=person → person pipeline.
...
def test_motion_event_promoted_to_person_when_gate_says_person_on_CAM2(
    ...
    """Phase.145 — same as above but with `event=motion` (legacy alias).
...
def test_md_event_NOT_promoted_to_person_when_camera_not_person_gatekeeper(
    ...
    test_gate_person_verdict_on_CAM5_routes_to_person_pipeline). This
...
def test_md_event_NOT_promoted_when_gate_disabled(monkeypatch):
    """Phase.145 — without a gate verdict, no promotion happens.
...
def test_md_event_NOT_promoted_when_gate_says_vehicle(monkeypatch, gate_vehicle_verdict):
    """Phase.145 — vehicle promotion (6B.129a) takes precedence over
...
def test_person_promotion_logs_decision(monkeypatch, gate_person_verdict, caplog):
    """Phase.145 — when the person promotion fires, the log line
...
def test_vehicle_event_short_circuits_when_gate_suppresses(monkeypatch, gate_suppress_verdict):
    ...
    (no vehicle_event_pipeline.process_alert call at all) — regardless of
...
def gate_person_suppressed_vehicle_event_verdict():
    ...
    the listener routes to vehicle_event_pipeline (Phase.161, 2026-08-28).
    """
    from listener.motion_gate_pipeline import GateVerdict
    return GateVerdict(
        decision="suppress",
...
def test_gate_person_suppression_on_vehicle_event_routes_to_vehicle_pipeline_6B161(
    ...
    """Regression for Bug 1 (Phase.161, 2026-08-28):
    ...
    Before this fix (Phase.161), the Tesla drive-out at 17:36:31 (CAM5 alert
    ...
    assert mock_vehicle.called, (
        ...
        "expected vehicle_event_pipeline.process_alert to be called."
    )
