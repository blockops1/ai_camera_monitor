"""Phase.175 tests — two independent per-frame subject bboxes.
...
# ---------------------------------------------------------------------------
# 1. GateVerdict exposes subject_bbox_a + subject_bbox_b (Phase.175)
# ---------------------------------------------------------------------------


def test_gateverdict_has_two_subject_bbox_fields(tmp_path, monkeypatch):
    """Phase.175: GateVerdict exposes TWO subject bboxes —
...
def _find_morning_frames() -> dict[str, list[Path]]:
    ...
    candidates = [
        ...
        Path.home() / "<legacy-repo>-data" / "frames",
        Path("/data/frames"),
    ]
    alert_ids = [
