"""
test_preview_endpoint.py — Tests for the Phase.124 /preview endpoint.

The /preview route locates <nas> preview thumbnails by camera +
timestamp. Unlike /snapshot (which uses live RTSP frames), /preview
reads pre-existing files from the SMB-mounted <nas> share. We
monkeypatch `infra.<nas>_preview.NAS_ROOT` to a temporary
...
from infra import <nas>_preview as sp
...
def fake_<nas>(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
    """Build a fake <nas> tree with one camera, one session, a few files.
...
class TestPreviewEndpoint:
    ...
    def test_preview_404_when_no_preview_found(
        ...
        # Monkeypatch NAS_ROOT to a non-existent path
        monkeypatch.setattr(sp, "NAS_ROOT", "/nonexistent/path")
        app = _make_app()
        resp = app.test_client().get(
