"""
test_snapshot_endpoint.py — Tests for the Phase.88 / PLAN §11.19
on-demand /snapshot endpoint.

the operator asks Jill in chat for "an CAM1 snapshot" (or CAM2). Jill curls
...
Test inventory (10 cases):
  1. test_resolve_snapshot_camera_maps_CAM1_to_canonical
  2. test_resolve_snapshot_camera_maps_CAM2_to_canonical
...
def _make_cameras():
    ...
    Phase.167 §13.4 Commit 17 (T3 C17): cameras dict is keyed by
    ...
    """
    return {
        "CAM3": {"ip": "10.0.0.3"},   # → CAM2 (synthetic)
        "CAM4": {"ip": "10.0.0.4"},   # → CAM3  (synthetic)
        "CAM5": {"ip": "10.0.0.5"},   # → CAM1  (synthetic)
    }
...
def test_resolve_snapshot_camera_maps_CAM1_to_canonical():
    """Phase.167 §13.4 Commit 17: _SNAPSHOT_CAMERA_ALIASES now maps
    operator shorthand to CAM{N} codes, not operator-flavored names."""
    assert _resolve_snapshot_camera("CAM1", _make_cameras()) == "CAM5"


def test_resolve_snapshot_camera_maps_CAM2_to_canonical():
    """Phase.167 §13.4 Commit 17: CAM2 → CAM3 (CAM2)."""
    assert _resolve_snapshot_camera("CAM2", _make_cameras()) == "CAM3"


def test_resolve_snapshot_camera_passes_through_canonical():
    """Phase.167 §13.4 Commit 17: cameras dict is CAM{N}-keyed —
...
def test_resolve_snapshot_camera_returns_None_for_unknown():
    cams = _make_cameras()
    assert _resolve_snapshot_camera("XYZ", cams) is None  # unknown alias — §11.80 added CAM3/BACK/CAM5/CAM4 to the alias map
...
def test_snapshot_400_when_camera_missing():
    ...
    assert "CAM1" in body["known_aliases"]
    assert "CAM2" in body["known_aliases"]
...
def test_snapshot_503_when_reader_not_registered():
    ...
    try:
        client = app.test_client()
        resp = client.get("/snapshot?camera=CAM1")
...
def test_snapshot_400_when_max_size_malformed():
    ...
    for bad in ("notasize", "100", "100x", "x100", "-10x10", "abcxdef"):
        resp = client.get(f"/snapshot?camera=CAM1&max_size={bad}")
...
def test_snapshot_200_serves_jpeg_with_correct_headers_and_max_size():
    ...
    try:
        ...
        captured_kwargs.clear()
        resp = client.get("/snapshot?camera=CAM2&max_size=1280x720")
        assert resp.status_code == 200
        assert resp.mimetype == "image/jpeg"
...
# -----------------------------------------------------------------------------
# T3 C19 (Phase.167 §13.5 follow-on): listener must call
...
# /snapshot?camera=CAM1 returns 503 even though _resolve_snapshot_camera
# correctly mapped CAM1→CAM5.
# -----------------------------------------------------------------------------
