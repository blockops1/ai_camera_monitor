"""
test_vehicle_event_pipeline_6B112.py — Phase.112 integration tests.

Verifies the 3-Telegram CAM5 message stack (per the operator's spec 2026-08-21):
  ...
  TG#3 = match/no-match + 3-crop vertical composite photo — fires from
         emit_result_stage per vehicle AFTER TG#2 (per Note
...
# Phase.112 (2026-08-21): The fix in match_telegram.py /
...
class TestVisionSummaryStr:
    def test_multi_vehicle_schema(self):
        """Phase.130 (§11.53): all vehicles joined with ', plus ' — primary
    ...
    def test_multi_vehicle_primary_first_when_not_index_0(self):
        """Phase.130 (§11.53): when primary_vehicle_index points to a
    ...
    def test_multi_vehicle_legacy_no_primary_index(self):
        """Phase.130 (§11.53): backward compat — when primary_vehicle_index
    ...
    def test_multi_vehicle_drops_empty_identifications(self):
        """Phase.130 (§11.53): a vehicle entry with no usable fields is
...
class TestMatchStageImportResilience:
    ...
    Bug history:
      - Phase.113: changed `from vehicle_matcher.matcher import` to bare
      ...
      - Phase.114: reverted to explicit `from vehicle_matcher.matcher import`.
      ...
      - Phase.116: moved the `from vehicle_matcher.matcher import` to
...
# --- Test 5: _extract_signature helper (Phase.129b §11.52) ---------------

class TestExtractSignature:
    """Phase.129b (§11.52) — _extract_signature now reads from vehicles[]
