...
Phase 6B.115 (2026-08-25): the legacy fallback path is REMOVED. The
...
def gate_verdict_with_frames(tmp_path):
    ...
    Phase 6B.115 (§11.46.6): the verdict carries frames as PIL.Image,
...
def gate_verdict_with_person_frames(tmp_path):
    """GateVerdict for the person path (Phase 6B.139, §11.60).
...
def test_vehicle_capture_fast_path_uses_gate_frames(
    ...
    Phase 6B.115 (§11.46.6): the hot path reads PIL.Image objects from
...
# =============================================================================
# Phase 6B.139 (§11.60) — person path gate-aware capture tests
# =============================================================================
#
...
def person_ctx(tmp_path):
    ...
    Phase 6B.140 (2026-08-27): camera_name updated to CAM3.
    """
...
def test_person_capture_writes_pil_when_verdict_paths_empty(
    ...
    gate_aware_person_capture must write the 2 selected PIL frames to
    ctx.output_dir as frame_gate_001.png and frame_gate_002.png (§11.88
    2026-09-01: PNG lossless, NOT JPEG q85).
    """
    ...
    assert len(person_ctx.frame_paths) == 2
    assert _Path(person_ctx.frame_paths[0]).name == "frame_gate_001.png"
    assert _Path(person_ctx.frame_paths[1]).name == "frame_gate_002.png"
    # Files exist on disk
