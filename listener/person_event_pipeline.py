"""
person_event_pipeline.py — Phase 6B.139 (2026-08-27) — slim person-event pipeline.
...
The 4 stages (in order):
  1. person_capture_stage   — DEPRECATED stub (Phase 6B.139). The motion
  ...
  4. person_emit_stage      — builds structured Telegram body; sends body
                              ...
                              excluded since Phase 6B.153 / 2026-08-28);
...
STATUS: provisional (Phase 6B.139; will stabilize after live telemetry)
...
OUTPUTS:
    - return value from process_person_event: dict  (alert_id,
      ...
      gate=person but Qwen=no_person_in_frame — Phase 6B.162)
    ...
    - Structured log lines (Phase 6B.164): vision_attrs (full
...
PUBLIC API:
    process_person_event(ctx: PersonContext) -> dict
        ...
        Stage 1 calls gate_aware_person_capture (Phase 6B.139).
...
DOES NOT DO:
    ...
    - Capture frames from RTSP — Phase 6B.139: that responsibility was
...
WHY HERE:
    Mirrors vehicle_event_pipeline's context-object pattern per maintainer's
...
CALLS INTO:
    - listener._gate_aware_capture: gate_aware_person_capture (Phase 6B.139 —
...
RELATED:
    ...
    - PLAN.md §11.60 — Phase 6B.139 (this commit): gate-aware person capture
    - PLAN.md §11.61 — Phase 6B.140 (2026-08-27): person-gatekeeper swap
    - PLAN.md §11.64 — Phase 6B.141 (2026-08-27): 6-image Telegram media group (album)
    - PLAN.md §11.65 — Phase 6B.143 (2026-08-27): person-gatekeeper joins pre-event trail capture
    - PLAN.md §11.76 — Phase 6B.153 (2026-08-28): crops-only (2 images) — wide frames excluded
...
# Phase 6B.167 §13.5 Commit 15: resolve operator-flavored friendly
...
# Phase 6B.153 (2026-08-28) — person-emit sends a 2-image Telegram
# media group: just the 2 YOLO crops (frame_NNN_crop*.jpg). Wide
# frames excluded by request ("I want to send few images"). Phase
# 6B.141 (2026-08-27) previously sent 4 wide + 2 crops; Phase 6B.153
...
def _collect_person_album_paths(output_dir: str) -> list[str]:
    ...
    Wide frames are intentionally excluded. Phase 6B.153
    (2026-08-28, maintainer OOB): "I think we are going to get a lot
    ...
    Phase 6B.141 (2026-08-27): prior version sent 4 wide frames
    + 2 crops. Phase 6B.153 reduced to crops-only.
    """
...
class PersonContext:
    ...
    State populated by stages:
        ...
        selected_frames: list[PIL.Image.Image] — 6B.139: 2-frame subset
            ...
            Phase 6B.140 best-frame selection.
    ...
    # --- Phase 6B.139 (§11.60): gate-aware capture inputs ---
    ...
    # (Phase 6B.115: legacy_capture_avoided was REMOVED — see vehicle path.
    #  Phase 6B.139: the 6-second-late fresh RTSP pull is REMOVED —
    ...
    # --- Phase §11.115.9: cascade-result inputs (set by matcher_adapters) ---
    # §11.115 cascade produces classify + call2_response. person_identify
    # consumes call2_response (better_crop + attributes + signature) and
    # selects the right crop (crop_a/crop_b) for ArcFace. The person
    # pipeline DOES NOT call Qwen when call2_response is provided.
    classify: Any = None  # ClassifyResult (label + confidence)
    call2_response: dict | None = None  # {better_crop, attributes, signature, confidence, notable_details}
    crop_a_path: str = ""  # disk path to crop_a (for ArcFace)
    crop_b_path: str = ""  # disk path to crop_b (for ArcFace)
    ...
    # §11.115.9: vision_result is now populated from ctx.call2_response
    # (NOT from a Qwen call inside the person pipeline). Old schema had
    # {"persons": [...], "primary_person_index": ...}; new schema has
    # {"better_crop": ..., "attributes": ..., "signature": ...}.
    # Existing matcher code reads from vision_result; we map into the
    # new shape so downstream stages (match, emit) work unchanged.
...
def person_capture_stage(ctx: PersonContext) -> None:
    ...
    Phase 6B.139 (§11.60, 2026-08-27): the motion gate is now the sole
    ...
    log.warning(
        ...
        f"(Phase 6B.139) — gate is sole frame producer. Returning with no frames."
    )
    ctx.frame_paths = []
...
def person_identify_stage(ctx: PersonContext) -> None:
    """§11.115.9: consume the cascade's call2_response.

    Two paths:
      A. CASCADE PATH (preferred): ctx.call2_response is set by
         matcher_adapters.person() — the shared §11.115 cascade already
         ran call 1 (shared classify) and call 2 (person-specific
         schema with better_crop selection). We do NOT call Qwen here;
         we copy the call2_response into ctx.vision_result in the same
         shape downstream matcher/emit code expects, then run ArcFace
         on the crop chosen by Qwen's `better_crop` (crop_a | crop_b).

      B. FALLBACK PATH (legacy): ctx.call2_response is None — run the
         original Qwen person call + use the old `face_visible` field
         + fallback to frame_paths[0]. Preserved for callers that
         bypass the cascade.
    """
    if ctx.call2_response is not None:
        _person_identify_cascade_path(ctx)
    else:
        _person_identify_legacy_path(ctx)


def _person_identify_cascade_path(ctx: PersonContext) -> None:
    """Cascade path: consume call2_response, run ArcFace on chosen crop.

    The shared cascade has already invoked Qwen once for each alert.
    We translate the cascade's call2_response into the same
    vision_result shape downstream code expects, then run ArcFace on
    the crop the call2_response pointed at.

    Vision result mapping (cascade → matcher/emit shape):
      call2_response.better_crop         → vision_result.better_crop
      call2_response.attributes          → vision_result.attributes
      call2_response.signature           → vision_result.signature
      call2_response.confidence          → vision_result.confidence
      call2_response.notable_details     → vision_result.notable_details
      call2_response                     → vision_result.call2_response
      No "persons"[] list (single-person schema in §11.115).
    """
    call2 = ctx.call2_response
    assert isinstance(call2, dict), (
        f"[{ctx.alert_id}] call2_response must be a dict, got {type(call2).__name__}"
    )
    ctx.vision_result = {
        "better_crop": call2.get("better_crop"),
        "attributes": call2.get("attributes"),
        "signature": call2.get("signature"),
        "confidence": call2.get("confidence"),
        "notable_details": call2.get("notable_details"),
        "call2_response": call2,
        "_source": "cascade",
    }
    _log_call2_attrs(ctx, call2)

    better_crop = call2.get("better_crop")
    if better_crop == "crop_a" and ctx.crop_a_path:
        ctx.face_recognition = _run_face_recognition_on_path(
            ctx, ctx.crop_a_path, reason="better_crop=crop_a"
        )
    elif better_crop == "crop_b" and ctx.crop_b_path:
        ctx.face_recognition = _run_face_recognition_on_path(
            ctx, ctx.crop_b_path, reason="better_crop=crop_b"
        )
    else:
        # "neither" → no face visible, OR a crop path is missing.
        # Log and skip ArcFace. The matcher falls back to tier-3
        # (signature/attributes) scoring.
        ctx.face_recognition = None
        log.info(
            f"[{ctx.alert_id}] person_identify: better_crop={better_crop!r} → "
            f"skipping ArcFace (no face visible or crop path missing)"
        )


def _person_identify_legacy_path(ctx: PersonContext) -> None:
    """Legacy path: call Qwen directly (used when matcher_adapters
    isn't routed through, or for standalone tests).
    """
    from infra.vision_analyzer import analyze_frames_queued

    log.info(
        f"[{ctx.alert_id}] person_identify (legacy): calling Qwen mode=person "
    ...
    _log_vision_attrs(ctx, ctx.vision_result)
...
def _log_call2_attrs(ctx: PersonContext, call2: dict) -> None:
    """Emit a single structured log line for the cascade call2 response.

    Mirrors _log_vision_attrs but reads from the new §11.115 schema.
    """
    attrs = call2.get("attributes") or {}
    if not isinstance(attrs, dict):
        attrs = {}
    sil = attrs.get("silhouette") or {}
    hair = attrs.get("hair") or {}
    parts = [
        f"better_crop={call2.get('better_crop')}",
        f"conf={call2.get('confidence')}",
        f"upper.color={((attrs.get('clothing_upper') or {}).get('color'))}",
        f"upper.type={((attrs.get('clothing_upper') or {}).get('type'))}",
        f"lower.color={((attrs.get('clothing_lower') or {}).get('color'))}",
        f"lower.type={((attrs.get('clothing_lower') or {}).get('type'))}",
        ...
        f"skin_tone={attrs.get('skin_tone')}",
        f"age_range={attrs.get('age_range')}",
        ...
        f"facial_hair={attrs.get('facial_hair')}",
        f"glasses={attrs.get('glasses')}",
        f"stable={((call2.get('signature') or {}).get('stable'))}",
    ]
    log.info(f"[{ctx.alert_id}] call2_attrs: {' '.join(parts)}")
...
def _log_vision_attrs(ctx: PersonContext, vision_result: dict[str, Any]) -> None:
    ...
    Phase 6B.164 (2026-08-29): added so the matcher scoring becomes
...
def _run_face_recognition_on_path(
    ctx: PersonContext,
    crop_path: str,
    *,
    reason: str = "better_crop",
) -> dict | None:
    """§11.115.9: run ArcFace directly on a chosen crop file.

    Unlike the legacy `_run_face_recognition` (which uses Qwen's
    face_bbox to crop from frame_paths[0]), this version assumes the
    crop is already a face-bearing rectangle — the Qwen-selected
    crop_a or crop_b from the cascade. ArcFace's on-device detector
    (RetinaFace from buffalo_l) finds faces inside the crop; if any
    is found, identify. No bbox mapping needed.

    Returns the recognize_faces dict (with `faces`, `identified_person`,
    `best_confidence`) or None on failure / no face found.
    """
    try:
    ...
    if not crop_path:
        log.warning(
            f"[{ctx.alert_id}] person_identify: empty crop_path for {reason}"
        )
        return None

    try:
        result = recognize_faces(crop_path)
        n_faces = len(result.get("faces") or [])
        log.info(
            f"[{ctx.alert_id}] person_identify: ArcFace on {reason} "
            f"({crop_path}) found {n_faces} face(s), "
            f"identified={result.get('identified_person')!r} "
            f"conf={result.get('best_confidence')}"
        )
        # If no faces detected on the chosen crop, log it but still
        # return the dict — the matcher uses identified_person which
        # will be None. Lowering this to None changes downstream
        # "did ArcFace run?" semantics, which _log_call2_attrs would
        # not see. Returning an empty-faces dict is the most
        # information-preserving move.
        return result
    except Exception:
        log.exception(
            f"[{ctx.alert_id}] person_identify: ArcFace on {reason} raised"
        )
        return None


def _run_face_recognition(ctx: PersonContext, person: dict) -> dict | None:
    """LEGACY: Run ArcFace recognition on the primary frame using
    Qwen's face_bbox. Used by the legacy path only. The new cascade
    path uses _run_face_recognition_on_path instead, which avoids the
    bbox mapping by passing an already-cropped file path.
...
def person_match_stage(ctx: PersonContext) -> None:
    ...
    if isinstance(result, MatchVerdict):
        ...
        # Phase 6B.164: also log best_candidate_confidence so we can see
...
def person_emit_stage(ctx: PersonContext) -> dict:
    ...
    # per Phase 6B.153 / 2026-08-28, maintainer OOB: "I think we are going
    ...
    # Phase 6B.141 (2026-08-27, maintainer OOB): separate text + album
...
def _result_dict(ctx: PersonContext) -> dict:
    ...
    return {
        ...
        "telegram_sent": ctx.telegram_sent,
        "telegram_error": ctx.telegram_error,
        # Phase 6B.162 — suppression tracking (for early-return path).
...
def process_person_event(ctx: PersonContext) -> dict:
    ...
    Phase 6B.139 (§11.60, 2026-08-27): Stage 1 (capture) is now
    ...
    maintainer 2026-08-27: *"I'd like it to work more like the vehicle
    pipeline does."* Phase 6B.139 implements that alignment.
    """
    log.info(
    ...
    try:
        ...
        # Phase 6B.139: gate didn't produce frames; no legacy fallback.
    ...
    # Phase 6B.162: suppress Telegram if gate says person but Qwen says
