...
PERSON_GATEKEEPER_CAMERAS (currently Outside Front Garage, §11.61 / 6B.140).
...
DOES NOT DO:
    ...
    - Re-prompt for height / bbox ratio — parked to §11.36b (OFG).
...
RELATED:
    ...
    - PLAN.md §11.65 — Phase.143 (2026-08-27): OFG joins pre-event trail capture
...
class PersonContext:
    ...
    Attributes:
        ...
        camera_name: Human-readable camera label (e.g. "Front Door Outside").
...
def person_emit_stage(ctx: PersonContext) -> dict:
    ...
    try:
        ...
        if album_paths:
            ...
            # happen for OFG since the gate is gatekeeper-tier, but
