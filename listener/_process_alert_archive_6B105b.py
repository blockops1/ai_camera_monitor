...
def _process_alert(
    ...
    if is_vehicle_event:
        # Single RTSP session for all 6 frames. the operator 2026-08-02: the
        ...
        # problem.
        #
        # 2026-08-06 — gatekeeper (CAM1) override: instead of the trailing
        # tail, sample 6 frames spanning T-12s through T+0s of camera time
        # at 15fps (indices [0, 30, 60, 90, 120, 150]). the operator wants to
    ...
    with acquire_for_camera(camera_name):
            ...
            if motion_result is not None and not motion_result.no_motion_detected:
                ...
                if crop_paths:
                    ...
                    try:
                        ...
                        # 15:08 EDT CAM2: Tesla Model Y blue conf=0.98
        ...
        # Only CAM1 (the gatekeeper camera) flips state;
        ...
        try:
            ...
            # the operator's directive: no state machine, no tracking of what's
            ...
            # alert (below). Lead Telegram = "Vehicle motion at CAM1" with
            # the picture. Match Telegram = "🎯 Matched vehicle at CAM1"
            ...
            def _send_match_alert(
                ...
                Sends a 2nd Telegram when the CAM1 motion alert's vision
                ...
                msg = "\n".join(msg_lines)
                log.info(
                    f"[{alert_id}] CAM1 match alert: kv={kv.get('id')!r} "
                ...
                if bot_token and chat_id:
                    ...
                    log.warning(
                        f"[{alert_id}] CAM1 match alert NOT sent (no telegram creds)"
                    )
            ...
            def _send_no_match_alert(
                ...
                Message format:
                    ❌ <b>No match for vehicle at CAM1</b>
                       14:08 EDT
                       ...
                       Top 3 candidates:
                         ...
                         3. v_carson_white ([name two]'s pickup pickup): score=2.20
                ...
                msg = "\n".join(msg_lines)
                log.info(
                    f"[{alert_id}] CAM1 no-match alert: top3={[t[0].get('id') for t in top3]} "
                ...
                if bot_token and chat_id:
                    ...
                    log.warning(
                        f"[{alert_id}] CAM1 no-match alert NOT sent (no telegram creds)"
                    )
            ...
            def _send_motion_alert(
                ...
                Sends one Telegram message per CAM1 vehicle alert whose
                vision result enumerated at least one VEHICLE IN MOTION.
                Criterion (the operator 2026-08-03): vision's per-vehicle
                ...
                The 4th frame is well-defined for the CAM1 6-frame burst
                (the operator 2026-08-18: "just the fourth frame").
                ...
                log.info(
                    f"[{alert_id}] CAM1 motion alert (6B.89 minimal): "
                ...
                if photo_ok:
                    ...
                    log.warning(
                        f"[{alert_id}] CAM1 motion alert NOT sent (no telegram creds or send failed)"
                    )
            ...
            # the operator: vehicle state (arrival/departure) is no longer the
            ...
            # from CAM1-only to all GATEKEEPER_CAMERAS. CAM2 joined the
            # gatekeeper tier in 6B.87 (persistent RTSP + motion detector +
            # 3-crop vision pipeline) but the match-alert loop at
            # line ~3639 was still hard-coded to CAM1, dropping the
            # 2nd-Telegram (and the no-match 3rd Telegram) for every
            # CAM2 vehicle event. Verified on alert 0eefa8e9 (Tesla
            ...
            # match loop never ran because camera_name != CAM1.
            # Symptom: zero 2nd-Telegrams, zero no-match Telegrams for
            # CAM2 vehicle events, even when the matcher would have
            # cleared v_owner1_darkblue_tesla_y. Fixed by switching the
            # gate to `camera_name in GATEKEEPER_CAMERAS:` so CAM1 + CAM2
            # both got the lead motion + match + no-match Telegram stack.
            #
            # Phase.104 (2026-08-20) — CAM2 demoted from gatekeeper
            # tier per the operator's request that CAM2 behave like the other
            # cameras. The gate below still says `camera_name in
            # GATEKEEPER_CAMERAS:` but the set is now CAM1-only, so CAM2
            ...
            if camera_name in GATEKEEPER_CAMERAS:
                ...
                if vehicles_in_scene:
                    ...
                    # motion across the 6 frames. the operator's principle
                    ...
                    # the gravel road but the CAM1 gate said no motion.
                    ...
                    log.info(
                        f"[{alert_id}] CAM1 motion gate (6B.71): "
                    ...
                    for _pv in vehicles_in_scene:
                        ...
                        if _prose != _field_moving:
                            log.warning(
                                f"[{alert_id}] CAM1 motion disagreement: "
                    ...
                    if moving_vehicles:
                        ...
                        if _cooldown_key and _MOTION_COOLDOWN.is_cool(_cooldown_key):
                            log.info(
                                f"[{alert_id}] CAM1 motion cooldown: "
                            ...
                            # Telegram in the CAM1 message stack
                        ...
                        # the operator: "What I want is for the matcher to run after
                        ...
                        # scope as variables in the CAM1/CAM2 alert handler. The
                        ...
                        try:
                            ...
                            if _failure_count >= 2:
                                log.error(
                                    f"[{alert_id}] CAM1 match path failed "
                                    f"({_failure_count}x in 5min): {_match_exc!r}"
                                )
                            else:
                                log.warning(
                                    f"[{alert_id}] CAM1 match path failed: {_match_exc!r}"
                                )
                    else:
                        log.info(
                            f"[{alert_id}] CAM1 motion alert SKIPPED: "
                    ...
                    # scene themselves. the operator's ask: "send me whatever
                    ...
                    if _vision_failed and frame_paths:
                        log.warning(
                            f"[{alert_id}] CAM1 motion alert SKIPPED: vision "
                        ...
                        try:
                            ...
                            _fallback_caption = (
                                f"⚠️ [CAM1_VISION_FAILED] {camera_name} vehicle "
                            ...
                            log.exception(
                                f"[{alert_id}] CAM1 vision-failed fallback send failed"
                            )
                    else:
                        log.info(
                            f"[{alert_id}] CAM1 motion alert SKIPPED: "
            ...
            if not VEHICLE_ESCALATION_ENABLED:
                ...
                for evt in vehicle_events:
                    ...
                    try:
                        ...
                        # Phase.10 — the operator: "I prefer my notifications not to
