...
RELATED:
    ...
    - PLAN.md §11.61 — Phase.140 (2026-08-27) CAM5 → CAM2 person-gatekeeper swap
...
# so spec.code is the legacy prefix (e.g. "CAM1"). When
...
GATEKEEPER_CAMERAS = frozenset({
    ...
    "CAM3",   # → CAM2
    "CAM4",   # → CAM3
    "CAM5",   # → CAM1 (original gatekeeper §11.79)
    "CAM6",   # → CAM4
})
...
def _zone_for(camera_code: str, env_path: str | None = None) -> str:
    ...
    Args:
        camera_code: A CameraSpec.code value (today: legacy prefix like
            "CAM1"; future: CAM{N} after §13.4 migration).
...
# the operator — "I just want [it] to be like the other cameras. Not a vehicle
...
#
# Tuned 4s → 6s on 2026-08-07 per the operator: "I really like the four second
...
DISABLED_CAMERA_EVENTS: frozenset[tuple[str, str]] = frozenset({
    ...
    # CAM5 = CAM1 (the original gatekeeper).
    ("CAM5", "animal"),
})
...
def _process_alert(
    ...
    # fall through to the existing camera-based routing (CAM5 person events still
    ...
    # camera-based routing. Per the operator 2026-08-23: keep gate + redundant pipeline
...
if __name__ == "__main__":
    ...
    # Phase.150: heartbeat thread removed (the operator 2026-08-28:
