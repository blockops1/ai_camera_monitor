...
# Phase.104 (2026-08-20) — one camera was demoted from gatekeeper tier.
...
# Phase.104 (2026-08-20) demoted that camera from the gatekeeper tier
# per Note: "I just want [it] to be like the other cameras. Not a
...
# ----------------------------------------------------------------------
# Camera registry helpers (Phase.167 §13.5 Commit 14)
# ----------------------------------------------------------------------
...
# GATEKEEPER_CAMERAS expansion. Note 2026-08-28: "make every camera
...
# Phase.167 §13.4 Commit 18 (T3 C18): alias dict is hydrated at
...
_SNAPSHOT_CAMERA_ALIASES_FALLBACK: dict[str, str] = {
    ...
    "OFS": "CAM5",   # Outside Front Solar
    "OFG": "CAM3",   # Outside Front Garage
    "BACK": "CAM2",  # Back Door Inside
    "FDO": "CAM1",   # Front Door Outside
    "OBS": "CAM6",   # Outside Back Solar
    "OFP": "CAM4",   # Outside Front Power
}
...
def _zone_for(camera_code: str, env_path: str | None = None) -> str:
    ...
    Phase.167 §13.5 Commit 14: zone-based routing for vehicle/person
...
def _resolve_snapshot_camera(arg: str, cameras: dict) -> str | None:
    ...
    Accepts:
      ...
      - shorthand from _SNAPSHOT_CAMERA_ALIASES (e.g. "OFS" → "CAM5")
...
# Origin: 2026-08-03 (one camera only). 2026-08-17 (Phase.87,
...
# Note — "I just want [it] to be like the other cameras. Not a vehicle
...
def _classify_queue(camera_name: str, event_type: str) -> str | None:
    ...
    # Phase.167 §13.5 Commit 14: GATEKEEPER_CAMERAS and
...
DISABLED_CAMERA_EVENTS: frozenset[tuple[str, str]] = frozenset({
    ...
    # effect. Per Note 2026-08-28: "make every camera a person
    ...
    # suppress the worst false positives for person events; the OFS
    # animal disable persists as a separate, narrower filter.
    #
    # Phase.167 §13.4 Commit 17 (T3 C17): camera half of each tuple
...
def create_app(test_config: dict | None = None) -> Flask:
    ...
    def receive_alert():
        ...
        # Validate IP matches (prevent spoofing). Phase.167 §13.5
        ...
        # Phase.167 §13.5 Commit 14: rtsp_url comes from the
...
def _process_alert(
    ...
    """
    Run the §11.115 single pipeline for one alert.

    Phase §11.115.6 (2026-09-02): replaced the 270-line legacy body
    (gate_cooldown + motion_gate dispatch + per-camera routing +
    event promotion + 3 structured pipelines + 1 catch-all) with a
    single call to listener.single_pipeline.run().

    The legacy code is preserved in
    listener/_process_alert_legacy_6B111_archive.py for archaeology.

    Stages (§11.115):
        1. RTSP-presence filter (cameras.has_rtsp) → drop if no RTSP.
        2. Capture 4 frames from RTSP (gate-aware capture).
        3. Pairwise diff (frame_2→3, frame_3→4) → crop_a, crop_b.
        4. YOLO pre-filter on crops (no new crops produced).
        5. Qwen call 1: shared classify (vehicle|person|animal|other).
        6. Qwen call 2: class-specific (per-class schema).
        7. Class-specific matcher via matcher_adapters.
        8. Telegram send with BOTH crops (crop_a + crop_b).

    Two-crop invariant: only crop_a + crop_b exist throughout. No
    YOLO crops, no Qwen bbox crops. Same 2 images from start to finish.

    Args:
        alert_id: UUID for this alert run.
        camera_name: Human-readable label.
        timestamp: ISO 8601 event timestamp.
        event: Event type ("motion", "person", "vehicle", "animal", ...).
        rtsp_url: Full RTSP URL for this camera.

    Returns:
        None. Side effects: Telegram send, log lines, audit row.
    """
    # Stage 0: import the single-pipeline entry point (dual-context).
    try:
        from listener.single_pipeline import run as pipeline_run
    except ImportError:  # pragma: no cover
        from single_pipeline import run as pipeline_run  # type: ignore
    try:
        from listener import matcher_adapters
    except ImportError:  # pragma: no cover
        import matcher_adapters  # type: ignore
    try:
        from infra.cameras import has_rtsp
    except ImportError:  # pragma: no cover
        from listener.infra.cameras import has_rtsp  # type: ignore
    try:
        from infra.vision_analyzer import analyze_frames_queued
    except ImportError:  # pragma: no cover
        from listener.infra.vision_analyzer import analyze_frames_queued  # type: ignore
    try:
        from infra.frame_diff import crop_frame_to_bbox, diff_pair_with_bbox
    except ImportError:  # pragma: no cover
        from listener.frame_diff import crop_frame_to_bbox, diff_pair_with_bbox  # type: ignore

    # Stage 0.5: motion gate (capture + classify). Phase §11.115.6
    # preserves the gate as the frame-capture + YOLO-classifier
    # mechanism; the cascade (call 1 + call 2) runs on top of its
    # frame_paths. Gate verdict is logged but does NOT short-circuit
    # the cascade (the cascade owns the new "drop if no RTSP" and
    # "log-only for 'other'" semantics).
    try:
        from _motion_gate_dispatch import maybe_run_motion_gate
    except ImportError:
        from listener._motion_gate_dispatch import maybe_run_motion_gate  # type: ignore
    ...
    if gate_verdict is not None and not gate_verdict.is_suppress:
        log.info(
            f"[{alert_id}] motion_gate: pass (decision={gate_verdict.decision} "
            f"class={gate_verdict.class_label} "
            f"conf={gate_verdict.confidence:.2f}) — feeding frames to single_pipeline"
        )

    # Resolve gate's frame_paths (may be empty if gate ran disabled).
    gate_frame_paths: list[str] = []
    if gate_verdict is not None and not gate_verdict.is_suppress:
        gate_frame_paths = list(getattr(gate_verdict, "frame_paths", []) or [])

    # Load Telegram creds once up front (the cascade uses them later).
    bot_token, chat_id = _load_telegram_creds()

    # ---------- frame_diff_fn closure (frame_paths → crop_a, crop_b) ----------
    def _frame_diff_fn(frame_paths: list[str]) -> tuple[str, str]:
        """Pairwise diff(frame_2,frame_3) + diff(frame_3,frame_4) →
        crop_a + crop_b.

        Returns (crop_a_path, crop_b_path). When diff produces no
        motion bbox, falls back to the source frame so the cascade
        can still run (rather than failing on None).
        """
        if len(frame_paths) < 3:
            # Need at least 2 frames for one diff. Use whatever we have.
            log.warning(
                f"[{alert_id}] _frame_diff_fn: only {len(frame_paths)} frames; "
                "using fallback crop selection"
            )
            idx_a = min(1, len(frame_paths) - 1)
            idx_b = min(2, len(frame_paths) - 1)
            return (frame_paths[idx_a], frame_paths[idx_b])

        idx_a = 1
        idx_b = 2
        if len(frame_paths) >= 4:
            idx_a = 1
            idx_b = 2  # use frames 2-3
            # second diff: frame_3→4 (idx_a+1=2, idx_a+2=3)
        else:
            idx_a, idx_b = 0, 1

        # Run pairwise diffs (frame_2→3 + frame_3→4 when available).
        bbox_a: tuple[int, int, int, int] | None = None
        bbox_b: tuple[int, int, int, int] | None = None
        try:
            bbox_a, _, _ = diff_pair_with_bbox(frame_paths[idx_a], frame_paths[idx_a + 1])
        except Exception as exc:  # pragma: no cover — defensive
            log.warning(f"[{alert_id}] _frame_diff_fn: diff(a) failed: {exc!r}")
        if len(frame_paths) >= 4:
            try:
                bbox_b, _, _ = diff_pair_with_bbox(frame_paths[idx_a + 1], frame_paths[idx_a + 2])
            except Exception as exc:  # pragma: no cover — defensive
                log.warning(f"[{alert_id}] _frame_diff_fn: diff(b) failed: {exc!r}")
            src_b = frame_paths[idx_a + 1]
        else:
            src_b = frame_paths[idx_b]

        crop_a_path: str | None = None
        crop_b_path: str | None = None
        try:
            if bbox_a is not None:
                crop_a_path = crop_frame_to_bbox(frame_paths[idx_a], bbox_a)
        except Exception as exc:  # pragma: no cover — defensive
            log.warning(f"[{alert_id}] _frame_diff_fn: crop_a failed: {exc!r}")
        try:
            if bbox_b is not None:
                crop_b_path = crop_frame_to_bbox(src_b, bbox_b)
        except Exception as exc:  # pragma: no cover — defensive
            log.warning(f"[{alert_id}] _frame_diff_fn: crop_b failed: {exc!r}")

        # Fallback to source frames when crop failed / no bbox.
        crop_a = crop_a_path if crop_a_path else frame_paths[idx_a]
        crop_b = crop_b_path if crop_b_path else src_b
        return (crop_a, crop_b)

    # ---------- qwen_fn closure (frame_paths → analyze_frames_queued) ----------
    def _qwen_fn(
        ...
        **_unused: Any,
        ...
        """Wrapper that adapts analyze_frames_queued's signature to
        the two_call_cascade's expected callable shape.

        analyze_frames_queued returns either the parsed result dict or
        an error-sentinel dict (per infra/vision_analyzer.py:432).
        We never get None — but be defensive just in case.
        """
        ...
        if result is None:  # pragma: no cover — defensive
            return {
                ...
                "error": "analyze_frames_queued returned None",
            }
        return result

    # ---------- matcher adapter (vehicle/person/animal) ----------
    def _camera_code_for(name: str) -> str:
        """Translate friendly camera name to CAM{N} code; pass-through otherwise.

        §11.115.21 OFS miss (2026-09-03): match_stage expects CAM{N} keys
        against ctx.gatekeeper_cameras. The previous implementation used
        infra.cameras.by_code(name).code, which raises KeyError on a
        friendly name (by_code indexes on spec.code only) and falls
        through to `return name`. When called with friendly name
        "Outside Front Solar", this returned "Outside Front Solar" —
        failing the gatekeeper membership check and silently skipping
        the vehicle match-alert Telegram path. Use code_for() which
        matches BOTH spec.code and spec.name (handles either input).
        """
        try:
            return _code_for_camera(name)
        except Exception:
            return name

    matchers = matcher_adapters.SinglePipelineMatchers(
        alert_frame_dir=ALERT_FRAME_DIR,
        camera_name=camera_name,
        timestamp=timestamp,
        rtsp_url=rtsp_url,
        ...
        camera_code_lookup=_camera_code_for,
        # Pass gate verdict so per-class matchers can reuse the gate's
        # 4 PIL frames via gate_aware_*_capture (per PLAN §11.46).
    ...
    def _telegram_no_op(
        ...
        crop_b: str,
        classify_label: Any,
        call2_response: dict[str, Any] | None,
        alert_id: str,
    ) -> bool:
        """Phase §11.115.6 cutover: per-class pipelines currently own
        their own Telegram sends via emit_result_stage / inline notify.
        `single_pipeline.run` would double-send if it called telegram_fn
        after the matcher. So for this cutover, telegram_fn is a no-op.
        The single-Telegram contract (one Telegram send with both crops)
        arrives in §11.115.9 (person) / §11.115.10 (animal) /
        §11.115.11 (vehicle) when each per-class pipeline strips its
        inline send and the pipeline becomes the sole sender.
        """
        return True

    # ---------- run the single pipeline ----------
    # §11.115.13: build a per-listener PipelineCooldown so the
    # property-wide 15-minute throttle has stable state across all
    # webhook events for this listener process. Cooldown is shared
    # across cameras (property-wide) per Note 2026-09-02 PM.
    try:
        from listener.pipeline_filters import PipelineCooldown
    except ImportError:  # pragma: no cover — dual-context
        from pipeline_filters import PipelineCooldown  # type: ignore
    pipeline_cooldown = PipelineCooldown()

    pipeline_run(
        alert_id=alert_id,
        camera_name=camera_name,
        camera_code=_camera_code_for(camera_name),
        captured_at=timestamp,
        frame_paths=gate_frame_paths,
        event_type=event,
        has_rtsp_fn=has_rtsp,
        frame_diff_fn=_frame_diff_fn,
        qwen_fn=_qwen_fn,
        matchers=matchers,
        telegram_fn=_telegram_no_op,
        log_fn=_log_with_context,
        cooldown=pipeline_cooldown,
        # §11.115.22 — thread the gate's pairwise_diff PNG through to the
        # Qwen cascade as the first image in the 3-image payload. This
        # gives the model spatial motion context so it can pick the
        # MOVING subject instead of enumerating every parked vehicle.
        pairwise_diff_path=getattr(gate_verdict, "pairwise_diff_path", None),
    )
...
if __name__ == "__main__":
    ...
    # (Phase.87). 2026-08-20 (Phase.104) — one camera was demoted
    ...
    # Phase.167 §13.5 (T3 C19 follow-on): seed the persistent_rtsp
    ...
    # Phase.167 §13.5 Commit 14: iterate over the registry, not a
