...
OUTPUTS:
    ...
    - data/audit_telegram/ JSONL audit lines (Phase.124)
    ...
    - Phase.154 (§11.77): cooldown-suppressed alerts logged but produce no
...
CALLS INTO:
    ...
    - infra.gate_cooldown.is_in_gate_cooldown (per-camera × per-event-type
      suppression at the gate; Phase.154 / §11.77. Runs BEFORE the gate.)
...
RELATED:
    ...
    - PLAN.md §11.51 — Phase.129a event promotion (md → vehicle)
    - PLAN.md §11.38 — Phase.106 refactor (this module's current shape)
    - PLAN.md §11.61 — Phase.140 (2026-08-27) FDO → OFG person-gatekeeper swap

Historical reference (Phase.81 — extracted in 6B.106):
...
or
    <install-path>/ai_camera_monitor/.venv/bin/python \\
        <install-path>/ai_camera_monitor/listener/listener.py
...
# <install-path>/ai_camera_monitor. Nothing in this file references
# <legacy-repo>/src/.
...
# Phase.45 / 6B.55 — sibling modules log under their own __name__
...
# Phase.16 — Per-class queues with vehicle priority.
...
# gatekeeper-camera (Outside Front Solar) vehicle events at the top.
# Phase.104 (2026-08-20) — OFG demoted from gatekeeper tier.
# OFG vehicle events no longer route to QUEUE_GATEKEEPER_VEHICLE, the
# capture-delay path, or the match-alert Telegram stack. OFG is now a
# normal camera for vehicle events — same as Front Door Outside, Outside
# Front Porch, Outside Back Solar, Outside Front Power. OFG keeps its
# persistent RTSP reader (still in the boot loop at line ~4133) because
# that's about reliable frame capture, not vehicle handling. If a future
# phase adds a person-gatekeeper tier, OFG may rejoin the gatekeeper set
# for person events only — that's deferred, see PLAN §11.32.
# Phase.104 (2026-08-20) demoted OFG from the gatekeeper tier per
# Note: "I just want OFG to be like the other cameras. Not a
# vehicle motion gatekeeper anymore." §11.79 (2026-08-28) reverted that
# decision after 3 vehicles entered the property from non-OFS cameras
# (OFP, OBS) and produced no Telegram alerts — only history.jsonl rows.
# Note 2026-08-28: "Let's do this for now and see how it works.
...
# (TG#1 arriving + TG#2 motion composite + TG#3 match/no-match). OFG
# keeps its persistent RTSP reader because that's about reliable frame
# capture, not vehicle handling. §11.77 gate_cooldown limits per-camera
...
GATEKEEPER_CAMERAS = frozenset({
    "Outside Front Solar",       # OFS  1.103 — original gatekeeper
    "Outside Front Garage",      # OFG  1.73  — promoted in §11.79
    "Back Door Inside",          # BACK 1.85  — promoted in §11.79
    "Front Door Outside",        # FDO  1.39  — promoted in §11.79
    "Outside Back Solar",        # OBS  1.113 — promoted in §11.79
    "Outside Front Power",       # OFP  1.108 — promoted in §11.79
})


# Phase.106 — person-gatekeeper cameras. Phase.140 (2026-08-27):
# switched from Front Door Outside (RLC-833A, close-mounted) to Outside
# Front Garage (RLC-510A, points at the driveway leading to the front
# door and front garage). OFG has its own persistent RTSP reader
# (boot loop at line ~1853), same hardware as OFS (no Reolink-510A
# pre-buffer bug risk), and a wider view that captures the full
# approach + entry sequence rather than just the door panel. FDO is
# now class_disabled for person events (see DISABLED_CAMERA_EVENTS)
# — it stays in the network for vehicle/animal/motion events but no
# longer routes to the person pipeline. Will revisit FDO when §11.36b
...
#
# §11.80 (2026-08-28) — expanded from {OFG} to all 6 active cameras
# alongside GATEKEEPER_CAMERAS expansion. Note 2026-08-28: "make
# every camera a person gatekeeper camera and every camera a vehicle
# gatekeeper camera. When I get too many alerts I'll let you know."
# Note: with all 6 cameras in both sets, ALL_GATEKEEPER_CAMERAS is now
# the same 6-camera set. Per-camera thresholds in motion_gate_thresholds.json
# already tune individual sensitivity (OFS person=0.3, OFG=0.35,
# BACK/FDO=0.3, OBS=0.35, OFP=0.35-default). The §11.77 gate_cooldown
# at person=120s on all 6 cameras caps per-camera person floods to
# 1/min/camera (added to OFP as part of §11.80 since it was missing).
# Mirrored at listener/_motion_gate_dispatch.py:104.
PERSON_GATEKEEPER_CAMERAS = frozenset({
    "Outside Front Solar",       # OFS  1.103 — promoted §11.80
    "Outside Front Garage",      # OFG  1.73  — original (Phase.140)
    "Back Door Inside",          # BACK 1.85  — promoted §11.80
    "Front Door Outside",        # FDO  1.39  — promoted §11.80
    "Outside Back Solar",        # OBS  1.113 — promoted §11.80
    "Outside Front Power",       # OFP  1.108 — promoted §11.80
})

# Phase.88 / PLAN §11.19 — operator shorthand for the /snapshot
# endpoint. Operator says "give me an OFS snapshot" in chat; curl
# /snapshot?camera=OFS. Add new gatekeeper cameras here as they
# enroll so the shorthand stays one-look-up. Module-level so tests
# can import the resolver directly without closure introspection.
_SNAPSHOT_CAMERA_ALIASES: dict[str, str] = {
    "OFS": "Outside Front Solar",
    "OFG": "Outside Front Garage",
    "BACK": "Back Door Inside",
    "FDO": "Front Door Outside",
    "OBS": "Outside Back Solar",
    "OFP": "Outside Front Power",
}


def _resolve_snapshot_camera(arg: str, cameras: dict) -> str | None:
    """Map a /snapshot camera arg to its canonical name.

    Accepts:
      - canonical name already known to the listener (case-sensitive
        match against the cameras dict)
      - shorthand from _SNAPSHOT_CAMERA_ALIASES

    Returns the canonical name, or None if the arg is unknown.
    Does not lowercase canonical names — typos like 'outside front
    solar' return None on purpose so the operator sees a clear 400
...
# Origin: 2026-08-03 (OFS only). 2026-08-17 (Phase.87, PLAN §11.17)
# added Outside Front Garage as a second gatekeeper camera — both got
# the 8s deferred capture + pre-event motion trail (frame_offsets
# [0, 30, 60, 90, 120, 150]) and a dedicated persistent RTSP reader
# (QUEUE_GATEKEEPER_VEHICLE, Worker 0). 2026-08-20 (Phase.104)
# demoted OFG back out of the vehicle gatekeeper tier per Note —
# "I just want OFG to be like the other cameras. Not a vehicle motion
# gatekeeper anymore." OFG vehicle events now flow through
# QUEUE_OTHER_VEHICLE (no capture delay, no match-alert Telegram stack).
# OFG retains its persistent RTSP reader (still in the boot loop at
# line ~4142) because persistent RTSP is about reliable frame capture
# (Reolink pre-buffer-dump fix), not vehicle gatekeeping.

# Phase.62 (2026-08-07) — Deferred capture delay for gatekeeper cameras.
...
#
# Tuned 4s → 6s on 2026-08-07 per Note: "I really like the four second
...
#
# Rationale: the OFS smart_vehicle detector fires when the truck is already
# mid-yard (~12s after it first entered the FOV). Capturing immediately means
# the trail only shows the truck already parked. Waiting 8s shifts the
# 6-frame trail forward — frame_006 now catches the truck arriving at the
...
def _classify_queue(camera_name: str, event_type: str) -> str | None:
    ...
    the (camera, event_type) combo is in DISABLED_CAMERA_EVENTS (Phase.53).

    Phase.16 routing map (highest to lowest priority):
    ...
    Phase.53: hard-disable list checked first. If `(camera, event)`
    ...
    # Phase.53 — listener-side disable for selected camera/event combos
    if (c, e) in DISABLED_CAMERA_EVENTS:
        return None
    if e == "vehicle":
        if c in GATEKEEPER_CAMERAS:
...
# Phase.53 — listener-side disable for selected (camera, event_type)
...
DISABLED_CAMERA_EVENTS: frozenset[tuple[str, str]] = frozenset({
    ...
    # Pre-§11.80 this set contained:
    #   (OFS, person), (OFS, people) — OFS class_disabled for persons
    #   (OFS, animal) — KEEP (animal events on OFS remain dropped; only
    #     §11.80 promoted vehicle+person, not animal)
    #   (OFP, person), (OFP, people) — OFP perimeter noise filter (6B.73)
    #   (OBS, person), (OBS, people) — OBS work-area noise filter (6B.73)
    #   (FDO, person), (FDO, people) — FDO door-panel noise filter (6B.140)
    # All but the OFS animal entry are now removed per Note
    # 2026-08-28: "make every camera a person gatekeeper camera. When I
    # get too many alerts I'll let you know." Per-camera thresholds
    # (OFS person=0.3, etc.) and gate_cooldown.person=120s already
    # suppress the worst false positives.
    ("Outside Front Solar", "animal"),
})
...
class _ClassedWebhookExecutor:
    ...
    Phase.16. Replaces the single-FIFO `_BoundedWebhookExecutor`
    ...
    def __init__(self, max_workers: int = 4, queue_capacity: int = 64) -> None:
        ...
        self._rejected_per_class: dict[str, int] = {
            ...
            self.QUEUE_ANIMAL: zero,
            self.QUEUE_MOTION: zero,
            # Phase.53 — webhooks dropped because (camera, event_type)
        ...
        for i in range(max_workers):
            if i == 0:
                ...
                # lane so an OFS vehicle alert never waits behind a
    ...
    def submit(self, fn, *args, **kwargs):
        ...
        Phase.16 also exposes `submit_for_class(fn, camera_name,
    ...
    def submit_for_class(
        ...
        (Phase.53). When disabled, logs an audit line and bumps
    ...
    def _gatekeeper_worker_loop(self) -> None:
        ...
        Phase 2026-08-03. Exists so the OFS vehicle lane never blocks
    ...
    def queue_depths(self) -> dict[str, int]:
        """Per-queue depth (Phase.16 stats endpoint)."""
...
# `_BoundedWebhookExecutor` directly. Phase.16 keeps this name pointing
...
def _should_debounce(camera_name: str, window_seconds: float | None = None) -> bool:
    ...
    Args:
        camera_name: Friendly camera name (e.g. "Front Door Outside")
...
def _load_cameras_from_creds() -> dict:
    ...
    Returns a dict keyed by friendly camera name (e.g. "Front Door Outside"):
        {"Front Door Outside": {"rtsp_url": "...", "ip": "192.168.1.39"},
         "Back Door Inside":   {"rtsp_url": "...", "ip": "192.168.1.85"}}
...
def create_app(test_config: dict | None = None) -> Flask:
    ...
    def status():
        ...
        # Phase.25 — per-pass matcher telemetry. Read-only snapshot;
        ...
        # Phase.47 — merge scored shadow counts into the same telemetry
        ...
        # Phase.102 (2026-08-20) — `prompt_mode` block removed. Phase.78
    ...
    def snapshot():
        ...
        as a JPEG image. Phase.88 / PLAN §11.19.

        Query params:
          camera (required): shorthand ('OFS', 'OFG') or canonical name
                             ('Outside Front Solar', etc.).
        ...
        with tempfile.TemporaryDirectory(prefix="listener_snapshot_") as td:
            ...
            # Phase.88 — disable caching. Operators hitting
            # /snapshot?camera=OFS expect the current frame, not
    ...
    def preview():
        ...
        Read-only. Returns a <nas> file directly via send_file;
        no caching of our own.
        """
        from datetime import datetime, timedelta, timezone

        from infra.<nas>_preview import find_preview
        ...
        if result is None:
            return jsonify({
                ...
                "detail": (
                    f"no <nas> preview for camera={cam_arg!r} at "
        ...
        # <nas> files don't change retroactively, but disable
    ...
    def receive_alert():
        ...
        # camera-creds.env. Currently only "Front Corner Inside" -> "Front
        # Door Outside" is mapped; the legacy "Building back solar" alias
        # was removed 2026-07-29 when the camera was physically retired.
        ...
        # Validate IP matches (prevent spoofing)
        expected_ip = cameras[camera_name]["ip"]
        if ip != expected_ip:
            log.warning(
        ...
        # Phase.16 — route the alert to its class queue based on
        # the camera + event type, so vehicle events (especially from
        # the gatekeeper camera, Outside Front Solar) jump ahead of
        # person/animal/motion events.
        # Phase.53 — snapshot class_disabled counter so we can
        ...
        future = executor.submit_for_class(
            # Phase.62 — gatekeeper cameras get a configured deferred capture
        ...
        if future is None:
            ...
            #      (Phase.53) → 202, the drop is intentional.
...
def _normalize_payload(payload: dict, source_ip: str) -> dict | None:
    ...
    2. Reolink default webhook shape:
        {
          ...
          "alarm": {
            ...
            "message": "...",
            "name": "Front Door Outside",  # ← this is the camera's friendly name
            "time": "ISO timestamp",
    ...
    - For Reolink shape: match `alarm.name` to known camera's friendly name.
      ...
      RLC-510A swap the fleet is mixed (4x RLC-510A new + 2x RLC-833A
    ...
    if "alarm" in payload and isinstance(payload["alarm"], dict):
        ...
        # "Person Detected from Front Door Outside". Prefer channelName.
        device_name = (
        ...
        # Phase.99 (PLAN.md §11.26): every timestamp visible to the user
...
# Phase.158 (2026-08-28) — §11.81: unified Qwen3.6-35B-A3B server.
...
# Phase.62 (2026-08-07) — Deferred capture wrapper for gatekeeper cameras.
...
def _process_alert_with_gatekeeper_delay(
    ...
    if camera_name not in GATEKEEPER_CAMERAS:
...
def _load_telegram_creds() -> tuple[str, str]:
    ...
    Phase.9 "arriving" message can fire right after phase-1 capture,
...
# ---------------------------------------------------------------------------
# Phase.106 — Telegram body format helpers moved to telegram_formatter/
# ---------------------------------------------------------------------------
...
def _process_alert(
    ...
    Phase.105c (2026-08-21): the original 1754-line `_process_alert` block
    ...
    Phase.106 (2026-08-22): person events on PERSON_GATEKEEPER_CAMERAS
    ...
    """
    # Phase.107 (§11.37) + 6B.108a-rev1 (§11.38.13) — motion gate runs
    # FIRST, ABOVE all routing logic. Gate verdict is suppress → exit; pass →
    # fall through to the existing camera-based routing (FDO person events still
    ...
    # camera-based routing. Per Note 2026-08-23: keep gate + redundant pipeline
    # for now (safer); pipeline simplification (6B.108 §11.38.3-§11.38.12) is
    # deferred to a follow-on phase.

    # Phase.154 (PLAN §11.77): per-camera × per-event-type cooldown at the
    ...
    # no audit row. Note 2026-08-28: "Can we do a cool down per camera
    ...
    gate_verdict = maybe_run_motion_gate(
        ...
        timestamp=timestamp,  # Phase.116 timestamp-fix
        event_type=event,     # Phase.152 — per-camera × per-event-type config
    )
    if gate_verdict is not None:
        ...
        # alert" — we route to vehicle pipeline. Phase.161 (2026-08-28).
        ...
        # Otherwise: fall through to legacy camera-based routing. The gate's
        # verdict is logged but does NOT change the routing decision (gate
        # only adds suppress-on-noise; routing is camera+event based, as
        # designed in Phase.106).
        log.info(
            f"[{alert_id}] motion_gate: pass (decision={gate_verdict.decision} "
            f"class={gate_verdict.class_label} "
            f"conf={gate_verdict.confidence:.2f}) — continuing to legacy routing"
        )

    # Phase.106 — route person events on person-gatekeeper cameras
    ...
    # Phase.108a (§11.38.6) — pass the gate verdict through so the
    # person pipeline can reuse the gate's 4 frames when
    # PIPELINE_USES_GATE_CROPS=1.
    #
    # Phase.145 (§11.67) — promote `event=md` to 'person' when the
    ...
    if (
        ...
        and camera_name in PERSON_GATEKEEPER_CAMERAS
    ):
    ...
    # Phase.165 (PLAN §11.86) — animal event pipeline scaffold.
    ...
    # Lazy imports — the heavy infrastructure modules (frame_capture, motion_detector,
    # vision_analyzer, alert_generator, notifier, …) load on first stage call.
    # Dual-context import: bare 'vehicle_event_pipeline' works when listener.py
    # runs as __main__ (sys.path[0] = listener/); 'listener.vehicle_event_pipeline'
    # works when listener.py is imported as listener.listener (tests).
    try:
        from vehicle_event_pipeline import AlertContext, process_alert
    except ImportError:
        from listener.vehicle_event_pipeline import AlertContext, process_alert

    # Load Telegram creds up front — Phase.9 message 1 ("arriving") fires before
    ...
    # Phase.129a (§11.51) — promote non-vehicle events to vehicle when
    ...
    ctx = AlertContext(
        ...
        # Phase.108a (§11.38.6) — pass the gate verdict onto the
...
def _process_person_alert(
    ...
    """Phase.106 — person-gatekeeper pipeline driver.
    ...
    Phase.108a (§11.38.6) — gate_verdict kwarg added. When the
    ...
    ctx = PersonContext(
        ...
        chat_id=_chat_id,
        api_url=VISION_API_URL,
        # Phase.108a (§11.38.6) — same wiring as AlertContext.
        gate_verdict=gate_verdict if gate_verdict is not None and not gate_verdict.is_suppress else None,
    )
...
def _process_animal_alert(
    ...
    """Phase.165 (§11.86.1) — animal event pipeline driver (scaffold).
    ...
    Args:
        ...
        gate_verdict: Optional motion-gate verdict (Phase.108a
...
if __name__ == "__main__":
    # Phase.38 — Test/prod isolation. Defense-in-depth: if a test
    ...
    # Phase.150: heartbeat thread removed (Note 2026-08-28:
    ...
    # Start a persistent RTSP reader for OFS (and OFG) to avoid the
    # Reolink 510A pre-buffer dump + replay bug. Holding the connection
    ...
    # Origin: 2026-08-05 (OFS); 2026-08-17 adds OFG (Phase.87).
    # 2026-08-20 (Phase.104) — OFG demoted from the *vehicle*
    # gatekeeper tier (no longer in GATEKEEPER_CAMERAS at line ~297)
    # but kept on the persistent RTSP list because persistent RTSP is
    # about reliable frame capture, not vehicle gatekeeping. If OFG
    # later needs to drop persistent RTSP too, remove it from the
    # tuple below. See PLAN §11.32.
    # The registry is per-camera (keyed by canonical name); each
    # persistent RTSP camera boots independently, so a failure on one
    # doesn't take down the others.
    from infra.frame_capture import load_camera_creds as _lcc
    from infra.persistent_rtsp import (
        PersistentRTSPReader,
        get_reader,
        set_reader,
    )
    _creds_map = _lcc(CAMERA_CREDS_FILE)

    # Phase.151 originally wired persistent RTSP to all 6 cameras.
    ...
    # 2026-08-28 — Phase.160: reverted to all-6 cameras. All Reolink
    ...
    for _camera_name in (
        "Outside Front Solar",      # OFS 1.103
        "Front Door Outside",       # FDO 1.39
        "Outside Front Garage",     # OFG 1.73
        "Back Door Inside",         # BDI 1.85
        "Outside Front Power",      # OFP 1.108
        "Outside Back Solar",       # OBS 1.113
    ):
        try:
            _url = _creds_map.get(_camera_name, {}).get("rtsp_url")
            if not _url:
                log.warning(
                    "%s RTSP URL not found in camera-creds.env — "
                    "persistent reader NOT started.",
                    _camera_name,
                )
                continue
            # Phase.80 (2026-08-16, PLAN §11.13) — scheduled_reconnect_seconds
    ...
    # Phase.25 — write matcher_telemetry.json every 5 minutes so
