...
def _send_arriving_message(
    ...
    detected, identifying...") on every OFS vehicle event where motion
    detector confirms motion. The previous VEHICLE_ARRIVING_ENABLED env
...
def identify_stage(ctx: AlertContext) -> None:
    ...
    # Per Note's spec, TG#1 is OFS-only. Non-gatekeeper vehicle events
    # (OFG, Back Door Inside, Front Door Outside) skip TG#1 entirely —
    # the entire vehicle Telegram stack is the gatekeeper's exclusive
    # job until/unless Note later adds per-camera channels.
...
def _emit_match_loop(ctx: AlertContext) -> None:
    """Phase.112: per-vehicle match loop — TG#3a / TG#3b in the OFS stack.
...
def emit_result_stage(ctx: AlertContext) -> dict:
    ...
    # photo. Note's spec (2026-08-21) defines a 3-Telegram OFS stack:
    ...
    # the 3-Telegram OFS message stack (TG#2 + TG#3). TG#1 already fired
    ...
    # 7. composite motion-trail Telegram (TG#2 in the OFS stack).
    ...
    # 8. per-vehicle match loop (TG#3 in the OFS stack).
    ...
    if _defer_alert:
        if ctx.match_alerts:
            try:
                ...
                #   L1 = known vehicle at OFS (you want Telegram alert)
                #   L2 = unknown vehicle at OFS (higher alert — no match)
