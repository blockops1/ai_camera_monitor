...
# the operator observed they were all at the same instant (recent_frames pulled
...
# CAM2→CAM3, CAM3→CAM4,
# CAM1→CAM5, CAM4→CAM6). The operator's
...
GATEKEEPER_CAMERAS = frozenset({
    ...
    "CAM3",  # CAM2 — promoted §11.79
    "CAM2",  # BACK                      — promoted §11.79
    "CAM1",  # FRONT                     — promoted §11.79
    "CAM6",  # CAM4        — promoted §11.79
    "CAM4",  # CAM3       — promoted §11.79
})
...
def _compute_gatekeeper_offsets(
    ...
    webhook_offsets_seconds: tuple[float, ...] = (-2.0, 0.0, 2.0, 4.0),
) -> tuple[int, ...]:
    """Compute deque indices for the gate trail.

    the operator (2026-08-28): the camera waits ~2s to verify motion before
    firing the webhook, so we want a trail that starts BEFORE the
    webhook moment (the motion that triggered the camera) and ends
    AFTER it (the motion that confirmed detection). Default anchors:
        T_w - 2s, T_w + 0s, T_w + 2s, T_w + 4s
    which is a 6-second window at 2-second spacing, centered on the
    webhook moment.
    ...
    Examples (ring_len=32, fps=2, capture_delay=8s):
      webhook_offsets_seconds = (-2, 0, 2, 4)
        → indices = (11, 15, 19, 23)
        = T_w-2s, T_w+0s, T_w+2s, T_w+4s
...
def is_motion_gate_enabled() -> bool:
    ...
    The cutover plan (§11.37):
      ...
      - After 1 week of validation: the operator approves, default flips to 1
    """
...
def maybe_run_motion_gate(
    ...
    # Phase.128 — gatekeeper cameras (CAM1) sample 4 evenly-spaced frames
