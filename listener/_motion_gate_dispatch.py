...
STATUS: provisional (Phase 6B.107 §11.37, 2026-08-23)
...
# Phase 6B.152 — per-camera gate_enabled lookup. Dual-context import
...
# Phase 6B.128 — CAM1 gatekeeper gets a 4-frame pre-event motion trail
# (indices [0, 30, 60, 90] at 15fps → T-6s through T+0s of camera time,
# 2s spacing). All other cameras keep the trailing-tail behavior.
#
# Phase 6B.143 (2026-08-27, maintainer OOB): CAM2 (Outside Front Garage) joins
...
# maintainer observed they were all at the same instant (recent_frames pulled
# 4 consecutive frames at ~67ms apart) instead of the spaced trail he
# expects. Adding CAM2 here makes its gate capture use the offset path,
# mirroring CAM1. Qwen still gets frames[1]+frames[2] bracketing the
# motion event, so the spacing change is invisible to vision but
# visible in the Telegram album.
#
# Phase 6B.160 (2026-08-28, maintainer OOB): the offsets now anchor the
...
# Phase 6B.143: capture the same trail (offsets counting backward from
...
# 4 evenly-spaced deque indices for the gate. Phase 6B.160 (2026-08-28):
...
# Phase 6B.128 replaced the previous "trailing 4 frames from the
...
# Phase 6B.160 — Back-compat constant. The live path computes offsets at
...
def _compute_gatekeeper_offsets(
    ...
    maintainer (2026-09-01, Phase 6B.174): trail shifted +4s from previous
    ...
    Previous (2026-08-28): maintainer wanted pre+post webhook context
    (`T_w-2s, T_w+0s, T_w+2s, T_w+4s`). That assumption was wrong for
    REOLINK_MODEL with `delay_person=0` — the camera fires the webhook
...
def is_motion_gate_enabled() -> bool:
    ...
    The cutover plan (§11.37):
      ...
      - After 1 week of validation: maintainer approves, default flips to 1
    """
...
def maybe_run_motion_gate(
    ...
    Returns:
      ...
      - None if the gate is disabled for this (camera, event_type) combination
        ...
        legacy path (Phase 6B.152, PLAN §11.75)
    ...
    Args:
      timestamp: Phase 6B.116 timestamp-fix. ISO string (webhook format) or
      ...
      event_type: Phase 6B.152. Webhook event type (vehicle/person/md/...).
    ...
    # Phase 6B.152 — per-camera × per-event-type gate configuration.
    ...
    # Phase 6B.128 — gatekeeper cameras (CAM1) sample 4 evenly-spaced frames
    ...
    # Phase 6B.143 (2026-08-27, maintainer OOB): gatekeeper-trail capture now
    # applies to BOTH vehicle gatekeepers (CAM1) and person gatekeepers
    # (CAM2). Previously only CAM1 got the [0, 30, 60, 90] pre-event
    # trail — CAM2 was using recent_frames which gave 4 consecutive
    # frames at ~67ms apart. Now both use the spaced trail.
    # Phase 6B.167 §13.4 Commit 17 (T3 C17): resolved via _code_for_camera
    ...
    # Phase 6B.160 — query the persistent reader's actual fps so the offsets
    ...
    if is_gatekeeper:
        ...
        # Phase 6B.160 (2026-08-28): use the user-specified trail anchors
    ...
    # Run the gate
    try:
        # Phase 6B.116 timestamp-fix: parse ISO string from webhook to a
