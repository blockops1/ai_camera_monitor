...
CONSTANTS:
    VISION_CONFIDENCE_FLOOR: float = 0.3
        Phase 6B.170 (2026-09-01): minimum Qwen confidence for match-alert
...
# Phase 6B.116 + §11.170 (2026-09-02): module-level imports of
...
# Phase 6B.170 (2026-09-01): confidence floor for the
...
def match_stage(ctx: AlertContext) -> None:
    ...
    if ctx.camera_code not in ctx.gatekeeper_cameras:
        # Phase 6B.168 (2026-08-31): compare camera_code (CAM{N}) against
    ...
    # Phase 6B.170 (2026-09-01): suppress match-alert path when
    ...
    if match_result is None:
        ...
        from vehicle_matcher.matcher import (
            NoMatch,  # 6B.90 package-form (per ai_camera_monitor skill)
        )
...
def _extract_signature(ctx: AlertContext) -> dict:
    ...
    Phase 6B.129b (§11.52): now reads from vehicles[] first (the
...
def _vision_summary_str(vision_result) -> str:
    ...
    Phase 6B.129 (initial): looked at vision_result["vehicles"][0] first
    (multi-vehicle schema) then fell back to top-level fields.

    Phase 6B.130 (§11.53): handles multi-vehicle results correctly.
    ...
    # Multi-vehicle schema (Phase 6B.129 + 6B.130)
...
def _emit_match_loop(ctx: AlertContext) -> None:
    """Phase 6B.112: per-vehicle match loop — TG#3a / TG#3b in the gatekeeper stack.

    Fires AFTER TG#1 (arriving) and TG#2 (vehicle in motion + composite).
    Per maintainer OOB 2026-08-21: "the matcher should run after the other two
    ...
    for v_idx, veh in enumerate(vehicles):
        ...
        if match_detail is None:
            ...
            # Phase 6B.122 (2026-08-22, maintainer OOB): stash the
            ...
            # Phase 6B.121 (2026-08-22): stash the MatchTelegramInput so
