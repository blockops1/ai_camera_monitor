...
PUBLIC API:
    capture_stage(ctx: AlertContext) -> None
        LEGACY capture stub. Phase 6B.115 (2026-08-25) removed the
...
DOES NOT DO:
    - Actually capture frames from RTSP — that path is REMOVED in
      Phase 6B.115. The motion gate is the sole producer now.
...
WHY HERE:
    Phase 6B.115 removed the RTSP-based capture path. This stub
...
def capture_stage(ctx: AlertContext) -> None:
    ...
    Phase 6B.115 (2026-08-25): REMOVED. The motion gate is now the sole
    ...
    """
    log.warning(
        f"[{ctx.alert_id}] capture_stage: DEPRECATED 2026-08-25 (Phase 6B.115) "
