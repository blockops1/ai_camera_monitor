...
OUTPUTS:
    - mutates ctx: motion_result, id_result, vision_result, vision_error,
      ...
      pairwise_diff_path (Phase 6B.144), frame_positions (Phase 6B.111).
...
PUBLIC API:
    ...
    _coerce_vision_result(ctx: AlertContext) -> None
        Phase 6B.87: handle VisionResult vs VisionError vs dict vs None
...
def identify_stage(ctx: AlertContext) -> None:
    ...
    # --- Motion detector (Phase 6B.115: gate-only path, in-memory) ---
    # Phase 6B.115 (§11.46.6): the motion gate is the sole producer of
    ...
    if ctx.is_vehicle_event and len(getattr(ctx, "frames", [])) >= 4:
        ...
        if verdict is not None:
            ...
            # Phase 6B.144 (§11.66): pull the gate's pairwise diff
    ...
    if (
        ...
        if ctx.motion_result is not None and not ctx.motion_result.no_motion_detected:
            ...
            if crop_paths:
                try:
                    # Phase 6B.144 (§11.66): pass the gate's pairwise
        ...
        # Phase 6B.132 (§11.54): no fallback. Vehicle events without
        ...
        # Non-vehicle: single-frame first-pass. Phase 6B.132 keeps this
        ...
        # Phase 6B.78 (motion is owned by the gate).
        if not (
            ctx.is_vehicle_event and cascade_provided_vision
        ):
            _non_vehicle_first_pass(ctx)

    # --- Phase 6B.111: trajectory injection (POST-vision) ---
    ...
    # --- Phase 6B.112: TG#1 ("arriving" Telegram) ---
    # Fires at the end of identify_stage, AFTER motion detector confirms
    # motion + vision ran. Per maintainer's spec (2026-08-21), TG#1 fires
    ...
    #
    # Phase 6B.168 (2026-08-31): gate on camera_code ∈ gatekeeper_cameras.
    ...
    # Per maintainer's spec, TG#1 is gatekeeper-only. Non-gatekeeper vehicle
    # events skip TG#1 entirely — the entire vehicle Telegram stack is
    # the gatekeeper's exclusive job until/unless maintainer later adds
    # per-camera channels.
    ...
    if (
        ...
        # Phase 6B.121 (2026-08-22): pick the FIRST frame where the vehicle
...
def _coerce_vision_result(ctx: AlertContext) -> None:
    ...
    Phase 6B.87: identify_from_crops returns vision_result as a
...
def _non_vehicle_first_pass(ctx: AlertContext) -> None:
    ...
    Phase 6B.132: the previous vehicle-without-crops caller was deleted
    (vehicle events without crops from the gate are suppressed — see
    §11.54, maintainer 2026-08-26:
    ...
    gate (per maintainer's plan: "Let's get the vehicle system working
