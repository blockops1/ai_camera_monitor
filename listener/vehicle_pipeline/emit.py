...
WHY HERE:
    ...
    match_loop TG#3 (maintainer OOB 2026-08-21).
...
def emit_result_stage(ctx: AlertContext) -> dict:
    ...
    Order matters:
        ...
        7.5. composite Telegram (Phase 6B.111 — motion-trail visualization
    ...
    # Phase 6B.112: notify() (the LLM-generated alert body Telegram)
    # was REMOVED from this stage. The slim previously sent a single
    # Telegram with the Qwen9B-generated title+summary + best-frame
    # maintainer's spec (2026-08-21) defines a 3-Telegram gatekeeper stack:
    ...
    # Phase 6B.122 (2026-08-22, maintainer OOB): defer append_alert until
    ...
    # Phase 6B.168 (2026-08-31): same camera_code-vs-gatekeeper gate as
    ...
    # Phase 6B.112: split the single notify() (LLM body Telegram) into
    ...
    # Fires AFTER TG#1 (arriving — fired in identify_stage). Per maintainer
    ...
    # Phase 6B.168 (2026-08-31): same camera_code gate as TG#1.
    ...
    if (
        ...
        try:
            ...
            # Phase 6B.115: read bbox_a + bbox_b from the gate verdict
            ...
            composite_sent = send_composite_alert(
                ...
                frames=ctx.frames,                # Phase 6B.115: in-memory PIL images
    ...
    # Runs AFTER TG#1 + TG#2 (per maintainer OOB 2026-08-21). For each vehicle
    ...
    # Phase 6B.168 (2026-08-31): camera_code gate; pre-fix silently
    ...
    # Phase 6B.122 (2026-08-22, maintainer OOB): if the match_loop populated
    ...
    if _defer_alert:
        if ctx.match_alerts:
            try:
                ...
                # Phase 6B.122 (2026-08-22): handle BOTH match and
                ...
                # Phase 6B.123 (2026-08-22, maintainer OOB): vehicle threat
                # level routing per maintainer's spec:
                ...
                #       Per maintainer 2026-08-22: "we can add other
                ...
                if is_match:
                    ...
                    # No match — unknown vehicle. L2 per maintainer spec.
