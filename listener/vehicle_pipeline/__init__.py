...
PACKAGE STRUCTURE:
    ...
    capture.py       — capture_stage (Phase.115 legacy stub)
...
DOES NOT DO:
    - Decide which event type routes here — the listener's _process_alert
      owns that. (Phase.106 + §11.111: gate is suppress-only, routing
...
WHY HERE:
    Split from listener/vehicle_event_pipeline.py in Phase.170 (§11.111,
    ...
    package-shadowing bug fixed in Phase.116. If anything in the
...
# Phase.116: module-load imports. MUST be at module-load time, not
