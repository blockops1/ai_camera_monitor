...
PUBLIC API:
    _send_arriving_message(alert_id, camera_name, frame_path, bot_token, chat_id, captured_at="") -> bool
        Phase 6B.112 (2026-08-21): TG#1 ("vehicle detected, identifying...")
...
def _send_arriving_message(
    ...
    """
    Phase 6B.9 message 1: instant heads-up that a vehicle is on the
    property and the system is identifying it.

    Inlined from listener.listener._process_alert (Phase 6B.105c, 2026-08-21)
    ...
    Phase 6B.112 (2026-08-21): maintainer spec calls for TG#1 ("vehicle
    ...
    RETIRED — it predated maintainer's current spec. The Telegram now fires
    ...
    Phase 6B.113 (2026-08-21): Switched from `infra.notifier.notify()`
    ...
    Phase 6B.114 (2026-08-25): Removed the [alert_id] prefix (diagnostic
    ...
    with logs. Event time, not send time, per maintainer OOB correction
    ...
    # Phase 6B.114: footer with event time at the end.
    body_lines = [
