...
Phase.115 (§11.46.6, 2026-08-25): the gate now returns frames + crops
...
Phase.139 (§11.60, 2026-08-27): person pipeline gets the same gate-aware
...
RELATED:
  - PLAN.md §11.46.6 (Phase.115, in-memory frames via verdict)
  - PLAN.md §11.38 (gate-aware pipeline cutover, predecessor)
  - PLAN.md §11.60 (Phase.139, person path mirror — this commit)
...
# objects which we save to disk via .save(). Phase.139 disk-write
...
class SkipEvent(Exception):
    ...
    Phase.115: legacy fallback is gone. If the gate didn't run (no
...
def gate_aware_vehicle_capture(ctx) -> None:
    ...
    Phase.115 (§11.46.6): the gate returns frames + crops as
    ...
    if verdict is None:
        log.error(
            ...
            f"motion gate did not run. Phase.115: no legacy fallback. "
...
# --- Person path (Phase.139, PLAN §11.60) ---------------------------
#
...
def gate_aware_person_capture(ctx) -> None:
    ...
    Phase.139 (§11.60, 2026-08-27): the gate is now the sole producer
    ...
    if verdict is None:
        log.error(
            ...
            f"motion gate did not run. Phase.139: no legacy fallback. "
            f"Alert will be sent=False."
        )
        ctx.capture_source = "missing"
        raise SkipEvent("no gate verdict")
    ...
    # Phase.140 will read selected_frames for best-frame selection.
    ...
    if len(verdict_paths) >= 4:
        ...
        if not output_dir:
            ...
            for i, pil_img in enumerate(selected_pil, start=1):
                p = os.path.join(output_dir, f"frame_gate_{i:03d}.jpg")
                try:
