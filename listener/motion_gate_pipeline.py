...
STATUS: provisional (Phase.107 §11.37, 2026-08-23; Phase.109 §11.39 v2 fixes 2026-08-24;
        ...
        Phase.169 §11.93 crop source-frame fix 2026-08-31;
        Phase.170 vision-confidence floor 2026-09-01;
        Phase.171 subject_bbox_from_mask 2026-09-01;
        Phase.172 STRICT commit — no fallbacks 2026-09-01;
        Phase.173 two-mask intersection anchor 2026-09-01;
        Phase.174 gatekeeper offset shift +4s 2026-09-01;
        Phase.175 two per-frame subject bboxes (independent ANDs) 2026-09-01)
...
DOES NOT DO:
  ...
  - Does NOT fall back to the diff bbox when subject detection fails
    (Phase.172 STRICT — caller suppresses the alert instead).
    Honest detection beats plausible-looking crops.
  - Does NOT compute per-crop subject bboxes (Phase.175 — two
...
Architecture (LOCKED §11.37, modified §11.66 + §11.93 Phase.169):
...
Phase.169 §11.93 (2026-08-31): crop source frame corrected. The diff
bbox covers the *trail* of motion between two frames — at the LATER
frame of the pair, the moving subject has often already left the bbox
(by the time the pixel diff was computed, the subject was somewhere
later in its trajectory). Applying the bbox to the EARLIER of the two
frames in the pair keeps the subject visible inside the crop. This
matches the existing convention in `infra/motion_detector._crop_one`
(comment: "the 'departure' region ... corresponds to the moving
object's position on frame_paths[frame_idx - 1]"). Previously
`motion_gate_pipeline` applied each diff bbox to the LATER frame in
the pair, producing empty crops whenever the subject had moved past
the bbox during the 1-2s inter-frame interval (Note: "about half
the crops have no vehicle in them").
...
def _write_pairwise_diff_image(
    ...
    out_path = out_dir / "pairwise_diff.png"
    try:
...
class GateVerdict:
    ...
    Fields:
      ...
      crop_bbox_a: (x, y, w, h) ACTUALLY used for crop_a — Phase.175:
        equals subject_bbox_a (logical AND of diff(1,2) and diff(2,3)).
        None if no motion OR if subject detection failed.
      crop_bbox_b: (x, y, w, h) ACTUALLY used for crop_b — Phase.175:
    ...
    # Phase.175: two per-frame subject bboxes, each from the logical
...
def _classify_crop(
    ...
    # person threshold (e.g. 0.30 for Outside Front Solar) is permissive.
...
def run(
    ...
    Per §11.37 LOCKED architecture + §11.93 (Phase.169) crop fix +
    Phase.175 per-frame AND bboxes:
      ...
      - diff(frame_2, frame_3) + diff(frame_3, frame_4) → AND → subject_bbox_b
        ...
        (Each bbox is applied to the frame it was computed for. See
         Phase.175 PLAN entry: replaces 6B.173's single-subject-bbox
    ...
    # ---- Phase.175: TWO per-frame subject bboxes ----
    # Note 2026-09-01 (fe3f88c6 walk-test): "the AND of 1→2 and 2→3 should
    ...
    # Phase.175: each crop uses its own AND-bbox on the frame it was
    ...
    log.info(
        f"[{alert_id}] motion_gate: Phase.175 per-frame logical-AND bbox — "
        f"crop_bbox_a={crop_bbox_a} crop_bbox_b={crop_bbox_b}"
    )

    # ---- edge case: no subject detected by either crop ----
    # Phase.171 STRICT commit. Two failure modes collapse here:
    ...
    # No V2 carve-out — if diff is empty, suppress. Note 2026-09-01:
    ...
    # Phase.169 §11.93: bbox_a/b describe the diff TRAIL between two
    ...
    # the earlier frame.
    # Phase.175: crop_bbox_a = subject_bbox_a (applied to frame_2),
    ...
    crop_b_path: str | None = None
    if keep_disk:
        crop_a_path = crop_frame_to_bbox(frame_2_path, crop_bbox_a) if crop_bbox_a else None
        crop_b_path = crop_frame_to_bbox(frame_3_path, crop_bbox_b) if crop_bbox_b else None
    ...
    # "the moving object is whatever lights up the diff."
    #
    # §11.115.21 (2026-09-03): boxes drawn on the diff PNG MUST match the
    # saved crop file extents. We pass crop_bbox_a / crop_bbox_b (the
    # per-frame subject AND bboxes used by crop_frame_to_bbox to write
    # crop_a.png / crop_b.png) — NOT the diff-trail bboxes (bbox_a,
    # bbox_b = raw diff(2,3) / diff(3,4) extents). Trail bboxes can
    # cover different regions than the actual subject, leaving the
    # user's eye confused about where the saved crops came from.
    pairwise_diff_path: str | None = None
