...
Design notes:
  ...
  - `other` class → log only, NO Telegram. This is Note's 2026-09-02 PM
    directive ("we're just gonna log for now. No telegram.").
  - Telegram always sends BOTH crops (Note: "I always want both crops
...
# Dual-context import for sibling module (listener.pipeline_filters).
# This module is imported via two paths:
#   (a) Production: listener.py runs as `python listener/listener.py`
#       (launchd plist). The fallback `from single_pipeline import …`
#       makes THIS module a top-level module with `__package__ == None`,
#       so a bare relative import (`from .pipeline_filters`) fails.
#   (b) Tests:      pytest sets `__package__ = "listener"` via PYTHONPATH.
#       The first branch (`from listener.pipeline_filters`) succeeds.
# We try bare name first because production is the hot path.
try:
    from pipeline_filters import (  # type: ignore[no-redef]
    ...
    *,
    ...
    Args:
        ...
        cooldown:     PipelineCooldown instance (§11.115.13). Pre-cascade
                       ...
                       every call.
        pairwise_diff_path: optional PNG path to the pairwise differential
                       image (abs(frame_3 − frame_4)) written by §6B.144.
                       When provided, it is prepended as the FIRST image
                       in the Qwen payload so Qwen sees motion context:
                       [diff, crop_a, crop_b]. Defaults to None for backward
                       compatibility (§11.115.5). §11.115.22.
    ...
    # ----- Stage 2.5: build the Qwen vision payload (§11.115.22) -----
    # When the motion gate wrote a pairwise differential image (§6B.144),
    # prepend it as the FIRST image so Qwen sees motion context. Without it,
    # Qwen has no way to disambiguate the moving subject from stationary
    # vehicles in the same frame — alert 9275a92b had 1 moving tractor past
    # 2 parked vehicles and Qwen returned 3 vehicles (causing duplicate TG#3).
    # With it, the diff shows where motion is; Qwen picks that subject.
    qwen_frame_paths: list[str] = (
        [pairwise_diff_path, crop_a, crop_b]
