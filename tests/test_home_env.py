...
class TestLoadHomeEnv:
    ...
    def test_telegram_token_length_revealed_not_value(self, tmp_path, monkeypatch, caplog):
        ...
        # 29-char fixture token (matches the shape of a real Telegram bot token).
        # Length is asserted as len(fixture_token) so this test does not rot
        # if the fixture string changes.
        fixture_token = "secret-value-1234567890abcdef"
        expected_len = len(fixture_token)
        env_file = tmp_path / ".env"
        env_file.write_text(f"TELEGRAM_BOT_TOKEN={fixture_token}\n")
        _redirect_repo_env(monkeypatch, env_file)
        # Redirect ~/.env so the test is hermetic and doesn't pick up
        # the developer's actual ~/.env token (which has a different length).
        monkeypatch.setattr(
            os.path, "expanduser",
            lambda p: str(tmp_path / "no_home.env") if p == "~/.env" else p,
        )
        ...
        assert any(fixture_token not in m for m in all_msgs)
        # At least one log line must include the token length.
        assert any(
            str(expected_len) in m and "token" in m.lower()
            for m in all_msgs
        ), (
            f"expected token length {expected_len} in some log line; got: {all_msgs}"
        )
